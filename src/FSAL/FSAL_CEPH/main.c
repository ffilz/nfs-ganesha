...
void create_unique_id(struct ceph_mount *cm, char *nodeid, char **uniq_id)
{
	...
	if (CephFSM.use_old_uuid) {
		...
		*uniq_id = gsh_malloc(len);
		...
		(void)snprintf(buff, 8192, "%s%s%s%s%d", nodeid, cm->cm_user_id,
				   ...
				   const struct fsal_up_vector *up_ops)
{
	...
	/* stx is filled in */
	bool stxr = false;
	/* client_id - unique for every client */
	char client_id[1024];
	...
	/* Set the client id */
	snprintf(client_id, 1024, "%s.%d", cm->cm_user_id, cm->cm_clnt_index);

	/* allocates ceph_mount_info */
	ceph_status = ceph_create(&cm->cmount, client_id);
