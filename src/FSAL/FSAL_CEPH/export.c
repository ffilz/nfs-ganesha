...
static void release(struct fsal_export *export_pub)
{
	...
	gsh_free(export->sec_label_xattr, MEM_COMP_CONFIG);
	gsh_free(export->fs_name, MEM_COMP_CONFIG);
	gsh_free(export->cmount_path, MEM_COMP_CONFIG);
	gsh_free(export->user_id, MEM_COMP_CONFIG);
	gsh_free(export->secret_key, MEM_COMP_CONFIG);
