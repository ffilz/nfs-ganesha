			    ...
			    struct attrlist *attrs_out)
{
	...
	struct ceph_handle *dir = container_of(dir_pub, struct ceph_handle, handle);
				...
				struct attrlist *attrs_out)
{
	...
	struct ceph_export *export =
			container_of(op_ctx->fsal_export, struct ceph_export, export);
	/* The private 'full' directory handle */
	struct ceph_handle *dir = container_of(dir_hdl, struct ceph_handle, handle);
	/* Stat result */
	struct ceph_statx stx;
	mode_t unix_mode;
...
fsal_status_t ceph_close_my_fd(struct ceph_handle *handle, struct ceph_fd *my_fd)
{
	int rc = 0;
				    ...
				    struct fsal_fd *fd)
{
	...
	return ceph_open_my_fd(container_of(obj_hdl, struct ceph_handle, handle),
...
static fsal_status_t ceph_close_func(struct fsal_obj_handle *obj_hdl,
				     struct fsal_fd *fd)
{
	return ceph_close_my_fd(container_of(obj_hdl, struct ceph_handle, handle),
				(struct ceph_fd *)fd);
}
			 ...
			 bool *caller_perm_check)
{
 ...
 fileerr:
	...
	(void) ceph_close_my_fd(container_of(*new_obj, struct ceph_handle, handle),
				my_fd);
...
static void ceph_deleg_cb(Fh *fh, void *vhdl)
{
	...
	struct ceph_handle *hdl = container_of(obj_hdl, struct ceph_handle, handle);
...
{
	struct ceph_handle *myself = container_of(obj_hdl, struct ceph_handle, handle);
	fsal_status_t status = {0, 0};
	int retval = 0;
	Fh *my_fd = NULL;
	unsigned int cmd;
...
{
	/* The private 'full' object handle */
	struct ceph_handle *handle = container_of(handle_pub, struct ceph_handle, handle);
