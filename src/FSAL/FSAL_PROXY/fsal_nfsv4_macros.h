...
#define COMPOUNDV4_ARG_ADD_OP_COMPOUND_SETATTR(opcnt, argarray, inattr) \
do { \
	nfs_argop4 *op = argarray + opcnt; opcnt++;			\
	op->argop = NFS4_OP_SETATTR;					\
	op->nfs_argop4_u.opsetattr.stateid.seqid = 1;			\
	memset(op->nfs_argop4_u.opsetattr.stateid.other, 0,		\
...
do { \
	nfs_argop4 *op = argarray+opcnt; opcnt++;			\
	op->argop = NFS4_OP_READ;					\
	memset(&op->nfs_argop4_u.opread.stateid, 1, sizeof(stateid4));	\
	op->nfs_argop4_u.opread.offset = inoffset;			\
	op->nfs_argop4_u.opread.count  = incount;			\
} while (0)
...
do { \
	...
	op->nfs_argop4_u.opwrite.stable = __stable;			\
