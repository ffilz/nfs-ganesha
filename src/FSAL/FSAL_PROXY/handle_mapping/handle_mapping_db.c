			       ...
			       unsigned int nb_dbop_prealloc)
{
	...
	PTHREAD_MUTEX_init(&p_thr_info->work_queue.queues_mutex);
	PTHREAD_COND_init(&p_thr_info->work_queue.work_avail_condition);
	PTHREAD_COND_init(&p_thr_info->work_queue.work_done_condition);
	...
	PTHREAD_MUTEX_init(&p_thr_info->pool_mutex);
