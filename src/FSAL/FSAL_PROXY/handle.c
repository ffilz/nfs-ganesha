			       ...
			       bool *caller_perm_check)
{
	...
	fattr4 inattrs;
	open_claim4 claim;
	struct attrlist attrs_in_setattr;
	fattr4 inattrs_setattr;
	/* PUTFH, OPEN, (SETATTR), GETFH, GETATTR, CLOSE */
	#define FSAL_OPEN_NB_OP 6
	...
	*caller_perm_check = false;
	...
	/* add SETATTR if we need to truncate */
	if (openflags & FSAL_O_TRUNC) {
		/* prepare attrs to truncate */
		memset(&attrs_in_setattr, 0, sizeof(attrs_in_setattr));
		attrs_in_setattr.valid_mask |= ATTR_SIZE;
		attrs_in_setattr.filesize = 0;
		if (pxy_fsalattr_to_fattr4(&attrs_in_setattr,
					   &inattrs_setattr) == -1)
			return fsalstat(ERR_FSAL_INVAL, -1);
		fsal_release_attrs(&attrs_in_setattr);
	...
	nfs4_Fattr_Free(&inattrs);
	if (openflags & FSAL_O_TRUNC)
		nfs4_Fattr_Free(&inattrs_setattr);
	if (rc != NFS4_OK)
		return nfsstat4_to_fsal(rc);
				  ...
				  struct attrlist *attrib_set)
{
	...
	uint32_t opcnt = 0;
	struct pxy_obj_handle *ph;
#define FSAL_SETATTR2_NB_OP_ALLOC 2 /* PUTFH, SETATTR */
	...
	* RFC5651, sections 8.2.3 and 18.30.3, says that special bypass stateid
	* is only used for READ, and not for write or setattr.
	*/
	COMPOUNDV4_ARG_ADD_OP_SETATTR(opcnt, argoparray, input_attr);
...
fsal_openflags_t pxy_status2(struct fsal_obj_handle *obj_hdl,
			     struct state_t *state)
{
