...
#define FSAL_VERIFIER_T_TO_VERIFIER4(verif4, fsal_verif)		\
	memcpy(verif4, fsal_verif,					\
	       sizeof(fsal_verifier_t) < sizeof(verifier4) ?		\
			sizeof(fsal_verifier_t) : sizeof(verifier4))
			       ...
			       bool *caller_perm_check)
{
	...
	* support_ex, we need to do open on server in three cases :
	*/
	if (name ||                         /* 1) open by name to get handle */
	    createmode != FSAL_NO_CREATE || /* 2) create to create new file */
	    openflags & FSAL_O_TRUNC) {     /* 3) truncate to cut file */
		...
		fattr4 inattrs;
		open_claim4 claim;
		#define FSAL_OPEN_NB_OP 4 /* GETFH, OPEN, GETFH, GETATTR */
		...
		*caller_perm_check = false;
		...
		owner_len = strnlen(owner_val, sizeof(owner_val));
		/* inattrs and openhow */
		st = fill_openhow_OPEN4args(&openhow, &inattrs, attrs_in,
					    createmode, verifier);
		if (FSAL_IS_ERROR(st))
			return st;
		...
		/* GETATTR */
		atok =
		&resoparray[opcnt].nfs_resop4_u.opgetattr.GETATTR4res_u.resok4;
		...
		if (name == NULL && openflags & FSAL_O_TRUNC) {
		/*We simply truncated an old existing entry :
		* new_obj is already the existing one, we simply update attrs.
		*/
			/* update attrs_out */
			if (attrs_out) {
				compound_data_t data; /* for fattr_to_fsal */

				memset(&data, 0, sizeof(data));
				memset(&attrs_out, 0, sizeof(attrs_out));

				/* we prepare fattr_to_fsal call */
				data.current_obj = obj_hdl;
		...
		} else { /* we opened a new entry */
			struct pxy_obj_handle *new_ph;
			...
			new_ph = container_of(*new_obj, struct pxy_obj_handle,
					      obj);

			/* set openflags */
			new_ph->openflags = openflags;
		}
	} else {
	/*
	* This first version of support_ex doesn't manage state.
	* We only check and store openflags.
	*/
		/* no open call, no perm check */
					   ...
					   fsal_openflags_t openflags)
{
	...
	/* share access */
	if (openflags & FSAL_O_READ && openflags & FSAL_O_WRITE)
	...
	else if (openflags & FSAL_O_DENY_READ)
		*share_deny = OPEN4_SHARE_DENY_READ;
	else if (openflags & FSAL_O_DENY_WRITE ||
		 openflags & FSAL_O_DENY_WRITE_MAND)
