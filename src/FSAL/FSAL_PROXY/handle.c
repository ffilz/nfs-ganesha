			       ...
			       bool *caller_perm_check)
{
	...
	fattr4 inattrs;
	open_claim4 claim;
	struct attrlist attrs_in_truncattr;
	fattr4 inattrs_truncattr;
	/*
	* PUTFH, OPEN,
	* (SETATTR for truncate), (SETATTR to setattr),
	* GETFH, GETATTR, CLOSE
	*/
	#define FSAL_OPEN_NB_OP 7
	nfs_argop4 argoparray[FSAL_OPEN_NB_OP];
	nfs_resop4 resoparray[FSAL_OPEN_NB_OP];
	/* PUTFH, (SETATTR for truncate), (SETATTR to setattr), GETATTR */
	#define FSAL_SETATTR_AFTER_OPEN_NB_OP 4
	nfs_resop4 resoparray2[FSAL_SETATTR_AFTER_OPEN_NB_OP];
	OPEN4resok *opok;
	GETFH4resok *fhok;
	GETATTR4resok *atok;
	char fattr_blob[FATTR_BLOB_SZ];
	...
	/* open by handle */
	if (!name) {
		/* we don't do any open waiting v4.1 version of FSAL_PROXY */
	...
	/* open by name */
		...
		*caller_perm_check = false;
		...
		/* add SETATTR to truncate if no EXCLUSIVE4 create */
		if (openflags & FSAL_O_TRUNC &&
		    (openhow.opentype != OPEN4_CREATE ||
		     openhow.openflag4_u.how.mode != EXCLUSIVE4)) {
			memset(&attrs_in_truncattr, 0,
			       sizeof(attrs_in_truncattr));
			attrs_in_truncattr.valid_mask |= ATTR_SIZE;
			attrs_in_truncattr.filesize = 0;
			if (pxy_fsalattr_to_fattr4(&attrs_in_truncattr,
						   &inattrs_truncattr) == -1) {
				nfs4_Fattr_Free(&inattrs);
				return fsalstat(ERR_FSAL_INVAL, -1);
			}
			fsal_release_attrs(&attrs_in_truncattr);

			/* add setattr to compound call */
			resoparray[opcnt].nfs_resop4_u.opsetattr.attrsset =
								empty_bitmap;
			COMPOUNDV4_ARG_ADD_OP_COMPOUND_SETATTR(opcnt,
							       argoparray,
							inattrs_truncattr);
		}

		/* add setattr if needed and if no EXCLUSIVE4 create */
		if (attrs_in->valid_mask &&
		    (openhow.opentype != OPEN4_CREATE ||
		     openhow.openflag4_u.how.mode != EXCLUSIVE4)) {
			/* add setattr to compound call */
			resoparray[opcnt].nfs_resop4_u.opsetattr.attrsset =
		...
		if (openflags & FSAL_O_TRUNC &&
		    (openhow.opentype != OPEN4_CREATE ||
		     openhow.openflag4_u.how.mode != EXCLUSIVE4))
			nfs4_Fattr_Free(&inattrs_truncattr);
		if (rc != NFS4_OK) {
			nfs4_Fattr_Free(&inattrs);
			return nfsstat4_to_fsal(rc);
		}
	...
	/*
	* We could need a GETATTR if name == NULL but attrsout is asked.
	*
	* We could need a SETATTR for truncate or to setattr.
	*
	* This SETATTR is not already included in the previous OPEN compound
	* in two cases :
	*
	* 1) open by handle (no previous OPEN compound request)
	* 2) EXCLUSIVE4 create excludes compound setattr.
	* (Waiting v4.1 FSAL_PROXY to delete this additionnal setattr.)
	*/
	if ((name == NULL && attrs_out) ||
	    ((name == NULL ||
	     (createmode == FSAL_EXCLUSIVE || createmode == FSAL_EXCLUSIVE_41))
	      && (attrs_in->valid_mask || openflags & FSAL_O_TRUNC))) {
		nfs_fh4 currentFH;

		if (name)
			currentFH = fhok->object; /* from previous OPEN */
		else
			currentFH = ph->fh4; /* open by handle */

		/* prepare open call */
		/* we reuse input previous structures */
		opcnt = 0;
		/* PUTFH */
		COMPOUNDV4_ARG_ADD_OP_PUTFH(opcnt, argoparray, currentFH);
		...
		if (openflags & FSAL_O_TRUNC) {
			...
			/* add setattr to compound call */
			resoparray2[opcnt].nfs_resop4_u.opsetattr.attrsset =
								empty_bitmap;
		...
		/* GETATTR */
		/* new attr res, even if we already have one */
		atok = pxy_fill_getattr_reply(resoparray2 + opcnt, fattr_blob,
					      sizeof(fattr_blob));
		COMPOUNDV4_ARG_ADD_OP_GETATTR(opcnt, argoparray,
					      pxy_bitmap_getattr);
...
#define FSAL_READ2_NB_OP_ALLOC 2 /* PUTFH + READ */
	...
	rok->data.data_val = buffer;
	rok->data.data_len = buffer_size;
	if (state->state_type != STATE_TYPE_SHARE && bypass)
