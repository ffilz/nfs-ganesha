...
static void export_release(struct fsal_export *exp_hdl)
{
	...
	gsh_free(glfs_export->mount_path, MEM_COMP_MISC);
	gsh_free(glfs_export->export_path, MEM_COMP_MISC);
	gsh_free(glfs_export->sec_label_xattr, MEM_COMP_MISC);
	gsh_free(glfs_export, MEM_COMP_FSAL);
}
				 ...
				 struct fsal_attrlist *attrs_out)
{
	...
	*pub_handle = &objhandle->handle;
	...
	gsh_free(realpath, MEM_COMP_MISC);
...
skip_upcall:
	...
	glfs_fini(gl_fs->fs);
	gsh_free(gl_fs->volname, MEM_COMP_MISC);
				      ...
				      const struct fsal_up_vector *up_ops)
{
	...
	gl_fs->fs = fs;
	gl_fs->volname = gsh_strdup(params.glvolname);
				      ...
				      const struct fsal_up_vector *up_ops)
{
	...
	glfsexport =
		gsh_calloc(1, sizeof(struct glusterfs_export), MEM_COMP_FSAL);
...
out:
	gsh_free(params.glvolname, MEM_COMP_MISC);
	gsh_free(params.glhostname, MEM_COMP_MISC);
	gsh_free(params.glfs_log, MEM_COMP_MISC);

	if (status.major != ERR_FSAL_NO_ERROR) {
		gsh_free(params.glvolpath, MEM_COMP_MISC);
		...
		gsh_free(glfsexport, MEM_COMP_FSAL);
	}
