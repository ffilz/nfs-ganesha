...
glusterfs_free_fs(struct glusterfs_fs *gl_fs)
{
	...
	refcnt = atomic_dec_int64_t(&gl_fs->refcnt);
	...
	atomic_add_int8_t (&gl_fs->destroy_mode, 1);
		 ...
		 const struct fsal_up_vector *up_ops)
{
	...
	glist_for_each_safe(glist, glistn, &GlusterFS.fs_obj) {
		gl_fs = glist_entry(&GlusterFS.fs_obj, struct glusterfs_fs,
				    fs_obj);
...
found:
	atomic_inc_int64_t(&gl_fs->refcnt);
	PTHREAD_MUTEX_unlock(&GlusterFS.lock);
	return gl_fs;
