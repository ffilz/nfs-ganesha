				...
				struct attrlist *fsalattr)
{
	...
	if (NFSv4_ACL_SUPPORT && FSAL_TEST_MASK(fsalattr->mask, ATTR_ACL)) {
		...
		status = posix_acl_2_fsal_acl(buffxstat->acl, &fsalattr->acl);
	}
				    ...
				    glusterfs_fsal_xstat_t *buffxstat)
{
	...
	if (attrs->acl) {
		...
		buffxstat->acl = glfs_h_acl_get(fs, object, ACL_TYPE_ACCESS);
		if (!buffxstat->acl) {
			LogCrit(COMPONENT_FSAL, "Previous ACL was NULL");
			return fsalstat(ERR_FSAL_FAULT, 0);
		}
		status = fsal_acl_2_posix_acl(attrs->acl, &buffxstat->acl);
