			     ...
			     struct glfs_object *object)
{
	...
	if (rc && rc != CACHE_INODE_NOT_FOUND) {
		LogWarn(COMPONENT_FSAL_UP,
			"Inode_Invalidate event could not be processed"
			" for fd %p, rc %d",
			glfsexport->gl_fs, rc);
	}
	...
	while (!glfsexport->destroy_mode) {
		...
		if (rc != 0) {
			...
			if (retry < 10) {
				...
				LogFatal(COMPONENT_FSAL_UP,
					 "Retry limit for poll_upcall exceeded"
					 " for (%p).",
					 glfsexport->gl_fs);
				return NULL;
			}
		...
		case GFAPI_INODE_INVALIDATE:
		{
			...
			if (cbk_inode_arg)
				LogWarn(COMPONENT_FSAL_UP, "Received NULL upcall"
							   "event arg");
