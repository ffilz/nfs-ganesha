...
/* *
 * Calculates hash based on the data given and its value ranges from
 * 0 to len - 1. for len = 1, hash will be '0'.
 *
 * In case of error, returns value of len.
 */
	  ...
	  size_t size)
{
	...
	ret = superfasthash(key, no_of_ds);
	if (ret == no_of_ds)
		goto out;
