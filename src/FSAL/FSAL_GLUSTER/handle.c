...
again:
	...
	if (not_open_correct(my_fd, openflags)) {
		...
		if (rc == EBUSY) {
			...
			if (FSAL_IS_ERROR(status)) {
				LogDebug(COMPONENT_FSAL,
					 "glusterfs_open failed with %s openflags 0x%08x",
 ...
 regular_open:
	...
	if (open_for_locks) {
		...
		if (state->state_data.lock.openstate != NULL) {
			struct glusterfs_fd *related_fd =
					(struct glusterfs_fd *)
...
/* case name_not_null */
	...
	/* XXX: we do not have openat implemented yet..meanwhile
	 *  use 'glfs_h_creat' */
			     ...
			     size_t len)
{
	 ...
	 */
	status = glusterfs_reopen_obj(obj_hdl, false, false, FSAL_O_WRITE,
				&my_fd, &has_lock, &closefd);
...
fsal_status_t glusterfs_getattr2(struct fsal_obj_handle *obj_hdl)
{
	...
	if (obj_hdl->fsal != obj_hdl->fs->fsal) {
		LogDebug(COMPONENT_FSAL,
			 "FSAL %s getattr for handle belonging to FSAL %s,
			 ignoring",
			 obj_hdl->fsal->name,
