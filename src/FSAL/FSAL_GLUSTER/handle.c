				  ...
				  bool *eof)
{
	...
	SET_GLUSTER_CREDS(glfs_export, &op_ctx->creds->caller_uid,
			  ...
			  socket_addr(&op_ctx->client->addrbuf),
			  socket_addr_len(&op_ctx->client->addrbuf));
				   ...
				   struct glusterfs_fd *my_fd)
{
	...
	if (socket_addr_len(&op_ctx->client->addrbuf <= GLAPI_LEASE_ID_SIZE)) {
		memcpy(my_fd->lease_id, socket_addr(&op_ctx->client->addrbuf),
		       socket_addr_len(&op_ctx->client->addrbuf));
	} else
