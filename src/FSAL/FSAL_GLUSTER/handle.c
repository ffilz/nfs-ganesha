				   ...
				   struct glusterfs_fd *my_fd)
{
	...
	(*garray_copy) = gsh_malloc(op_ctx->creds->caller_glen * sizeof(gid_t));
...
void glusterfs_copy_my_fd(struct glusterfs_fd *dst, struct glusterfs_fd *src)
{
	dst->glfd = src->glfd;
	dst->openflags = src->openflags;
	dst->creds.caller_uid = src->creds.caller_uid;
	dst->creds.caller_gid = src->creds.caller_gid;
	dst->creds.caller_glen = src->creds.caller_glen;
	dst->creds.caller_garray = src->creds.caller_garray;
}
		      ...
		      bool open_for_locks)
{
	...
	glusterfs_copy_my_fd(my_fd, tmp2_fd);
	return status;
}
				     ...
				     bool *caller_perm_check)
{
	...
	if (obj_hdl->fsal != obj_hdl->fs->fsal) {
		...
		glusterfs_copy_my_fd(my_fd, &tmp_fd);
