 ...
 *
 * @returns no of entires on success and -1 on failure
 */
...
posix_acl_2_fsal_acl(acl_t p_posixacl, bool is_dir, bool is_inherit,
			fsal_ace_t **ace)
{
	int ret = 0, ent, d_ent, total = 0, i = 0, j = 0;
	...
	for (ent = ACL_FIRST_ENTRY; ; ent = ACL_NEXT_ENTRY) {
		 ...
		 * If the tag is ACL_USER_OBJ then all other posix acl entries
		 * should be considered
		 * If the tag is ACL_GROUP_OBJ then ACL_GROUP or ACL_OTHER
		 * should be considered.
		 * If the tag is either ACL_USER or ACL_GROUP then consider
		 * only ACL_OTHER.
		 */
		if (tag == ACL_USER_OBJ || tag == ACL_GROUP_OBJ) {
			...
			/*
			 * Do not consider ACL_MASK entry in the
			 * following loop
			 */
		...
		pace += 2;
		total += 2; /* Calculating total no of ace entries */
	}

	/* Removing DENY entries which are not required */
	pace = *ace;
	for (i = 0; i < total; i++) {
		if ((pace + i)->perm != 0) {
			*(pace + j) = *(pace + i);
	...
	*ace = (pace + j); /* Returning last pointer in fsal acl list */
	return j; /* Returning no of entries in the list */
}
...
fsal_acl_2_posix_acl(fsal_acl_t *p_fsalacl, acl_type_t type)
{
	 ...
	 * such a way that it will conatins all permissions(READ,WRITE,EXECUTE)
