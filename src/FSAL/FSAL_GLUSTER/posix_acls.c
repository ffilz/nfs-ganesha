...
fsal_acl_2_posix_acl(fsal_acl_t *p_fsalacl)
{
	...
	acl_entry_t p_entry;
	acl_permset_t p_permset;
	uid_t uid[10], tmp_uid;
	gid_t gid[10], tmp_gid;
	bool user = false;
	bool group = false;
	 ...
	 * TODO: Use a seperate api for performing this function
	 ...
	 * a specfic order such that users, groups, other.So we will fetch
	...
	if (deny_ace) {
		if (IS_FSAL_ACE_READ_DATA(*deny_ace))
			acl_delete_perm(p_permset, ACL_READ);
		if (IS_FSAL_ACE_WRITE_DATA(*deny_ace))
			acl_delete_perm(p_permset, ACL_WRITE);
		if (IS_FSAL_ACE_EXECUTE(*deny_ace))
