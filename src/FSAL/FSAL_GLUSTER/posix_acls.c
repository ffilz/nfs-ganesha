...
/*
 * Check whether given perm(ACL_READ, ACL_WRITE or ACL_EXECUTE) is
 * allowed for a permset, it depends on given ace and permset of
 * @EVERYONE entry.
 */
bool
...
/*
 * Check whether given perm(ACL_READ, ACL_WRITE or ACL_EXECUTE) is
 * denied for a permset, it depends on permsets of deny entry
 * of the acl and @EVERYONE entry.
 */
bool
...
/*
 * It traverse entire list of entries for a posix acl and
 * finds ACL entry which corresponds to a given tag and id
 *
...
/*
 * It tries to find out whether an entry is present in posix acl for
 * the given (tag, id) tuple and returns it. If not , it will create
 * a new entry for given (tag, id).
 *
 * On success , it returns acl entry otherwise it returns NULL
 */
acl_entry_t
			...
			fsal_ace_t **ace)
{
	...
	/*
	 * Converts each entry in posix acl into fsal_ace by filling
	 * type, flag, perm , iflag , flag and who(uid, gid) appropiately
	 */
	for (ent = ACL_FIRST_ENTRY; ; ent = ACL_NEXT_ENTRY) {
...
fsal_acl_2_posix_acl(fsal_acl_t *p_fsalacl, acl_type_t type)
{
	 ...
	 * there is a possiablity of memory leak
	 */
	...
	/*
	 * Deny entry for @EVERYONE created only beacuse it will ease the
	 * manipulation of other acl entries. It will be updated only when
	 * deny entry for @EVERYONE is encountered
	 */
	...
	/*
	 * It is mandatory to have acl entries for ACL_USER_OBJ
	 * and ACL_GROUP_OBJ
	 */
	ret = acl_create_entry(&allow_acl, &a_entry);
	if (ret) {
	...
	/*
	 * It uses two posix acl - allow_acl and deny_acl which represents
	 * ALLOW and DENY aces repectively. They are filled according to
	 * the order in fsal_acl list. The allow acl is build based on
	 * ALLOW ace, @EVERYONE ace and deny acl. The permset for allow acl
	 * entry is constructed in such a way that it will conatins all
	 * permissions (READ, WRITE, EXECUTE) of ALLOW aces plus EVERYONE
	 * which is not denied by the corresponding deny acl entry
	 *
