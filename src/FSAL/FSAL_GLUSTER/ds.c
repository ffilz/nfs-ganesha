			...
			bool * const end_of_file)
{
	...
	if (rc < 0) {
		LogMajor(COMPONENT_PNFS, "Read failed on DS: %m");
			 ...
			 stable_how4 * const stability_got)
{
	...
	if (rc < 0) {
		LogMajor(COMPONENT_PNFS, "status after write: %m");
		return posix2nfs4_error(errno);
	}
			  ...
			  verifier4 * const writeverf)
{
	...
	if (ds->stability_got == FILE_SYNC4) {
		...
		rc = glfs_fsync(glfd);
		if (rc != 0)
			LogMajor(COMPONENT_PNFS, "ds_commit() failed: %m");
		rc = glfs_close(glfd);
		if (rc != 0)
			LogDebug(COMPONENT_PNFS, "status after close: %m");
