...
static fsal_status_t fsal_internal_error(int error, const char *msg)
{
	LogFullDebug(COMPONENT_FSAL, "%s returned errno=%d", msg, error);

	if (error == EUNATCH)
		LogFatal(COMPONENT_FSAL, "GPFS Returned EUNATCH");

	return fsalstat(posix2fsal_error(error), error);
}
