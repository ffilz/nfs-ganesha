				...
				xattrlist4 *lr_names)
{
	int rc;
	int errsv;
	char *nameP, *nextP, *endP, *valP, *valPstart;
	int entryCount = 0;
	char *bufP = NULL;
	...
	while (nameP < endP) {
		...
		LogDebug(COMPONENT_FSAL,
		"nameP %s at offset %ld", nameP, (nextP - nameP));

		if ((((char *)entry - (char *)lr_names->entries) +
		     sizeof(component4) > la_maxcount) ||
		     ((valP - valPstart) + (nextP - nameP) > la_maxcount)) {
			gsh_free(bufP);
			*lr_eof = false;
			...
			*la_cookie += entryCount;
			...
			if (entryCount == 0)
				return fsalstat(ERR_FSAL_TOOSMALL, 0);
			 lr_names->entryCount = entryCount;
			return fsalstat(ERR_FSAL_NO_ERROR, 0);
		}
		if ((entryCount > *la_cookie) || (*la_cookie == 0)) {
			entry->utf8string_len = nextP - nameP;
			entry->utf8string_val = valP;
			memcpy(entry->utf8string_val, nameP, entry->utf8string_len);

			LogFullDebug(COMPONENT_FSAL,
				"entry %d valP %p at %p len %d at %p name %s",
				entryCount, valP, entry, entry->utf8string_len,
				entry->utf8string_val, entry->utf8string_val);

			valP += entry->utf8string_len;
			entry += 1;
		}
