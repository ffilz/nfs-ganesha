		...
		void *caller_arg)
{
	...
	if (read_arg->info)
		status = gpfs_read_plus_fd(
			my_fd->fd, read_arg->offset, read_arg->iov[0].iov_len,
			read_arg->iov[0].iov_base, &read_arg->io_amount,
			&read_arg->end_of_file, read_arg->info, export_fd);
	else
		 ...
		 void *caller_arg)
{
	...
	status = GPFSFSAL_write(my_fd->fd, write_arg->offset,
				...
				&write_arg->io_amount, &write_arg->fsal_stable,
				export_fd);
