 ...
 * @param[out] confl_lock   Conflicting lock
 *
 * @return FSAL status.
 */
static fsal_status_t
_gpfs_lock_op(struct fsal_obj_handle *obj_hdl, struct state_t *state,
	      void *owner, fsal_lock_op_t lock_op, fsal_lock_param_t *req_lock,
	      fsal_lock_param_t *confl_lock, bool version2)
{
	struct fsal_export *export = op_ctx->fsal_export;
	...
	if (confl_lock == NULL && lock_op == FSAL_OP_LOCKT) {
		LogDebug(COMPONENT_FSAL,
	...
	if (version2) {
		...
		struct gpfs_fd *gfd =
			&((struct gpfs_fsal_obj_handle *) container_of(obj_hdl,
					     struct gpfs_fsal_obj_handle,
	...
	status = GPFSFSAL_lock_op(export, lock_op, req_lock, confl_lock,
				  &gpfs_sg_arg);
...
{
	return _gpfs_lock_op(obj_hdl, (void *) ~0, owner, lock_op, req_lock,
			     confl_lock, false);
...
gpfs_lock_op2(struct fsal_obj_handle *obj_hdl, struct state_t *state,
	      ...
	      fsal_lock_param_t *confl_lock)
{
	return _gpfs_lock_op(obj_hdl, state, owner, lock_op, req_lock,
			     confl_lock, true);
}
