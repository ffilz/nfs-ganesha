...
void *GPFSFSAL_UP_Thread(void *Arg)
{
	...
	/* Set up op_ctx for this thread. @todo: we have one upcall
	 * thread for each file system image. This is likely the first
	 * export created on this file system image. Do we have a
	 * refcount on this export to avoid getting freed on its
	 * removal?
	 *
	 * Only export_id is used currently from upcall code path, so
	 * hand construct gsh_export with export_id here.
	 */
	gexport.export_id = event_func->up_export->export_id;
	req_ctx.fsal_export = event_func->up_export;
	req_ctx.ctx_export = &gexport;
	op_ctx = &req_ctx;
