...
proxyv3_is_valid_lockop(struct fsal_obj_handle *obj_hdl,
   ...
   if (owner == NULL) {
      // We need the owner info to fill in the various alock fields in the
      // requests.
      LogCrit(COMPONENT_FSAL,
   ...
   if (lock_op == FSAL_OP_LOCKT && conflicting_lock == NULL) {
      LogCrit(COMPONENT_FSAL,
              "ERROR: Ganesha asked for NLM4_TEST, but output param is NULL");
      return false;
   }

   if (proxyv3_nlm_port() == 0) {
      LogCrit(COMPONENT_FSAL,
              "Got a lock op request, but we don't know where lockmanagerd is!");
      return false;
   }
...
proxyv3_nlm_fill_common_args(struct proxyv3_obj_handle *obj,
   ...
   // state->state_owner as well as a random void* are passed to lock_op2. Maybe
   // we should check that state_owner == state->state_owner?
   struct state_nlm_owner_t *nlm_owner = &state_owner->so_owner.so_nlm_owner;

   // Fill in the cookie.
   //
   // NFS Illustrated claims that the client (that's us!) get to fill this in
   // with whatever we want (I think it's an extra double check on top of the
   // XID in the RPC). My first plan was to use obj->fh3, but those are often
   // >32 bytes which Linux's NFSD doesn't like at the least:
   //
   //   lockd: bad cookie size 36 (only cookies under 32 bytes are supported.)
   //
...
proxyv3_nlm_test(struct proxyv3_obj_handle *obj,
   ...
   nlm4_stats *status = &result.test_stat.stat;
   fsal_status_t rc =
   ...
   // If we didn't get back an explicit DENIED response, just return the result.
   if (*status != NLM4_DENIED) {
      return rc;
   }

   // Otherwise, we need to fill in the conflict info.
   nlm4_holder *holder = &result.test_stat.nlm4_testrply_u.holder;

   // TODO(boulos): The holder also has the other owner information, but it's
   // not clear if you're supposed to fill in state_owner with that info...
   conflicting_lock->lock_type = (holder->exclusive) ? FSAL_LOCK_W : FSAL_LOCK_R;
...
proxyv3_nlm_lock(struct proxyv3_obj_handle *obj,
   ...
   return proxyv3_nlm_commonrpc(NLMPROC4_LOCK, "NLM_LOCK",
                                ...
                                &result.stat.stat, &args.cookie, &args.alock);
}
...
proxyv3_clear_conflicting_lock(fsal_lock_op_t lock_op,
   ...
   if (lock_op != FSAL_OP_LOCKT) {
      // TODO(boulos): Alternatively, we can do a TEST afterwards to fill in
      // who the conflict was likely to be. But that can also fail if the
      // conflict gives up in between our LOCK. The CEPH FSAL chooses to do
      // this though, and it probably makes Ganesha more able to handle
      // immediate responses for lock requests (i.e., if it knows that only a
      // certain range is locked, it might allow in a read lock to a
      // non-overlapping range). But the SAL do_lock_op always just fills in
      // *holder with &unknown_holder anyway... so it doesn't seem like we
      // should waste our time.
      LogDebug(COMPONENT_FSAL,
               "Lock op is %s, but Ganesha wants to know about the conflict."
               "  Report the whole file as locked like nlm_process_conflict.",
...
proxyv3_lock_op2(struct fsal_obj_handle *obj_hdl,
   ...
   // Before we fail or not, clear the output conflicting_lock if appropriate.
   // NOTE(boulos): Ganesha seems to (incorrectly?) fill in the response for
   // non-TEST calls with the conflict holder (e.g., in nlm4_Lock) even though
   // these RPCs are all supposed to return only a nlm4_res which has no holder
   // information.
   ...
   case FSAL_OP_LOCK:
      return proxyv3_nlm_lock(obj, state, owner, exclusive, request_lock);
   case FSAL_OP_UNLOCK:
      return proxyv3_nlm_unlock(obj, state, owner, exclusive, request_lock);
   case FSAL_OP_CANCEL:
      return proxyv3_nlm_cancel(obj, state, owner, exclusive, request_lock);
   default:
