				  ...
				  struct attrlist *attrs_out)
{
	...
	if (strncmp(full_path, root_path, root_len) != 0) {
		LogDebug(COMPONENT_FSAL,
			 "path '%s' doesn't begin with our root '%s'. mount should have handled that for us!",
			 full_path, root_path);
		return fsalstat(ERR_FSAL_INVAL, 0);
	}
	...
	if (*p != '/') {
		LogDebug(COMPONENT_FSAL,
			 "Non-root lookup, but first character (%c) of %s isn't a forward slash (mount should have handled this)",
			 *p, p);
		return fsalstat(ERR_FSAL_INVAL, 0);
	}
	...
	/*
	 * Now we've got a path with potentially multiple subdirectories
	 * (separated by forward slashes). Split the path on '/' as we go,
