...
fsal_status_t fsal_rename_access(struct fsal_obj_handle *obj_hdl,
				 struct fsal_obj_handle *old_dir_hdl,
				 ...
				 bool isdir)
{
	...
	status = fsal_remove_access(old_dir_hdl, obj_hdl, isdir);
	...
	status = fsal_test_access(new_dir_hdl, access_type, NULL, NULL);
	if (FSAL_IS_ERROR(status))
		return status;
