...
void fd_lru_run(struct fridgethr_context *ctx)
{
	...
	if ((current_total_fds >= fd_lru_state.fds_lowat) ||
		...
		if (extremis) {
			...
			LogDebug(COMPONENT_FSAL,
				 "Total FDs exceed high water mark (%" PRIi32
				 "). Breakdown: global=%" PRIi32
			...
			if (state_fds >= fd_lru_state.fds_hiwat) {
				LogWarn(COMPONENT_FSAL,
					"FD pressure is from state FDs (%" PRIi32
					") which meet/exceed hiwat (%" PRIi32
					"). LRU can only reclaim global FDs, not "
					"state FDs. Skipping aggressive reaping to "
					"avoid futile CPU-intensive work.",
		 ...
		 *
		 * If progress is insufficient, increment futility counter.
		 * When futility reaches futility_count, FD caching is temporarily
