				    ...
				    uint64_t *copied)
{
	...
	const uint64_t kDefaultChunk = 4 * 1024 * 1024ULL; /* 4MiB */
	uint64_t max_chunk = kDefaultChunk;
	uint64_t bytes_copied = 0;
	char *buffer;

	if (MaxRead  > 0)
		max_chunk = MIN(max_chunk, MaxRead);
	if (MaxWrite > 0)
		max_chunk = MIN(max_chunk, MaxWrite);
	if (max_chunk == 0)
		max_chunk = kDefaultChunk;
	...
	if (buffer == NULL) {
		LogCrit(COMPONENT_NFS_V4,
			"COPY gsh_malloc(%"PRIu64") FAILED", max_chunk);
