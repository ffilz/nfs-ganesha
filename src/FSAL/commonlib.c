			     ...
			     bool force)
{
	...
	retval = populate_posix_file_systems(force);
	if (retval != 0) {
		LogCrit(COMPONENT_FSAL,
	 ...
	 */
	if (retval == ENOENT && init_complete && !force) {
		LogDebug(COMPONENT_FSAL,
			 "Call resolve_posix_filesystem one more time");
		retval = resolve_posix_filesystem(path,
						  fsal, exp,
						  claim, unclaim,
						  root_fs, true);
	}
