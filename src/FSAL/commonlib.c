 ...
 *
 * The following function clear_op_context_export() should be called unless
 * the current contents really will not be used. The exceptions are this
 * is directly called by set_op_context_export_fsal(), set_op_context_pnfs_ds(),
 * release_op_context(), or restore_op_context_export().
 *
 * When called by set_op_context_export_fsal() or set_op_context_pnfs_ds(),
...
void save_op_context_export_and_clear(struct saved_export_context *saved)
{
	...
	/* An active op context will always have refstr but we the original
 ...
 *
 * If the thread's op_ctx is non-NULL, that will be saved in this op_context.
 *
 * The thread's op_ctx will be set to this op_context.
 *
 ...
 * @param[in] caller_data    The caller address data
