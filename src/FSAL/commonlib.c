...
uint32_t fsal_fd_global_counter;
...
void fd_lru_run(struct fridgethr_context *ctx)
{
	...
	currentopen = atomic_fetch_uint32_t(&fsal_fd_global_counter);
	...
	if (currentopen < fd_lru_state.fds_lowat) {
		LogDebug(COMPONENT_FSAL,
			 ...
			 atomic_fetch_uint32_t(&fsal_fd_global_counter),
		...
		/* The count of open file descriptors before this run
		   of the reaper. */
		uint32_t formeropen = currentopen;
	...
	LogDebug(COMPONENT_FSAL,
		 "After work, open_fd_count:%" PRIu32
...
static void remove_fd_lru(struct fsal_fd *fsal_fd)
{
	...
	case FSAL_FD_OLD_STYLE:
		...
		break;
	case FSAL_FD_GLOBAL:
		atomic_dec_int32_t(&fsal_fd_global_counter);
...
fsal_status_t fd_lru_pkginit(struct fd_lru_parameter *params)
{
	...
	atomic_store_uint32_t(&fsal_fd_global_counter, 0);
			    ...
			    bool is_reclaiming)
{
	...
	while (atomic_fetch_int32_t(&fsal_fd->io_work) != 0) {
		if (is_reclaiming) {
			/* We are trying to reclaim an in-use fsal_fd, bumo
			 * it so we skip it.
			 */
			bump_fd_lru(fsal_fd);
	...
	/** Need to decrement the appropriate counter and remove from LRU. */
...
static inline bool cant_reopen(struct fsal_fd *fsal_fd,
			       ...
			       bool may_reopen)
{
	uint32_t open_fds = atomic_fetch_uint32_t(&fsal_fd_global_counter);
