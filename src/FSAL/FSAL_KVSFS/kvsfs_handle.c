						 ...
						 struct fsal_export *exp_hdl)
{
	...
	hdl = gsh_malloc(sizeof(struct kvsfs_fsal_obj_handle) +
			 sizeof(struct kvsfs_file_handle), MEM_COMP_FSAL);
	...
	if ((hdl->obj_handle.type == SYMBOLIC_LINK) && (link_content != NULL)) {
		...
		hdl->u.symlink.link_content = gsh_malloc(len, MEM_COMP_MISC);
				       ...
				       bool refresh)
{
	 ...
	 */
	link_content->addr = gsh_malloc(link_content->len, MEM_COMP_MISC);
	...
	if (retlink) {
		...
		gsh_free(link_content->addr, MEM_COMP_MISC);
