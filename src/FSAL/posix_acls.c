...
/*
 * @brief Calculate the ACL count by xattr size
 *
 * @param[in]  size Length of extented attribute
 *
 * @return ACL count on success, -1 on failure.
 */

int posix_acl_xattr_count(size_t size)
{
...
acl_t xattr_2_posix_acl(const void *value, size_t size)
{
	const acl_ea_header *ea_header = value;

	const acl_ea_entry *ea_entry = (const void *)(ea_header + 1), *end;
	...
	if (count < 0) {
		LogMajor(COMPONENT_FSAL, "Invalid parameter:
			 size = %d", (int)size);
		return NULL;
	}
	...
	acl = acl_init(count);
	if (!acl) {
		LogMajor(COMPONENT_FSAL, "Failed to ACL INIT:
			 count = %d", count);
		return NULL;
	}
...
{
	acl_ea_header *ea_header = buf;
	acl_ea_entry *ea_entry;
	acl_entry_t acl_entry;
	acl_tag_t tag;
	acl_permset_t permset;
