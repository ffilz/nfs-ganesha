 ...
 * If we would use straight forward approach of converting each chunk to stripe
 * entry, we would be limited to file size of 256 GB (4096 * 64MB).
 *
...
static size_t lzfs_fsal_fs_da_addr_size(struct fsal_module *fsal_hdl) {
	// one stripe index + number of addresses + LZFS_EXPECTED_BACKUP_DS_COUNT
	// addresses per chunk each address takes 37 bytes (we use 40 for
	// safety) we add 32 bytes of overhead (includes stripe count and DS
	// count)
