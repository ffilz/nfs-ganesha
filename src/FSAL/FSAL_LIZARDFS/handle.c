...
out:
	...
	if (read_arg->state == NULL) {
		 ...
		 */
		update_share_counters(&lzfs_obj->share,
					...
					struct fsal_attrlist *attrib_set)
{
	...
	if (FSAL_TEST_MASK(attrib_set->valid_mask, ATTR_SIZE)) {
		...
		if (state == NULL) {
			...
			status = check_share_conflict(&lzfs_obj->share,
						      FSAL_O_WRITE, bypass);
			if (FSAL_IS_ERROR(status)) {
				PTHREAD_RWLOCK_unlock(&obj_hdl->obj_lock);
			 ...
			 * counters.
			 */
			update_share_counters(&lzfs_obj->share, FSAL_O_CLOSED,
					      FSAL_O_WRITE);
			has_share = true;
...
out:
	...
	if (has_share) {
		...
		/* Release the share reservation now by updating the counters.
		 */
		update_share_counters(&lzfs_obj->share, FSAL_O_WRITE,
				      FSAL_O_CLOSED);
