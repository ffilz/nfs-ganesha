...
mem_async_complete(struct fridgethr_context *ctx)
{
	...
	if (mem_export->async_type != MEM_FIXED) {
		/* Randomize delay */
		async_delay = random() % mem_export->async_delay;
	} else {
		/* Fixed delay */
		async_delay = mem_export->async_delay;
	}
	       ...
	       void *caller_arg)
{
	...
	if (mem_export->async_type > MEM_RANDOM_OR_INLINE ||
	    ((mem_export->async_type == MEM_RANDOM_OR_INLINE) &&
	    ((random() % 1) == 1))) {
