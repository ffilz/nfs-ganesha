...
static void mem_cleanup(struct mem_fsal_obj_handle *myself)
{
	...
	case REGULAR_FILE:
		...
		break;
	case SYMBOLIC_LINK:
		gsh_free(myself->mh_symlink.link_contents, MEM_COMP_PROTOCOL);
		break;
	case SOCKET_FILE:
	case CHARACTER_FILE:
	case BLOCK_FILE:
				 ...
				 struct fsal_attrlist *parent_post_attrs_out)
{
	...
	hdl->mh_symlink.link_contents =
		gsh_strdup(link_path, MEM_COMP_PROTOCOL);
				  ...
				  bool refresh)
{
	...
	link_content->addr =
		gsh_strdup(myself->mh_symlink.link_contents, MEM_COMP_PROTOCOL);
