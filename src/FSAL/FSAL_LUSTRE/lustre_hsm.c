...
/*lustre api for lustre_hsm calls*/
/*
...
fsal_status_t lustre_hsm_restore(struct fsal_obj_handle *obj_hdl)
{
	...
	rc = lustre_handle_to_path(obj_hdl->fs->path, myself->handle,
				   fsal_path);
	if (rc < 0) {
		LogCrit(COMPONENT_FSAL, "lustre_handle_to_path returned %d",
			rc);
	...
	if (rc) {
		...
		/*One more time test if file is already available*/
		rc = llapi_hsm_state_get(fsal_path, &hus);
		if (rc) {
			LogEvent(COMPONENT_FSAL,
				 "Error retrieving lustre_hsm status of %s: %s",
			...
			LogInfo(COMPONENT_FSAL,
				"Lustre hsm restore not yet ended on %s",
				fsal_path);
