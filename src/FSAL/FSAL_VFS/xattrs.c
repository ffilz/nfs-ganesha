				 ...
				 unsigned int *p_nb_returned, int *end_of_list)
{
	...
	for (index = cookie, out_index = 0;
		...
		if (do_match_type
		    (xattr_list[index].flags, obj_hdl->attrs->type)) {
				   ...
				   struct attrlist *p_attrs)
{
	...
	/* check that this index match the type of entry */
	if (xattr_id < XATTR_COUNT &&
	    !do_match_type(xattr_list[xattr_id].flags, obj_hdl->attrs->type)) {
		return fsalstat(ERR_FSAL_INVAL, 0);
	} else if (xattr_id >= XATTR_COUNT) {
	...
	rc = file_attributes_to_xattr_attrs(obj_hdl->attrs, p_attrs, xattr_id);
	if (rc != 0)
