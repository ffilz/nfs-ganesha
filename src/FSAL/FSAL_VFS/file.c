...
#if 0
	/** @todo find usable fd */
	if (lock_fd != NULL && (lock_fd->openflags & FSAL_O_READ) != 0) {
		/* Use the lock fd */
		my_fd = container_of(lock_fd, struct vfs_fd, fsal_fd);
	} else if (share_fd != NULL &&
		   (share_fd->openflags & FSAL_O_READ) != 0) {
		/* Use the share reservation's fd */
		my_fd = container_of(share_fd, struct vfs_fd, fsal_fd);
	} else {
#endif
	{
		...
		PTHREAD_RWLOCK_rdlock(&obj_hdl->lock);
		has_lock = true;
		/** @todo: needs more work here... must open if not currently
		 *         open for read.
		 */
	}
...
#if 0
	/** @todo find usable fd */
	if (lock_fd != NULL && (lock_fd->openflags & FSAL_O_WRITE) != 0) {
		/* Use the lock fd */
		my_fd = container_of(lock_fd, struct vfs_fd, fsal_fd);
	} else if (share_fd != NULL &&
		   (share_fd->openflags & FSAL_O_WRITE) != 0) {
		/* Use the share reservation's fd */
		my_fd = container_of(share_fd, struct vfs_fd, fsal_fd);
	} else {
#endif
