	       ...
	       void *caller_arg)
{
	...
	if (read_arg->info != NULL) {
		 ...
		 * sparse regions.  One content element is enough; the client can
		 * issue another READ_PLUS for the next extent.
		 */
		...
		if (next_data < 0 || next_data > read_arg->offset) {
			 ...
			 * FSAL knew this was a hole.  A hole result does not own the
			 * buffer.
			 */
