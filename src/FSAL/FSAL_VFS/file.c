			     ...
			     struct vfs_fd *my_fd)
{
	...
	if (fd < 0) {
		...
		my_fd->fd = fd;
		my_fd->fsal_fd.openflags = openflags;
	}
		       ...
		       bool *end_of_file)
{
	...
	assert(my_fd->fd >= 0 && my_fd->fsal_fd.openflags != FSAL_O_CLOSED);
...
fsal_status_t vfs_open_fd(struct fsal_obj_handle *obj_hdl,
			  ...
			  bool *caller_perm_check)
{
	...
	struct vfs_fd *my_fd = container_of(fsal_fd, struct vfs_fd, fsal_fd);
	...
	*new_obj = &hdl->obj_handle;
	...
	status = (*new_obj)->obj_ops.setattrs(*new_obj, attrib_set);
	return status;
...
fsal_status_t vfs_reopen_fd(struct fsal_obj_handle *obj_hdl,
			    ...
			    fsal_openflags_t openflags)
{
	...
	if (!FSAL_IS_ERROR(status)) {
		 ...
		 * one over.
		 */
		my_share_fd = container_of(share_fd, struct vfs_fd, fsal_fd);
 ...
 * @return FSAL status.
 */
fsal_status_t vfs_lock_op_fd(struct fsal_obj_handle *obj_hdl,
			     struct fsal_fd *share_fd,
...
{
	struct vfs_fsal_obj_handle *myself;
	struct vfs_fd *my_fd = container_of(lock_fd, struct vfs_fd, fsal_fd);
	struct flock lock_args;
	int fcntl_comm;
	fsal_status_t status = {0, 0};
	int retval = 0;
 ...
 * @return FSAL status.
 */
fsal_status_t vfs_setattr_fd(struct fsal_obj_handle *obj_hdl,
			     struct fsal_fd *share_fd,
			     struct fsal_fd *lock_fd,
			     struct attrlist *attrib_set)
{
...
fsal_status_t vfs_close_fd(struct fsal_fd *fsal_fd)
{
	struct vfs_fd *my_fd = container_of(fsal_fd, struct vfs_fd, fsal_fd);
