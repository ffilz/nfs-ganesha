			     ...
			     bool *closefd)
{
	...
	*closefd = false;
	...
	if (check_share) {
		/* Note that if caller just has FSAL_O_ANY, there will be no
	...
	while (((my_fd->openflags & openflags) != openflags)
		&& ((openflags & FSAL_O_ANY) == 0)
		&& ((my_fd->openflags & FSAL_O_RDWR) != 0)) {
		...
		if (rc == EBUSY) {
			...
			if (check_share) {
				...
				/* Note that if caller just has FSAL_O_ANY,
				 * there will be no conflict, which is good.
				 */
				status =
		...
		if (check_share) {
			/* Note that if caller just has FSAL_O_ANY, there will
			 * be no conflict, which is good.
			 */
			status = check_share_conflict(&myself->u.file.share,
						      openflags,
						      bypass);
			...
			struct fsal_obj_handle *dupe_hdl)
{
	...
	/* Just release the duplicate even if a share error occurred. */
			...
			bool *caller_perm_check)
{
 ...
 oops:
		...
		if (FSAL_IS_ERROR(status)) {
			/* Oops, we need to release our share reservation
			  ...
			  fsal_openflags_t openflags)
{
	...
	/* Remove the old share so we don't conflict with ourselves. */
	update_share_counters(&myself->u.file.share,
			      old_openflags,
			      FSAL_O_CLOSED);
	...
	if (FSAL_IS_ERROR(status)) {
		/* Restore the original share since there was an error. */
		update_share_counters(&myself->u.file.share,
				      FSAL_O_CLOSED,
				      old_openflags);
	 ...
	 * conflicting share be asserted.
	 */
	update_share_counters(&myself->u.file.share, FSAL_O_CLOSED, openflags);
		      ...
		      bool *closefd)
{
	...
	case SYMBOLIC_LINK:
		openflags |= (O_PATH | O_RDWR | O_NOFOLLOW);
		goto vfos_open;
	...
	case DIRECTORY:
 vfos_open:
		rc = vfs_fsal_open(myself, openflags, &status.major);
		if (rc < 0) {
			LogDebug(COMPONENT_FSAL,
				 "Failed with %s openflags 0x%08x",
				 strerror(-rc), openflags);
 ...
 global:
	...
	*need_fsync = false;
	 ...
	 * temporary file descriptor. Check share reservation.
	 */
	return vfs_reopen_obj(obj_hdl, true, bypass, openflags,
			      fd, has_lock, closefd);
}
