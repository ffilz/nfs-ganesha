	       ...
	       void *caller_arg)
{
	...
	if (read_arg->info != NULL) {
		/*
		 * READ_PLUS returns the content type at the requested offset.
		 * Use SEEK_DATA/SEEK_HOLE to avoid reading (and transmitting)
		 * sparse regions.  A single content element is enough: the client
		 * can issue another READ_PLUS for the following extent.
		 */
		...
		if (next_data < 0 || next_data > read_arg->offset) {
			...
			/* The protocol layer allocated a data buffer before the
			 * FSAL knew this was a hole.  A hole result does not own
			 * that buffer.
			 */
