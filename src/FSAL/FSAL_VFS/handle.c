				    ...
				    struct attrlist *attrs_out)
{
	...
	if (hdl->obj_handle.type == DIRECTORY && hdl->obj_handle.fs->private_data != NULL) {
		...
		if (status.major == ERR_FSAL_NO_ERROR) {
			...
			hdl->u.directory.exportid =
				CityHash64(xattr_content, attrsize);
		}
					 ...
					 uint64_t *exportid)
{
	...
	LogDebug(COMPONENT_FSAL,
		 "fs_location = %p:%s, exportid = %"PRIu64,
		 myself->u.directory.fs_location,
		 myself->u.directory.fs_location,
