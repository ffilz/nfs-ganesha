...
static fsal_status_t getattrs(struct fsal_obj_handle *obj_hdl)
{
	...
	cfd = vfs_fsal_open_and_stat(op_ctx->fsal_export, myself, &stat,
				     O_RDONLY, &fsal_error);
	if (cfd.fd >= 0) {
		st = posix2fsal_attributes(&stat, obj_hdl->attrs);
		if (cfd.close_fd)
			close(cfd.fd);
		if (FSAL_IS_ERROR(st)) {
			FSAL_CLEAR_MASK(obj_hdl->attrs->mask);
			FSAL_SET_MASK(obj_hdl->attrs->mask, ATTR_RDATTR_ERR);
			fsal_error = st.major;
			retval = st.minor;
		} else {
			obj_hdl->attrs->fsid = obj_hdl->fs->fsid;
		}
	} else {
