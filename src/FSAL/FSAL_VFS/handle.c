				  ...
				  bool *eof)
{
	...
	do {
...
#ifdef __FreeBSD__
		/*
		 * This is a very inefficient workaround to retrieve directory offsets.
		 * We rewind dirfd's to its previous offset in order read the directory
		 * entry by entry and fetch every offset.
		 */
		...
		for (bpos = 0; bpos < nread;) {
			...
			nreadent = vfs_readents(dirfd, entbuf, dentryp->vd_reclen, &entloc);
			if (nreadent < 0) {
				retval = errno;
				status = posix2fsal_status(retval);
				goto done;
