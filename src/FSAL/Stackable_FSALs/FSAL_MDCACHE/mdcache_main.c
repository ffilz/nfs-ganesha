			   ...
			   const struct fsal_up_vector *super_up_ops)
{
	...
	if (FSAL_IS_ERROR(status)) {
		...
		gsh_free(myself, MEM_COMP_CACHE);
		return status;
	}
...
static int mdcache_fsal_unload(struct fsal_module *fsal_hdl)
{
	...
	gsh_free(mdcache_entry_pool->name, MEM_COMP_MDCACHE_POOL);
	gsh_free(mdcache_entry_pool, MEM_COMP_MDCACHE_POOL);
