			...
			mdcache_dir_entry_t *dirent)
{
	...
	} else if (!glist_null(&dirent->chunk_list)) {
		/* The dirent is a loose dirent on an LRU list */
		rmv_loose_dirent(parent, dirent);
	}
...
again:
		...
		if (v->chunk != NULL && v2->chunk == NULL) {
			...
			if (code < 0) {
				...
				/* Remove v2 from the loose entry cache */
				rmv_loose_dirent(entry, v2);
			}
