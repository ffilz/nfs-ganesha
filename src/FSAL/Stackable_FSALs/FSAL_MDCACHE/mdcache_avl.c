...
static inline struct avltree_node *
avltree_inline_lookup_dirent_sorted(
	const struct avltree_node *key,
	const struct avltree *tree)
{
	return avltree_inline_lookup(key, tree, avl_dirent_sorted_cmpf);
}
		...
		if (v->ck == parent->fsobj.fsdir.first_ck) {
			...
			while (next != NULL &&
				...
				next =  glist_next_entry(&chunk->dirents,
							 ...
							 chunk_list,
							 &v->chunk_list);
				if (!(next->flags & DIR_ENTRY_FLAG_DELETED)) {
					/* Found a non-deleted entry. */
					break;
				}
				 ...
				 *            entries, we really should just
				 *            free the chunk...
				 */

				/* Look for the next chunk */
				if (mdcache_avl_lookup_ck(parent,
							  chunk->next_ck,
							  &next)) {
					chunk = next->chunk;
				}
