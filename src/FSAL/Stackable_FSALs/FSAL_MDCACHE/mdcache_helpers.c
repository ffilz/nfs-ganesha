...
again:
	...
	for (;
		...
		if (dirent->entry) {
			...
			/**
			 * We shouldn't get here without a write lock. the assumption is
			 * that while we are holding a write lock we get a refcount for
			 * dirent->entry in mdc_populate_dir_chunk and then we later
			 * release it in a if block down below.
			 * So if the dirent->entry is not NULL then somebody is holding
			 * a write lock. If somebody is holding a write lock then
			 * nobody else should hold a read lock on the same directory
			 * and so we can't get here without a write lock
			*/
			if(!has_write){
