	...
	bool *invalidate, struct state_t *state)
{
	...
	if (get_readdir_mode() == FSAL_RDDIR_CHUNK_ONLY) {
				     ...
				     bool *eod_met)
{
	...
	LogInfo(COMPONENT_NFS_READDIR, "Readdir mode %d", dirmode);

	if (dirmode == FSAL_RDDIR_UNCHUNK_ONLY) {
				    ...
				    struct fsal_attrlist *newdir_post_attrs_out)
{
	...
	} else if (get_readdir_mode() == FSAL_RDDIR_CHUNK_ONLY) {
		bool invalidate = true;
