...
#if 0
	/** @todo FSF: SATT15 still doesn't pass without the following...
	 *
	 * Open discussion about change attribute.
	 */
	if (entry->attrs.change < attrs.change) {
		LogDebug(COMPONENT_CACHE_INODE,
			 "refresh attrs tried to roll back change attribute from %lld to %lld",
			 (long long int) entry->attrs.change,
			 (long long int) attrs.change);
		attrs.change = entry->attrs.change;
	}
#endif
				      ...
				      struct attrlist *attrs)
{
	...
	fsal_status_t status;
	uint64_t change;
	LogCrit(COMPONENT_CACHE_INODE, "Got here");
