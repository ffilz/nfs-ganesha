...
#ifdef USE_LTTNG
		tracepoint(mdcache, mdc_lru_ref, __func__, __LINE__,
...
#ifdef USE_LTTNG
			tracepoint(mdcache, mdc_lru_reap, __func__, __LINE__,
...
#ifdef USE_LTTNG
			tracepoint(mdcache, mdc_lru_reap_chunk, __func__,
				   __LINE__, &entry->obj_handle, chunk);
#endif
...
void mdcache_lru_cleanup_try_push(mdcache_entry_t *entry)
{
	...
	cih_latch_entry(&entry->fh_hk.key, &latch, CIH_GET_WLOCK, __func__,
			__LINE__);
	QLOCK(qlane);
...
#ifdef USE_LTTNG
	tracepoint(mdcache, mdc_lru_get, __func__, __LINE__,
		   &nentry->obj_handle, sub_handle, nentry->lru.refcnt);
#endif
	return nentry;
}
...
#ifdef USE_LTTNG
	tracepoint(mdcache, mdc_lru_ref, func, line, &entry->obj_handle,
		   entry->sub_handle, refcnt, active_refcnt);
#endif
...
#ifdef USE_LTTNG
	tracepoint(mdcache, mdc_lru_unref, func, line, &entry->obj_handle,
		   entry->sub_handle, atomic_fetch_int32_t(&entry->lru.refcnt),
		   atomic_fetch_int32_t(&entry->lru.active_refcnt));
#endif
