...
{
	if (0 < chunk_dirty_manage.recovery_interval &&
		CHUNK_DIRTY ==
		atomic_fetch_int32_t(&chunk_dirty_manage.status)) {
		struct timespec current_time;
		struct timespec recovery_time = chunk_dirty_manage.dirty_time;
		int time_expired;
		uint64_t interval = chunk_dirty_manage.recovery_interval;

		interval = interval*NS_PER_SEC
		now(&current_time);
		timespec_add_nsecs(interval, &recovery_time);
		time_expired = gsh_time_cmp(&current_time, &recovery_time);
		if (time_expired >= 0) {
			atomic_set_uint32_t_bits(&chunk_dirty_manage.status,
									CHUNK_CLEAN);
		}
	}
}
