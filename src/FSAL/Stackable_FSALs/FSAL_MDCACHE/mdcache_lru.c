...
chunk_lru_queue_of(struct dir_chunk *chunk)
{
	...
	case LRU_ENTRY_L1:
		q = &LRU[(chunk->chunk_lru.lane)].L1;
		break;
	case LRU_ENTRY_L2:
		q = &LRU[(chunk->chunk_lru.lane)].L2;
		break;
	case LRU_ENTRY_CLEANUP:
		q = &LRU[(chunk->chunk_lru.lane)].cleanup;
		break;
	default:
		/* LRU_NO_LANE */
		q = NULL;
...
lru_reap_chunk_impl(enum lru_q_id qid, mdcache_entry_t *parent)
{
	...
	for (ix = 0;
		...
		/* Get the chunk and entry that owns the chunk, all of this
		 * is valid because we hold the QLANE lock, the chunk was in
		 * the LRU, and thus the chunk is not yet being destroyed,
		...
		if (entry == parent ||
			 ...
			 */
			if (entry != parent) {
				/* We need an LRU ref on entry to protect it
				 * while we do work on it's chunk.
				 */
 ...
 *
 * The caller must hold the content_lock of the parent for write.
 *
...
struct dir_chunk *mdcache_get_chunk(mdcache_entry_t *parent)
{
	...
	/* Enqueue. */
	lru_insert_chunk(chunk, &CHUNK_LRU[chunk->chunk_lru.lane].L1, LRU_LRU);
...
mdcache_lru_pkginit(void)
{
	...
	/* Set high and low watermark for chunks.  XXX This seems a
	   bit fishy, so come back and revisit this. */
	lru_state.chunks_hiwat = mdcache_param.entries_hwmark;
