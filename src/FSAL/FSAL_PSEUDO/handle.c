...
out:
	/* op_ctx->fsal_private == path signals that this is a lookup request
	 * from the export update process and so we should answer it with
	 * the current state */
	if (is_export_update_in_progress() && op_ctx->fsal_private != path) {
