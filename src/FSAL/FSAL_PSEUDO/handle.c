...
verify_handle_is_valid(const struct pseudo_fsal_obj_handle *hdl)
{
	...
	if (hdl->obj_handle.type == DIRECTORY &&
		...
		/* There could be a race case where the node was created, but the
		 * junction still wasn't updated during the export update process.
		 * In this case we return FSAL_DELAY util it is done.
		 */
