...
static int getxattr_cb(rgw_xattrlist *attrs, void *arg, uint32_t flags)
{
	...
	cb_arg->utf8string_val =
		strndup(attrs->xattrs->val.val, attrs->xattrs->val.len);
				...
				xattrvalue4 *xa_value)
{
	...
	attrs.xattrs->key.val =
		strndup(xa_name->utf8string_val, xa_name->utf8string_len);
				...
				xattrvalue4 *xa_value)
{
	...
	attrs.xattrs->val.val =
		strndup(xa_value->utf8string_val, xa_value->utf8string_len);
...
static int lsxattr_cb(rgw_xattrlist *attrs, void *cb_arg, uint32_t flag)
{
	...
	cb->names->entries[cb->names->entryCount].utf8string_val =
		strndup(attrs->xattrs[0].key.val, attrs->xattrs[0].key.len);
	cb->names->entries[cb->names->entryCount].utf8string_len =
		attrs->xattrs[0].key.len;
	cb->names->entryCount++;
