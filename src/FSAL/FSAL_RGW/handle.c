...
static int getxattr_cb(xattrlist *attrs, void *arg, uint32_t flags)
{
	arg = attrs->xattrs->val;
	return 0;
}
...
static int lsxattr_cb(xattrlist *attrs, void *cb_arg, uint32_t flag)
{
	cb_arg->names.entries[cb_arg->name.entryCount] = attrs->xattrs[0].key;
	cb_arg->names.entryCount++;

	if (cb_arg->names.entryCount == cb_arg->max) {
				...
				xattrlist4 *lr_names)
{
	...
	rc = rgw_lsxattrs(export->rgw->fs, handle->rgw_fh,
			  attrs->names->entries /*not yet implemented */,
