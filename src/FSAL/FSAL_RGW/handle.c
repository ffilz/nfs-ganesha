			...
			struct io_info *info)
{
	...
	bool need_correct = (obj_hdl->rgw_write_offset > offset)
		&& (buffer_size >= (obj_hdl->rgw_write_offset - offset));
	LogFullDebug(COMPONENT_FSAL, "FSAL WRITE operation prepare. "
		"now_off:%lu req_off:%lu size:%lu goto correct[%d]",
		obj_hdl->rgw_write_offset, offset, buffer_size, need_correct);
	void *write_buff = buffer;
	size_t write_io_size = buffer_size;

	if (need_correct) {
		LogFullDebug(COMPONENT_FSAL,
			"FSAL WRITE operation corrected! "
			"now_off:%lu req_off:%lu size:%lu, then size:%lu skip:%lu",
		...
		if (buffer_size == (obj_hdl->rgw_write_offset - offset)) {
			*wrote_amount = buffer_size;
			...
			write_io_size = buffer_size -
				(obj_hdl->rgw_write_offset - offset);

			uint8_t *tmp = (uint8_t *)buffer;

			write_buff = &(tmp[obj_hdl->rgw_write_offset - offset]);
	...
	LogFullDebug(COMPONENT_FSAL,
		"%s post obj_hdl %p state %p returned %d asked_size=%lu"
		" effective_size=%lu offset:%ld sync:%d",
		__func__, obj_hdl, state, rc, buffer_size,
		*wrote_amount, offset, *fsal_stable);
