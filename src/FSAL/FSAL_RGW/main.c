				   ...
				   const struct fsal_up_vector *up_ops)
{
	...
	if (!RGWFSM.rgw) {
		...
		if (!RGWFSM.rgw) {
			...
			if (RGWFSM.conf_path) {
				...
				conf_path =
					(char *)gsh_malloc(clen, MEM_COMP_MISC);
			...
			gsh_free(conf_path, MEM_COMP_MISC);
			gsh_free(inst_name, MEM_COMP_MISC);
			gsh_free(cluster, MEM_COMP_MISC);
		}
	...
	export = gsh_calloc(1, sizeof(struct rgw_export), MEM_COMP_FSAL);
