 ...
 *                                           file
 * @param [in] afterMknode                   The file is opened after
 *                                           performing a makenode operation
 *
 * @returns: FSAL status
 */
	     ...
	     bool afterMknode)
{
	...
	if (createmode >= FSAL_EXCLUSIVE || attributes) {
		...
		int getattrStatus =
			saunafs_getattr(export->fsInstance, &op_ctx->creds,
		...
		struct fsal_attrlist *attributes, bool *callerPermissionCheck)
{
	...
	if (name == NULL) {
		status = openByHandle(objectHandle, state, openflags,
				      createmode, verifier, attributes, false);
				...
				struct state_t *state)
{
	...
	return saunafsFd->fsalFd.openflags;
}
