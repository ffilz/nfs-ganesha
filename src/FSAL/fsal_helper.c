					      ...
					      struct fsal_attrlist *current)
{
	...
	if (FSAL_TEST_MASK(attr->valid_mask, ATTR_OWNER)) {
		...
		if (attr->owner != op_ctx->creds.caller_uid) {
			...
			if ((op_ctx->cred_flags & UID_SQUASHED) != 0 &&
				...
				/* Original caller was root. Allow setting owner to any UID
				 * that root would normally be able to set (including the
				 * squashed UID). For root_squash, this allows root to set
				 * owner to the squashed UID (65534).
				 */
				LogDebug(
					COMPONENT_FSAL,
	...
	if ((access_check & ~FSAL_ACE4_MASK_SET(FSAL_ACE_PERM_WRITE_DATA)) !=
	    0) {
		/* Need permissions like WRITE_ACL or WRITE_ATTR (but not WRITE_OWNER
		 * for taking ownership, which is already handled above).
		 * Without ACL, check write permission as a fallback.
		 */
		if (access_check & FSAL_ACE_PERM_WRITE_OWNER) {
			/* WRITE_OWNER without ACL is only allowed for taking ownership,
			 * which we already handled above. If we get here, it's for
			 * changing owner to a different UID, which we reject.
			 */
			status = fsalstat(ERR_FSAL_PERM, 0);
			note = " (no ACL to check WRITE_OWNER)";
			goto out;
		}
		/* For WRITE_ACL or WRITE_ATTR, check write permission as fallback */
