...
{
	const uint32_t lookup_access_check_policy = atomic_fetch_uint32_t(
		&op_ctx->ctx_export->lookup_access_check_policy);
	const fsal_accessflags_t access_mask =
		(FSAL_MODE_MASK_SET(FSAL_X_OK) |
		 FSAL_ACE4_MASK_SET(FSAL_ACE_PERM_EXECUTE));

	if (lookup_access_check_policy & lookup_access_check_policy_to_check)
			  ...
			  struct fsal_attrlist *attrs_out)
{
	...
	*obj = NULL;
	...
	fsal_status = allow_lookup(parent, LOOKUP_ACCESS_CHECK_POLICY_PRE);
	...
	fsal_status = allow_lookup(parent, LOOKUP_ACCESS_CHECK_POLICY_POST);
	if (FSAL_IS_ERROR(fsal_status)) {
...
{
	const uint32_t readdir_access_check_policy = atomic_fetch_uint32_t(
		&op_ctx->ctx_export->readdir_access_check_policy);
	fsal_status_t fsal_status = { 0, 0 };
	...
	*out_should_fetch_attr = false;
	...
	if (!(readdir_access_check_policy &
	      readdir_access_check_policy_to_check)) {
					    ...
					    fsal_cookie_t cookie)
{
	...
	if (!state->post_access_check_done) {
		status = allow_readdir(state->directory, state->attrmask,
				       READDIR_ACCESS_CHECK_POLICY_POST,
