...
def draw_footbar(stdscr, height, width):
    # content
    current_time = datetime.now()
    time_str = current_time.strftime("%Y-%m-%d %H:%M:%S")
    footbar_str = ("Press 'q' to exit | Press 'h' for help | {}"
                   .format(time_str))
...
def draw_menu(stdscr, interval):
    ...
    while (1):
        ...
        if screen_opt == ord('q'):
            ...
            draw_help_page(stdscr)
        elif screen_opt == ord('c'):
            draw_client_page(stdscr)
        elif screen_opt == ord('d'):
            draw_default_page(stdscr)
        elif screen_opt == ord('e'):
            draw_export_page(stdscr)
        else:
            draw_default_page(stdscr)
