...
To show/reset memory stat counters use:
  {progname} [mem_stats] [show | reset]
...
# requires fsal name
    ...
    if not len(opts) == 1:
        ...
        print_usage_exit(1)
    command_arg = opts[0]
    if command_arg not in ('show', 'reset'):
        print("\nError: Option '%s' must be followed by show/reset" % command)
        print_usage_exit(1)
elif command == "help":
    print_usage_exit(0)
...
try:
    ...
    if command == "global":
        ...
        elif command_arg == "reset":
            result = mem_interface.reset_mem_stats()
