...
class AllClientsIOops(Report):
    ...
    def __init__(self, stats):
        ...
        # stats format: [timestamp, [(client_ip, timestamp, nfsv3_available, ...), ...]]
        if len(stats) > 1 and stats[1]:
            for client_data in stats[1]:
                try:
    ...
    def fill_report(self, report):
        ...
        if len(self.stats) > 1 and self.stats[1]:
            for client_data in self.stats[1]:
                try:
                    client_ip = str(client_data[0])
