...
{
	gsh_free(info->ng_group.addr, MEM_COMP_CLIENT);
	gsh_free(info->ng_host.addr, MEM_COMP_CLIENT);
	gsh_free(info, MEM_COMP_CLIENT);
}
...
static void ng_add(const char *group, const char *host, bool negative)
{
	...
	info = gsh_malloc(sizeof(struct ng_cache_info), MEM_COMP_CLIENT);
	if (!info)
		LogFatal(COMPONENT_IDMAPPER, "memory alloc failed");

	info->ng_group.addr = gsh_strdup(group, MEM_COMP_CLIENT);
	info->ng_group.len = strlen(group) + 1;
	info->ng_host.addr = gsh_strdup(host, MEM_COMP_CLIENT);
