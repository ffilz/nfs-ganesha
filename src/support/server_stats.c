	...
	for (op = 1; op < NFS4_OP_LAST_ONE; op++) {
		if (path[0] != '\0' && strcmp(path, "/") != 0 &&
			...
			nfs_metrics__nfs4_request(
				op, export->metric_v4_full_stats[op].total,
				/*export->metric_v4_full_stats[op].latency.latency*/
				0, 0, export->export_id, path, NULL);
		}
	}
	return true;
}

void update_export_metrics()
{
	int op;
	...
	for (op = 1; op < NFS4_OP_LAST_ONE; op++) {
		if (metric_total_v4_full_stats[op].total != 0)
			nfs_metrics__nfs4_request(
				op, metric_total_v4_full_stats[op].total,
				/*metric_total_v4_full_stats[op].latency.latency*/ 0,
				0, 0, NULL, NULL);
