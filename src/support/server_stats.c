...
#endif
	stats_available = (st->nfsv40 != 0 && st->nfsv40->compounds.total != 0);
	protocol = "NFSv40";
	dbus_message_iter_append_protocol_info(&st_iter, &protocol,
					       &stats_available);
	stats_available = (st->nfsv41 != 0 && st->nfsv41->compounds.total != 0);
	protocol = "NFSv41";
	dbus_message_iter_append_protocol_info(&st_iter, &protocol,
					       &stats_available);

	stats_available = (st->nfsv42 != 0 && st->nfsv42->compounds.total != 0);
	protocol = "NFSv42";
	dbus_message_iter_append_protocol_info(&st_iter, &protocol,
					       &stats_available);
#ifdef _USE_9P
