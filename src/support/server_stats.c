...
void server_nfsmon_server_iostats(struct server_stats *export_st,
                                  struct xfer_op *opread, struct xfer_op *opwrite)
{
        if (export_st->st.nfsv3 != NULL) {
                (void)atomic_add_uint64_t(&opread->cmd.total,
                                          export_st->st.nfsv3->read.cmd.total);
                (void)atomic_add_uint64_t(&opread->requested,
                                          export_st->st.nfsv3->read.requested);
                (void)atomic_add_uint64_t(&opread->transferred,
                                          export_st->st.nfsv3->read.transferred);
                (void)atomic_add_uint64_t(&opwrite->cmd.total,
                                          export_st->st.nfsv3->write.cmd.total);
                (void)atomic_add_uint64_t(&opwrite->requested,
                                          export_st->st.nfsv3->write.requested);
                (void)atomic_add_uint64_t(&opwrite->transferred,
                                          export_st->st.nfsv3->write.transferred);
        }

        if (export_st->st.nfsv40 != NULL) {
                (void)atomic_add_uint64_t(&opread->cmd.total,
                                          export_st->st.nfsv40->read.cmd.total);
                (void)atomic_add_uint64_t(&opread->requested,
                                          export_st->st.nfsv40->read.requested);
                (void)atomic_add_uint64_t(&opread->transferred,
                                          export_st->st.nfsv40->read.transferred);
                (void)atomic_add_uint64_t(&opwrite->cmd.total,
                                          export_st->st.nfsv40->write.cmd.total);
                (void)atomic_add_uint64_t(&opwrite->requested,
                                          export_st->st.nfsv40->write.requested);
                (void)atomic_add_uint64_t(&opwrite->transferred,
                                          export_st->st.nfsv40->write.transferred);
        }

        if (export_st->st.nfsv41 != NULL) {
                (void)atomic_add_uint64_t(&opread->cmd.total,
                                          export_st->st.nfsv41->read.cmd.total);
                (void)atomic_add_uint64_t(&opread->requested,
                                          export_st->st.nfsv41->read.requested);
                (void)atomic_add_uint64_t(&opread->transferred,
                                          export_st->st.nfsv41->read.transferred);
                (void)atomic_add_uint64_t(&opwrite->cmd.total,
                                          export_st->st.nfsv41->write.cmd.total);
                (void)atomic_add_uint64_t(&opwrite->requested,
                                          export_st->st.nfsv41->write.requested);
                (void)atomic_add_uint64_t(&opwrite->transferred,
                                          export_st->st.nfsv41->write.transferred);
        }

        if (export_st->st.nfsv42 != NULL) {
                (void)atomic_add_uint64_t(&opread->cmd.total,
                                          export_st->st.nfsv42->read.cmd.total);
                (void)atomic_add_uint64_t(&opread->requested,
                                          export_st->st.nfsv42->read.requested);
                (void)atomic_add_uint64_t(&opread->transferred,
                                          export_st->st.nfsv42->read.transferred);
                (void)atomic_add_uint64_t(&opwrite->cmd.total,
                                          export_st->st.nfsv42->write.cmd.total);
                (void)atomic_add_uint64_t(&opwrite->requested,
                                          export_st->st.nfsv42->write.requested);
                (void)atomic_add_uint64_t(&opwrite->transferred,
                                          export_st->st.nfsv42->write.transferred);
        }
}
...
void server_dbus_nfsmon_export_iostats(struct export_stats *export_st,
                                       DBusMessageIter *iter)
{
...
void server_dbus_nfsmon_server_iostats(struct server_stats *export_st,
                                       DBusMessageIter *iter)
{
        struct xfer_op *op_preread = NULL;
        struct xfer_op *op_prewrite = NULL;
