...
/**
 * @brief Look up a client from its IP address for QoS operations
 *
 * @param ipaddr [IN] Client IP address
 * @param success [OUT] Lookup status
 * @param errormsg [OUT] Error message
 *
 * @return gsh_client on success, NULL otherwise
 */
static struct gsh_client *grpc_qos_lookup_client(const char *ipaddr,
						 bool *success,
						 const char **errormsg)
{
	sockaddr_t sockaddr;
	...
	*errormsg = "OK";
	...
	if (ipaddr == NULL || ipaddr[0] == '\0') {
		*success = false;
		*errormsg = "Invalid client IP address";
		return NULL;
	}
					  ...
					  size_t errmsg_len)
{
	...
	*success = false;
	...
	gsh_client =
		grpc_qos_lookup_client(client_ip, &lookup_success, &errormsg);
