				   ...
				   DBusError *error)
{
	...
	client_qos = get_client_qos(client_ip, true);
	if (client_qos) {
		PTHREAD_MUTEX_lock(&client_qos->lock);
		client_qos->rbucket.max_bw_allowed = read_bw;
		client_qos->wbucket.max_bw_allowed = write_bw;
				      ...
				      DBusError *error)
{
	...
	if (client_qos) {
		PTHREAD_MUTEX_lock(&client_qos->lock);
		client_qos->rbucket.max_available_tokens = max_tokens;
		client_qos->wbucket.max_available_tokens = max_tokens;
		client_qos->rbucket.tokens_renew_time = token_renewal;
		client_qos->wbucket.tokens_renew_time = token_renewal;
		PTHREAD_MUTEX_unlock(&client_qos->lock);
		put_qos_config_lock();
		return true;
	}
				      ...
				      DBusError *error)
{
	...
	if (client_qos) {
		PTHREAD_MUTEX_lock(&client_qos->lock);
		uint32_t max_tokens = client_qos->rbucket.max_available_tokens;
		uint64_t token_renewal = client_qos->rbucket.tokens_renew_time;
				   ...
				   DBusError *error)
{
	...
	if (client_qos) {
		PTHREAD_MUTEX_lock(&client_qos->lock);
		uint64_t read_bw = client_qos->rbucket.max_bw_allowed;
		uint64_t write_bw = client_qos->wbucket.max_bw_allowed;
 ...
 * @return true if successful, false otherwise
 */
static bool client_qos_to_dbus(qos_class_t *client, void *state)
{
	...
	if (client->client_addr->cl_addrbuf.ss_family == AF_INET) {
					  ...
					  DBusMessage *reply, DBusError *error)
{
	...
	export = get_export_qos(gsh_export, true);
	if (!export) {
				   ...
				   DBusError *error)
{
	...
	if (export) {
		...
		enable_bw_ptr = enable_bw_str;
		snprintf(read_bw_str, sizeof(read_bw_str), "%lu",
			 export->rbucket.max_bw_allowed);
		read_bw_ptr = read_bw_str;
		snprintf(write_bw_str, sizeof(write_bw_str), "%lu",
			 export->wbucket.max_bw_allowed);
		write_bw_ptr = write_bw_str;
				   ...
				   DBusError *error)
{
	...
	if (export) {
		PTHREAD_MUTEX_lock(&export->lock);
		export->rbucket.max_bw_allowed = read_bw;
		export->wbucket.max_bw_allowed = write_bw;
		PTHREAD_MUTEX_unlock(&export->lock);
	} else {
		put_gsh_export(gsh_export);
		gsh_dbus_status_reply(&iter, false, "check config values");
				      ...
				      DBusError *error)
{
	...
	if (export) {
		PTHREAD_MUTEX_lock(&export->lock);
		export->rbucket.max_available_tokens = max_tokens;
		export->wbucket.max_available_tokens = max_tokens;
		export->rbucket.tokens_renew_time = token_renewal;
		export->wbucket.tokens_renew_time = token_renewal;
		PTHREAD_MUTEX_unlock(&export->lock);
	} else {
		put_gsh_export(gsh_export);
		gsh_dbus_status_reply(&iter, false, "check config values");
					 ...
					 DBusMessage *reply, DBusError *error)
{
	...
	PTHREAD_MUTEX_lock(&export->lock);
	client = pepc_get_client_from_list(&export->clients, client_ip);
	if (client) {
		PTHREAD_MUTEX_lock(&client->lock);
		client->bw_enabled = true;
		client->rbucket.max_bw_allowed = read_bw;
		client->wbucket.max_bw_allowed = write_bw;
		PTHREAD_MUTEX_unlock(&client->lock);
	}
	PTHREAD_MUTEX_unlock(&export->lock);
