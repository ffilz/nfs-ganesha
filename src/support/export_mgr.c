 ...
 * @param token         [IN]  the name string.  We modify it.
		  ...
		  struct config_error_type *err_type)
{
	...
	//char *endptr;
	int errcnt = 0;
	//uint16_t export_id = (uint16_t)strtoull(token, &endptr, 10);

	//LogEvent(component, "exp_id is %s and in no. %d", token, export_id);
