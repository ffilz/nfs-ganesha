...
static void queue_deleg_transition_handler(struct fridgethr_context *ctx)
{
	...
	glist_for_each_safe(glist, glistn, &export->exp_state_list) {
		...
		if (state->state_type == STATE_TYPE_DELEG) {
			...
			/* Recall if there is an outstanding write delegation
			 * and the write delegation was disabled in the config
			 */
			if (deleg->sd_type == OPEN_DELEGATE_WRITE &&
			    !(op_ctx->export_perms.options &
			      EXPORT_OPTION_WRITE_DELEG))
				goto delegrecall;
			/* Recall if there is an outstanding read delegation
			 * and the read delegation was disabled in the config
			 */
			else if (deleg->sd_type == OPEN_DELEGATE_READ &&
				 !(op_ctx->export_perms.options &
...
delegrecall:
			...
			/* Asynchronous delegrecall implementation per state */
			if (async_delegrecall_per_state(general_fridge, obj,
							state) != 0)
				LogFullDebug(COMPONENT_FSAL,
					     "Unable to recall");
		}
