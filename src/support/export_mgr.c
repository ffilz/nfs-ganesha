			     ...
			     void *state)
{
	...
	case NETWORK_CLIENT:
		grp_name =
			cidr_to_str(client->client.network.cidr, MEM_COMP_DBUS);
		free_grp_name = true;
		if (grp_name == NULL) {
	...
	if (free_grp_name)
		gsh_free((char *)grp_name, MEM_COMP_DBUS);
		  ...
		  struct config_error_type *err_type)
{
	...
	expidli = gsh_calloc(1, sizeof(struct export_id_list), MEM_COMP_DBUS);
