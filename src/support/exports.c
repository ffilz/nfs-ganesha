...
static inline bool is_proxy_protocol_connection(const SVCXPRT *const xprt)
{
 ...
 *
 * Given an export client entry checks if the current request is in PROXY protocol
 * and the export client entry applies to PROXY protocol requests
 * or the current request is not in PROXY protocol and the client entry applies to 
 * non-PROXY protocol requests.
 */
	...
	const struct base_client_entry *const client)
{
	...
	if (is_proxy_protocol_connection(op_ctx->nfs_reqdata->svc.rq_xprt))
		return expclient->proxy_protocol_type ==
		       CLIENT_ENTRY_PROXY_PROTOCOL_WITH;
