			      ...
			      struct exportlist_client_entry *entry)
{
	...
	if (b_left > 0) {
		if (entry->proxy_protocol_type ==
		    CLIENT_ENTRY_PROXY_PROTOCOL_WITH)
			b_left = display_cat(&dspbuf, " proxy-protocol");
		else if (entry->proxy_protocol_type ==
			 CLIENT_ENTRY_PROXY_PROTOCOL_WITHOUT)
			b_left = display_cat(&dspbuf, " non-proxy-protocol");
	}
...
void export_client_fillter(struct base_client_entry *client, void *private_data)
{
	struct exportlist_client_entry *expcli;
	...
	expcli->proxy_protocol_type = proto_expcli->proxy_protocol_type;
			...
			struct config_error_type *err_type)
{
	...
	rc = add_client(COMPONENT_EXPORT, &client->cle_list, token, type_hint,
			...
			export_client_fillter, proto_cli);
	return rc;
}
...
static struct config_item_list export_client_proxy_protocol[] = {
	CONFIG_LIST_TOK("with_proxy", CLIENT_ENTRY_PROXY_PROTOCOL_WITH),
	CONFIG_LIST_TOK("without_proxy", CLIENT_ENTRY_PROXY_PROTOCOL_WITHOUT),
	CONFIG_LIST_TOK("any", CLIENT_ENTRY_PROXY_PROTOCOL_ANY), CONFIG_LIST_EOL
};
...
static struct config_item client_params[] = {
	...
	CONF_ITEM_TOKEN("ProxyProtocol", CLIENT_ENTRY_PROXY_PROTOCOL_ANY,
			export_client_proxy_protocol, exportlist_client_entry,
			proxy_protocol_type),
...
static inline bool
is_rpc_address_initialized(const struct rpc_address *const address)
{
	return address->ss.ss_family != 0;
}

/**
 * @brief Checks if an export client entry should apply to the current op
 * based on if it should apply to proxied requests.
 *
 * An export client entry might be scoped only to local (non-PROXY protocol) requests
 * or PROXY protocol requests.
 * This checks if a client applies to PROXY protocol requests and the current
 * op is in PROXY protocol or vice-versa.
 */

static bool client_entry_match_op_proxy_protocol(
	const struct base_client_entry *const client)
{
	...
	LogMidDebug_ExportClientListEntry("Check proxy match: ", expclient);

	if (expclient->proxy_protocol_type == CLIENT_ENTRY_PROXY_PROTOCOL_ANY) {
		LogMidDebug(COMPONENT_EXPORT,
			    "client_entry_match is ANY - returning true");
		return true;
	}

	const bool is_op_proxy_protocol = is_rpc_address_initialized(
		&op_ctx->nfs_reqdata->svc.rq_xprt->xp_proxy);

	return (is_op_proxy_protocol &&
		expclient->proxy_protocol_type ==
			CLIENT_ENTRY_PROXY_PROTOCOL_WITH) ||
	       (!is_op_proxy_protocol &&
		expclient->proxy_protocol_type ==
			CLIENT_ENTRY_PROXY_PROTOCOL_WITHOUT);
}
...
void export_check_access(void)
{
	...
	if (glist_empty(&op_ctx->ctx_export->clients)) {
		...
		client = client_match(COMPONENT_EXPORT, exp_str,
				      ...
				      client_entry_match_op_proxy_protocol);
	} else {
