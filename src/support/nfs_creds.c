...
static void set_extended_groups(void)
{
	...
	if ((op_ctx->cred_flags & MANAGED_GIDS) != 0) {
		...
		if (op_ctx->caller_gdata == NULL &&
			...
			GSH_AUTO_TRACEPOINT(nfs_creds, failed_attempt_fetch_managed_gids, TRACE_INFO,
			...
			if (nfs_param.core_param.enable_rpc_cred_fallback) {
				LogInfo(COMPONENT_DISPATCH,
					...
					op_ctx->original_creds.caller_uid);
				GSH_AUTO_TRACEPOINT(nfs_creds, use_rpc_cred_fallback, TRACE_INFO,
				...
				LogInfo(COMPONENT_DISPATCH,
					...
					op_ctx->original_creds.caller_uid);
				GSH_AUTO_TRACEPOINT(nfs_creds, failed_fetch_managed_gids, TRACE_INFO,
...
nfsstat4 nfs_req_creds(struct svc_req *req)
{
	...
	GSH_AUTO_TRACEPOINT(nfs_creds, req_creds, TRACE_INFO,
	...
	GSH_AUTO_TRACEPOINT(nfs_creds, req_creds_groups, TRACE_INFO,
		...
		TP_INT_ARR_TRUNCATED(op_ctx->creds.caller_garray, op_ctx->creds.caller_glen),
		(op_ctx->cred_flags & MANAGED_GIDS) != 0,
		(op_ctx->cred_flags & GARRAY_SQUASHED) != 0
	);
