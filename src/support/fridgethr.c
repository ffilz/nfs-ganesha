		   ...
		   const struct fridgethr_params *p)
{
	...
	frobj = gsh_malloc(sizeof(struct fridgethr), MEM_COMP_MISC);
	...
	*frout = NULL;
	...
	frobj->s = gsh_strdup(s, MEM_COMP_MISC);
...
inval:
	...
	gsh_free(frobj->s, MEM_COMP_MISC);
	gsh_free(frobj, MEM_COMP_MISC);
...
void fridgethr_destroy(struct fridgethr *fr)
{
	...
	gsh_free(fr->s, MEM_COMP_MISC);
	gsh_free(fr, MEM_COMP_MISC);
}
			   ...
			   void (*func)(struct fridgethr_context *), void *arg)
{
	...
	fe = gsh_calloc(1, sizeof(struct fridgethr_entry), MEM_COMP_MISC);
			   ...
			   void (*func)(struct fridgethr_context *), void *arg)
{
	...
	q = gsh_malloc(sizeof(struct fridgethr_work), MEM_COMP_MISC);
