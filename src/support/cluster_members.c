				     ...
				     char **errormsg)
{
	...
	if (strchr(token, '/') != NULL) {
		*errormsg =
			"Cluster_Members allows only individual IPv4/IPv6 addresses, not CIDRs";
		return false;
	}

	cidr = cidr_from_str(token);
	if (cidr == NULL) {
		*errormsg =
			"Cluster_Members entry is not a valid IPv4 or IPv6 address";
		return false;
	}
...
bool set_cluster_members(char **ips, size_t count, char **errormsg)
{
	...
	*errormsg = "OK";
	...
	FreeClientList(&nfs_param.core_param.cluster_members, NULL);
	FreeClientList(&nfs_param.core_param.cluster_self, NULL);

	glist_splice_tail(&nfs_param.core_param.cluster_members, &temp_list);
...
#ifdef _INTERNAL_STATD
	{
		...
		/* Drop list lock before touching nsm_notify_thread lifecycle. */
...
bool show_cluster_members(char ***ips_out, size_t *count_out, char **errormsg)
{
	...
	*errormsg = "";
	...
	ips = gsh_calloc(count, sizeof(*ips));
