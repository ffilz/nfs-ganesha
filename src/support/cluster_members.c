				     ...
				     char **errormsg)
{
	...
	cidr = cidr_from_str(token, MEM_COMP_CONFIG);
	if (cidr == NULL) {
...
bool set_cluster_members(char **ips, size_t count, char **errormsg)
{
	...
	*errormsg = "OK";
	...
	/* Verify Cluster_Memners doesn't change from empty to non-emptyt. It is
	 * valid for the list to ONLY be a single address that is the node's
	 * address.
	 */
...
void cqos_hosts_release(struct glist_head *list, mem_components_t comp)
{
	//
}
#endif
