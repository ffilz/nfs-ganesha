...
/**
 * @brief Determine whether one CIDR contains another CIDR
 *
 * @param[in]  a       Candidate containing CIDR
 * @param[in]  b       CIDR to test for inclusion
 *
 * Checks whether CIDR @p a fully contains CIDR @p b.  Two CIDRs are
 * considered in a containment relationship when they belong to the
 * same address family, the mask of @p a is equal to or broader than
 * the mask of @p b, and the network address of @p b lies within the
 * network defined by @p a.
 *
 * This is primarily used for client matching where a configured
 * network is allowed to match more specific client networks or
 * individual client addresses contained within it.
 *
 * @return true if CIDR @p a contains CIDR @p b, otherwise false.
 */
