...
void init_client_data_for_xprt(SVCXPRT *xprt)
{
	...
	/* Copy the socket address from xprt for logging */
	copy_xprt_addr(&addr, xprt);
	display_sockaddr(&db, &addr);

	if (xprt->xp_u1 != NULL) {
		LogInfo(COMPONENT_XPRT,
			"xp_u1 is already initialised for xprt with FD: %d and"
			" socket-addr: %s", xprt->xp_fd, sockaddr_str);
		return;
	}
	...
	nfs41_session_t *nfs41_session)
{
	...
	if (xprt->xp_u1 == NULL) {
		LogCrit(COMPONENT_XPRT,
			"xprt->xp_u1 is not initialised for xprt FD: %d",
			xprt->xp_fd);
		return false;
	}
	...
	if (xprt->xp_flags & SVC_XPRT_FLAG_DESTROYING) {
		PTHREAD_RWLOCK_unlock(
			&xprt_client_data->nfs41_session_list_lock);
		LogInfo(COMPONENT_SESSIONS,
			"Do not associate under-destruction xprt with FD: %d to"
			" the session", xprt->xp_fd);
		gsh_free(new_entry);
		return false;
	}
	...
	SVCXPRT *xprt)
{
	...
	/* After acquiring the lock, we re-check backchannel availability */
	...
	if (clnt_vc_get_client_xprt(session->cb_chan.clnt) != xprt) {
		...
		LogDebug(COMPONENT_XPRT,
			"Backchannel xprt for session %s does not match the"
			" xprt to be destroyed. Skip destroying backchannel",
			session_str);
		return;
	}
...
void destroy_client_data_for_destroyed_xprt(SVCXPRT *xprt)
{
	...
	LogInfo(COMPONENT_XPRT,
		"Processing client data for destroyed xprt: %p with FD: %d,"
		" socket-addr: %s", xprt, xprt->xp_fd, sockaddr_str);
