...
const nfs_function_desc_t *nfs_rpc_get_funcdesc(nfs_request_t *reqnfs)
{
	...
	if (reqnfs->svc.rq_msg.cb_prog == NFS_pcpp[P_NFS]) {
		funcdesc =
#ifdef _USE_NFS3
...
#ifdef _USE_NLM
	} else if (reqnfs->svc.rq_msg.cb_prog == NFS_pcpp[P_NLM]) {
		funcdesc = &nlm4_func_desc[reqnfs->svc.rq_msg.cb_proc];
#endif /* _USE_NLM */
	} else if (reqnfs->svc.rq_msg.cb_prog == NFS_pcpp[P_MNT]) {
	...
	} else if (reqnfs->svc.rq_msg.cb_prog == NFS_pcpp[P_RQUOTA]) {
...
void nfs_rpc_execute(request_data_t *reqdata)
{
	...
	if (reqdata->r_u.req.svc.rq_msg.cb_prog == NFS_pcpp[P_NFS]) {
		 ...
		 * in the compound */
#ifdef _USE_NLM
	} else if (reqdata->r_u.req.svc.rq_msg.cb_prog == NFS_pcpp[P_NLM]) {
		netobj *pfh3 = NULL;
...
#endif /* _USE_NLM */
	} else if (reqdata->r_u.req.svc.rq_msg.cb_prog == NFS_pcpp[P_MNT]) {
		progname = "MNT";
	}
	...
	if (op_ctx->ctx_export != NULL
		...
		LogInfoAlt(COMPONENT_DISPATCH, COMPONENT_EXPORT,
			...
			op_ctx->ctx_export->fullpath,
			(int)reqdata->r_u.req.svc.rq_msg.cb_vers,
			(int)reqdata->r_u.req.svc.rq_msg.cb_proc);
		auth_rc = AUTH_TOOWEAK;
		goto auth_failure;
	} else {
 ...
 */
	...
	if (isInfo(COMPONENT_DISPATCH) || isInfo(COMPONENT_EXPORT)) {
		...
		LogInfo(COMPONENT_DISPATCH,
			...
			", FH=%s",
			progname, client_ip,
			(int)reqdata->r_u.req.svc.rq_msg.cb_proc, dumpfh);
	}
	auth_rc = AUTH_FAILED;
