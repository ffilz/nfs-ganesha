					      ...
					      bool retry)
{
	...
	if (xprt->xp_tls.tls_pending) {
		...
		if (auth_rc == AUTH_OK) {
			...
			LogCrit(COMPONENT_CONFIG, "Processed AUTH_TLS probe on fd %d", xprt->xp_fd);
			return SVC_STAT(xprt);
		} else {
			/* Invalid AUTH_TLS probe */
			LogCrit(COMPONENT_CONFIG, "Invalid AUTH_TLS probe, auth_result=%d", auth_rc);
			//return SVC_STAT(xprt);
		...
		if ((op_ctx->export_perms.options &
	       ...
	       if ((op_ctx->export_perms.options & EXPORT_OPTION_TLS) != 0) {
		       if (!(xprt->xp_tls.tls_established)) {
			       ...
			       "[TLS] NFS auth not allowed on Export_Id %d %s for client %s",
