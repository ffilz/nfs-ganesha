...
static CLIENT *cqos_create_rpc_client(int fd)
{
	...
	if (clnt_control(clnt, CLSET_FD_NCLOSE, NULL)) {
		LogCrit(COMPONENT_QOS, "CQOS: Unable to set "
				"fd don't close option");
	}
...
static void cqos_rpc_call_process(struct clnt_req *cc)
{
	...
	LogCrit(COMPONENT_QOS, "CQOS: Sending RPCmsg failed status=%d",
	...
	if (!glist_empty(&cqos_hosts)) {
		...
		if (node->clnt == cc->cc_clnt) {
			LogCrit(COMPONENT_QOS, "CQOS: Found a node "
					"with client handle");
...
static bool cqos_send_rpc_msg(CLIENT *clnt, cluster_qos_msg msg)
{
	...
	if (cc->cc_error.re_status != RPC_SUCCESS) {
		LogCrit(COMPONENT_QOS, "CQOS: Sending clnt RPC msg failed");
		clnt_req_release(cc);
		return false;
	}
