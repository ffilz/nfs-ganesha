...
static void drc_to_dbus(drc_t *drc, void *state) {
	...
	dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_STRING, &str);
	dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_UINT32, &drc->size);
 ...
 * @param[in]  args
 */
static bool admin_dbus_get_drc_info(DBusMessageIter *args,
				DBusMessage *reply,
				DBusError *error)
{
	...
	dbus_message_iter_open_container(
		&iter, DBUS_TYPE_ARRAY, "(sssu)", &array_iter);
	...
	dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_STRING, &str);
	dbus_message_iter_append_basic(&struct_iter, DBUS_TYPE_UINT32, &counter);
