	...
	if (prctl(PR_SET_IO_FLUSHER, 1, 0, 0, 0) == -1) {
		if (errno == EPERM) {
			if (nfs_param.core_param
				    .allow_unprivileged_set_io_flusher)
				LogWarn(COMPONENT_MAIN,
					"Failed to set PR_SET_IO_FLUSHER due to EPERM, ignoring...");
			else {
				LogFatal(
					COMPONENT_MAIN,
					"Failed to PR_SET_IO_FLUSHER with EPERM. Take a look at config option allow_unprivileged_set_io_flusher to see if you should allow it");
				goto fatal_die;
			}
