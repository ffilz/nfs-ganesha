...
enum xprt_stat nfs_rpc_process_request(request_data_t *reqdata)
{
	...
	if (!SVCAUTH_CHECKSUM(reqdata->r_u.req.svc.rq_auth,
			      &reqdata->r_u.req.svc, xdrs,
			      reqdesc->xdr_decode_func, (caddr_t) arg_nfs)) {
		LogInfo(COMPONENT_DISPATCH,
