...
void Create_RDMA(protos prot)
{
	...
	LogEvent(COMPONENT_RDMA,
		 "RDMA: NFS Ganesha is now LISTENING for RPC/RDMA "
		 "connections on port %s (xprt %p)",
...
void Create_SVCXPRTs(void)
{
	...
	if (rdma) {
		...
		LogDebug(COMPONENT_RDMA,
			 "RDMA: NFS/RDMA compiled in but NOT ENABLED,"
			 "skipping SVCXPRT creation");
	}
#endif /* _USE_NFS_RDMA */
}
...
void nfs_Init_svc(void)
{
	...
	LogEvent(COMPONENT_RDMA,
		 "RDMA: svc_params configured: rdma_port=%d "
		 "max_rdma_connections=%d rdma_credits=%d",
