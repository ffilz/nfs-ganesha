...
void Create_RDMA(protos prot)
{
	...
	if (!netconfig_rdma) {
		...
		LogFullDebug(COMPONENT_RDMA,
			     "Create_RDMA: Retrieved netconfig_rdma=%p",
			     netconfig_rdma);
	}
	...
	if (netconfig_rdma && netconfig_rdma->nc_netid) {
		...
		LogEvent(COMPONENT_RDMA,
			 "Create_RDMA: Set xp_netid=%s on transport",
			 tcp_xprt[prot]->xp_netid);
	} else {
...
static bool __Register_program(protos prot, int vers)
{
	...
	/* RDMA protocol uses its own transport, skip regular TCP registration */
#ifdef _USE_NFS_RDMA
	if (prot != P_NFS_RDMA)
#endif
	{
	...
	if (prot == P_NFS_RDMA && netconfig_rdma) {
		LogEvent(COMPONENT_RDMA,
			 ...
			 prot, vers,
			 netconfig_rdma->nc_netid ? netconfig_rdma->nc_netid : "NULL");
		...
		LogEvent(COMPONENT_RDMA,
			 "RDMA Registration SUCCESS for %s V%d", tags[prot], (int)vers);
		...
		if (!v6disabled && netconfig_rdma6 && tcp_xprt[prot]) {
			struct sockaddr *sa = (struct sockaddr *)tcp_xprt[prot]->xp_local.nb.buf;
			if (sa && tcp_xprt[prot]->xp_local.nb.len > 0 && sa->sa_family == AF_INET6) {
				LogInfo(COMPONENT_RDMA, "Registering %s V%d/RDMAv6",
					tags[prot], (int)vers);
				if (!RDMA_REGISTER(prot, vers, netconfig_rdma6)) {
					LogMajor(COMPONENT_RDMA,
...
void nfs_Init_netconfig(void)
{
	...
	if (NFS_options & CORE_OPTION_NFS_RDMA) {
		...
		LogEvent(COMPONENT_RDMA,
			 "nfs_Init_netconfig: Getting RDMA netconfig entry");
		netconfig_rdma = (struct netconfig *)getnetconfigent("rdma");
		...
		if (netconfig_rdma == NULL)
			...
			LogEvent(COMPONENT_RDMA,
				 "nfs_Init_netconfig: RDMA netconfig found - nc_netid=%s",
				 netconfig_rdma->nc_netid);

		if (!v6disabled) {
			netconfig_rdma6 = (struct netconfig *)getnetconfigent("rdma6");
