...
#define NFS_pcpco NFS_pcp.core_options
#define NFS_pcpp NFS_pcp.program
...
{
	if ((NFS_pcpco & CORE_OPTION_NFSV3) != 0) {
		unregister(NFS_pcpp[P_NFS], NFS_V2, NFS_V4);
		unregister(NFS_pcpp[P_MNT], MOUNT_V1, MOUNT_V3);
	} else {
		unregister(NFS_pcpp[P_NFS], NFS_V4, NFS_V4);
	}
#ifdef _USE_NLM
	if (nfs_param.core_param.enable_NLM)
		unregister(NFS_pcpp[P_NLM], 1, NLM4_VERS);
#endif /* _USE_NLM */
	if (nfs_param.core_param.enable_RQUOTA) {
		unregister(NFS_pcpp[P_RQUOTA], RQUOTAVERS, EXT_RQUOTAVERS);
	}
}

static inline bool nfs_protocol_enabled(protos p)
{
	bool nfsv3 = NFS_pcpco & CORE_OPTION_NFSV3;
...
void nfs_Init_svc(void)
{
...
#ifdef RPC_VSOCK
	vsock = NFS_pcpco & CORE_OPTION_NFS_VSOCK;
#endif
	...
	if ((NFS_pcpco & CORE_OPTION_ALL_NFS_VERS) != 0) {
...
#endif /* _USE_NLM */
	if (nfs_param.core_param.enable_RQUOTA &&
	    (NFS_pcpco & (CORE_OPTION_NFSV3 | CORE_OPTION_NFSV4))) {
...
static bool is_rpc_call_valid(struct svc_req *req)
{
	...
	if (req->rq_msg.cb_prog == NFS_pcpp[P_NFS]) {
		if (req->rq_msg.cb_vers == NFS_V3) {
#ifdef _USE_NFS3
			if ((NFS_pcpco & CORE_OPTION_NFSV3)
			...
			if ((NFS_pcpco & CORE_OPTION_NFSV4)
			...
			if ((NFS_pcpco & CORE_OPTION_NFSV3) != 0)
				lo_vers = NFS_V3;
#endif /* _USE_NFS3 */
			if ((NFS_pcpco & CORE_OPTION_NFSV4) != 0)
				hi_vers = NFS_V4;
			goto progvers_err;
		}
#ifdef _USE_NLM
	} else if (req->rq_msg.cb_prog == NFS_pcpp[P_NLM]
	...
	} else if (req->rq_msg.cb_prog == NFS_pcpp[P_RQUOTA]) {
