			     ...
			     struct qos_block_config *qos_block)
{
	...
	if (!client_addr->qos_class)
		node = allocate_qos_class(QOS_CLIENT);
	else
		node = client_addr->qos_class;
...
/**
 * @brief function to put the qos_config_lock which has been acquired
 *      by only get_client_qos() or get_export_qos()
 */
 ...
 * @param [in] locked indicates lock need to be acquired if ret !NULL,
 *              which should be released later by the caller by
 *              calling put_qos_config_lock()
 *
 * @return qos_class_t pointer to QoS client class, or NULL if not found
 */
qos_class_t *get_client_qos(struct gsh_client *gsh_client, bool locked)
{
 ...
 */
qos_class_t *get_export_qos(struct gsh_export *gsh_export, bool locked)
{
	qos_class_t *qos_class = NULL;
				    ...
				    compound_data_t *data, unsigned int op_type)
{
	...
	if (export->bw_enabled || export->token_enabled) {
		...
		ret = qos_check_pepc(export, rsize, op_type, caller_data, data);
	}
	return ret;
}
...
{
	qos_class_t *export = get_export_qos(gsh_export, false);
	struct glist_head *glist;
	qos_class_t *client;
...
{
	qos_class_t *client = get_client_qos(cl, false);
...
{
	qos_class_t *client = get_client_qos(gsh_client, false);
