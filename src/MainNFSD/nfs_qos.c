...
{
	qos_class_t *node =
		gsh_calloc(1, sizeof(qos_class_t), MEM_COMP_SESSION);
...
static qos_client_entry_t *alloc_clientdetails_pe(compound_data_t *data)
{
	...
	new_entry = gsh_calloc(1, sizeof(qos_client_entry_t), MEM_COMP_SESSION);
						      ...
						      int ratecontrol)
{
	...
	qos_cb_args =
		gsh_calloc(1, sizeof(struct qos_op_cb_arg), MEM_COMP_SESSION);
						...
						void *args)
{
	timer_entry_t *new_entry =
		gsh_calloc(1, sizeof(timer_entry_t), MEM_COMP_SESSION);
...
static inline void resume_timer_entry(timer_entry_t *entry)
{
	...
	gsh_free(entry, MEM_COMP_SESSION);
}
...
static inline void pepc_free_client_list(struct glist_head *head)
{
	...
	glist_for_each_safe(glist, glistn, head) {
		...
		gsh_free(qos_class, MEM_COMP_SESSION);
	}
}
...
bool pepc_per_export_free_mem_iter(struct gsh_export *gsh_export, void *state)
{
	...
	if (sub_qos_class) {
		...
		gsh_free(sub_qos_class, MEM_COMP_SESSION);
	} else {
...
void qos_free_mem(void *gsh_ptr, qos_class_type_t class_type)
{
	...
	case QOS_PER_EXPORT_ENABLED:
		if (class_type == QOS_EXPORT) {
			...
			gsh_free(export->qos_class, MEM_COMP_SESSION);
			if (export->qos_block)
				gsh_free(export->qos_block, MEM_COMP_SESSION);
	...
	case QOS_PER_CLIENT_ENABLED:
		if (class_type == QOS_CLIENT && client->qos_class) {
			...
			gsh_free(client->qos_class, MEM_COMP_SESSION);
				      ...
				      qos_class_t *qos_class)
{
	...
	glist_for_each_safe(glist, glistn, clients) {
		...
		if (!client->num_ios_waiting) {
			...
			gsh_free(client, MEM_COMP_SESSION);
			if (epd) {
