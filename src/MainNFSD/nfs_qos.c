...
#define QOS_INITIALIZED		1 << 0
#define QOS_THREAD_RUNNABLE	1 << 1
uint8_t qos_bits;
 ...
 * @return True  always true so to continue the iterations.
 */
bool pc_token_control_iter(struct gsh_client *cl, void *state)
{
 ...
 * @return true to continue iteration.
 */
bool pc_io_control_iter(struct gsh_client *cl, void *state)
{
	qos_class_t *qos_class = get_client_qos(cl);
...
static void *qos_thread_func(void *arg)
{
	...
	while (atomic_fetch_uint8_t(&qos_bits) & QOS_THREAD_RUNNABLE) {
		resume_io(op_type);
...
static void qos_thread_init(void)
{
	...
	if (!g_qos_config->enable_qos)
		return;
	PTHREAD_MUTEX_lock(&g_qos_iopath_lock);
	if (!(atomic_fetch_uint8_t(&qos_bits) & QOS_INITIALIZED)) {
		atomic_set_uint8_t_bits(&qos_bits, QOS_INITIALIZED);
		atomic_set_uint8_t_bits(&qos_bits, QOS_THREAD_RUNNABLE);
...
void shutdown_qos(void)
{
	...
	PTHREAD_MUTEX_lock(&g_qos_iopath_lock);
	if ((!(atomic_fetch_uint8_t(&qos_bits) & QOS_INITIALIZED)) &&
	    (!(atomic_fetch_uint8_t(&qos_bits) & QOS_THREAD_RUNNABLE))) {
		PTHREAD_MUTEX_unlock(&g_qos_iopath_lock);
		return;
	}

	/* Signal threads to exit */
	atomic_clear_uint8_t_bits(&qos_bits, QOS_THREAD_RUNNABLE);
	...
	atomic_clear_uint8_t_bits(&qos_bits, QOS_INITIALIZED);
