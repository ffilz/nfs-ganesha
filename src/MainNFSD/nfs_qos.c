 ...
 *			    i.e allocate new qos_client_t and add in share list
				       ...
				       unsigned int qos_class_type)
{
	...
	if (qos_class_type == QOS_EXPORT) {
		...
		token_enabled = export->token_enabled;
		client_list = &export->client_entries;
		num_ios_waiting = &export->num_ios_waiting;
		...
		client_list = &client->client_entries;
		num_ios_waiting = &client->num_ios_waiting;
...
{
	pthread_mutex_t *qos_class_lock;

	if (qos_class_type == QOS_EXPORT)
		qos_class_lock = &(((qos_export_t *)qos_class)->lock);
	else
		qos_class_lock = &(((qos_client_t *)qos_class)->lock);

	PTHREAD_MUTEX_lock(qos_class_lock);
	...
	PTHREAD_MUTEX_unlock(qos_class_lock);
}
...
bool pepc_per_export_free_mem_cb(struct gsh_export *gsh_export, void *state)
{
	...
	qos_client_t *c_qos_class = NULL;

	/* list is not populated */
	if (e_qos_class == NULL || glist_empty(&e_qos_class->clients))
		return true;
	...
	case QOS_PEREXPORT_PERCLIENT_ENABLED:
		if (qos_class_type == QOS_EXPORT) {
			...
			glist_for_each(glist, &export->clients) {
				c_qos_class =
					glist_entry(glist, qos_client_t, glist);
				qos_drain_ios(c_qos_class, QOS_CLIENT);
			}
...
void copy_gsh_qos_conf(struct gsh_export *dest, struct gsh_export *src)
{
	...
	case QOS_PEREXPORT_PERCLIENT_ENABLED:
		...
		glist_for_each(glist, &qos_export->clients) {
			qos_client = glist_entry(glist, qos_client_t, glist);
			 ...
			 struct qos_block_config *qos_block)
{
	...
	if (gsh_export->qos_class == NULL) {
		...
		glist_init(&node->read_bucket.io_waitlist_qos_bc);
		glist_init(&node->write_bucket.io_waitlist_qos_bc);
		glist_init(&node->read_bucket.io_waitlist_qos_iops);
		glist_init(&node->write_bucket.io_waitlist_qos_iops);
 ...
 */
qos_client_t *allocate_client(sockaddr_t *client_addr,
			      struct qos_block_config *qos_block)
{
 ...
 */
qos_client_t *pepc_get_client_from_list(struct glist_head *head,
					sockaddr_t *client_addr)
{
	...
	glist_for_each(glist, head) {
		client = glist_entry(glist, qos_client_t, glist);
 ...
 */
qos_client_t *pepc_alloc_get_client(qos_export_t *export,
				    sockaddr_t *client_addr)
{
	/* Client Not Found */
	qos_client_t *client;
	...
	client = pepc_get_client_from_list(&export->clients, client_addr);
	if (client == NULL) {
		client = allocate_client(client_addr,
					 op_ctx->ctx_export->qos_block);
		glist_add_tail(&(export->clients), &client->glist);
	}
			 ...
			 struct gsh_client *client)
{
	...
	client->qos_class = allocate_client(&client->cl_addrbuf, lqos_block);
}
				  ...
				  unsigned int class_type)
{
	...
	return qos_process_bw(bucket, request_size, caller_data, op_type);
}
				      ...
				      unsigned int class_type)
{
	...
	qos_bw_bucket_deffer_task(bucket, caller_data, size, timeout, op_type);
}
			 ...
			 compound_data_t *data, unsigned int class_type)
{
	...
	ret = qos_control_bw(qos_class, request_size, op_type, caller_data,
			     QOS_EXPORT);
out:
 ...
 */
static bool qos_check_pc(void *class_ptr, uint64_t rsize, unsigned int op_type,
			 void *caller_data, compound_data_t *data,
			 unsigned int class_type)
{
	...
	qos_consume_token(qos_class, rsize, op_type, QOS_CLIENT);
			   ...
			   compound_data_t *data, unsigned int class_type)
{
	...
	qos_client_t *c_qos_class =
		pepc_alloc_get_client(e_qos_class, &op_ctx->client->cl_addrbuf);
			    ...
			    compound_data_t *data, unsigned int op_type)
{
	...
	return qos_check_pe(op_ctx->ctx_export->qos_class, size, op_type,
			    ...
			    compound_data_t *data, unsigned int op_type)
{
	...
	return qos_check_pc(op_ctx->client->qos_class, size, op_type,
			    caller_data, data, QOS_CLIENT);
}
					   ...
					   unsigned int op_type)
{
	...
	if (client->num_ios_waiting >= 5 && !client->epoll_disabled) {
...
get_and_insert_client_details(struct glist_head *head, compound_data_t *data)
{
	...
	glist_for_each(glist, head) {
		...
		if (current->client_addr == &op_ctx->client->cl_addrbuf)
			return current;
	}
				      ...
				      unsigned int class_type)
{
	...
	glist_for_each_safe(glist, glistn, clients) {
		...
		counter2 = (class_type == QOS_EXPORT)
		...
		if (!client->num_ios_waiting) {
			...
			LogDebug(COMPONENT_QOS,
				 "ResClntSock Cid:%p Xprt:%p epd:%d xprt:%p",
				 client->client_addr, client->rq_xprt,
...
{
	qos_bucket_t *bucket = qos_get_bw_bucket(export, QOS_EXPORT, op_type);
...
{
	qos_bucket_t *bucket = qos_get_bw_bucket(client, QOS_CLIENT, op_type);
					 ...
					 unsigned int op_type)
{
	...
	if (sbucket == NULL || (ctime < sbucket->bw_ldct))
		return;

	PTHREAD_MUTEX_lock(&sbucket->lock);
	glist_for_each(glist, &export->clients) {
		client = glist_entry(glist, qos_client_t, glist);
		cbucket = qos_get_bw_bucket(client, QOS_CLIENT, op_type);

		if (cbucket == NULL || (ctime <= cbucket->bw_ldct))
			continue;
...
uint32_t get_export_client_count(qos_export_t *qos_class)
{
	uint32_t count = 0;
	struct glist_head *glist;
...
unsigned int QoS_Process_iops(compound_data_t *data)
{
	...
	if (!g_qos_config->enable_qos || !g_qos_config->enable_iops_control ||
	    (op_ctx->ctx_export == NULL) || !op_ctx->ctx_export->is_mounted)
		return ret;
...
static inline void resume_iops_pe(qos_export_t *export, unsigned int op_type)
{
	qos_bucket_t *bucket = qos_get_iops_bucket(export, QOS_EXPORT, op_type);
...
static inline void resume_iops_pc(qos_client_t *client, unsigned int op_type)
{
	qos_bucket_t *bucket = qos_get_iops_bucket(client, QOS_CLIENT, op_type);
					...
					unsigned int op_type)
{
	...
	if (sbucket == NULL || ctime < sbucket->iops_ldct)
		return;

	PTHREAD_MUTEX_lock(&sbucket->lock);
	glist_for_each(glist, &export->clients) {
		client = glist_entry(glist, qos_client_t, glist);
		cbucket = qos_get_iops_bucket(client, QOS_CLIENT, op_type);

		if (cbucket == NULL || ctime <= cbucket->iops_ldct)
			continue;
