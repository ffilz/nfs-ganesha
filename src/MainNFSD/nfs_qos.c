...
qos_client_t *pepc_remove_client_from_list(struct glist_head *head,
...
bool pepc_per_export_free_mem_cb(struct gsh_export *gsh_export, void *state)
{
	...
	if (e_qos_class == NULL || glist_empty(&e_qos_class->clients) == true)
		return true;
...
/**
 * Function to Allocate, Init and Add new client in provided list.
 * Caller should hold export->lock.
 *
 * @param [in] head Pointer to the client list.
 * @param [in] client_addr Address of the client.
 * @param [in] Pointer to qos_block , which contains the applicable values.
 * @return Pointer to the newly allocated client.
 */
qos_client_t *pepc_alloc_init_add_client(struct glist_head *head,
					 sockaddr_t *client_addr,
					 struct qos_block_config *qos_block)
{
	...
	if (new_node)
		glist_add_tail(head, &new_node->glist);
 ...
 * Caller should hold export->lock in new allocation, else not required.
 *
 * @param [in] head Pointer to the client list.
 * @param [in] client_addr Address of the client.
...
/**
 * Function to remove a specific client list.
 *
 * @param [in] head Pointer to the client list.
 * @param [in] client_addr Address of the client.
 * @return Pointer to retrieved qos_client_t structure, or NULL if not found.
