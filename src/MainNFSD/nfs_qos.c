				  ...
				  compound_data_t *data)
{
	...
	if (!export_token_available) {
		qos_token_exausted_defer_task(e_qos_class, caller_data, data,
					       op_type);
		PTHREAD_MUTEX_unlock(&e_qos_class->lock);
		return QOS_TASK_ASYNC_SCHEDULED;
	} else if (!client_token_available) {
		qos_token_exausted_defer_task(c_qos_class, caller_data, data,
					       op_type);
		PTHREAD_MUTEX_unlock(&e_qos_class->lock);
		return QOS_TASK_ASYNC_SCHEDULED;
	} else {
		...
		if (c_qos_class->bw_enabled) {
			qos_bw_defer_task(c_qos_class, caller_data, rsize,
...
{
	qos_class_t *client = get_client_qos(gsh_client);
