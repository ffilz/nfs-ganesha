				      ...
				      const char *str)
{
	if (qos_class_type == QOS_SHARE) {
		...
		LogFullDebug(COMPONENT_QOS,
			     "%s PER_SHARE SI:%d s_wio:%d"
			     " bw_enabled:%d, token_enabled:%d"
			     " c_rw_bw:%d, c_rw_token:%d",
		...
		LogFullDebug(COMPONENT_QOS,
			     "%s PER_CLIENT CI:%p s_wio:%d"
			     " bw_enabled:%d, token_enabled:%d c_rw_bw:%d,"
			     " c_rw_token:%d",
			...
			struct qos_block_config *qos_block)
{
	...
	/*  Condition indicates this is new export or
					   ...
					   unsigned int op_type)
{
	...
	LogFullDebug(COMPONENT_QOS,
		     "Timer added: %p gio_waiters:%d client_io_waitlists:%d"
		     " CI:%p CT:%ld TO:%ld",
				       ...
				       uint64_t current_time, const char *str)
{
	...
	LogFullDebug(COMPONENT_QOS,
		     "%s PS SI:%d s_wio:%d op_type:%s"
		     " lct:%ld pct:%ld sb_io:%d sbw_ldct:%ld",
		     str, share->share_id, share->num_ios_waiting,
		     (op_type == QOS_READ) ? "QOS_READ" : "QOS_WRITE",
		     get_time_in_usec(), current_time, sbucket->num_ios_waiting,
		     sbucket->bw_ldct);
				       ...
				       uint64_t current_time, const char *str)
{
	...
	LogFullDebug(COMPONENT_QOS,
		     "%s PC CID:%p c_wio:%d op_type:%s"
		     " lct:%ld pct:%ld cb_io:%d cbw_ldct:%ld",
		     str, client->client_addr, client->num_ios_waiting,
		     (op_type == QOS_READ) ? "QOS_READ" : "QOS_WRITE",
		     get_time_in_usec(), current_time, cbucket->num_ios_waiting,
		     cbucket->bw_ldct);
					 ...
					 uint64_t current_time, const char *str)
{
	...
	LogFullDebug(COMPONENT_QOS,
		     "%s PSPC SI:%d CID:%p s_wio:%d c_wio:%d"
		     " op_type:%s lct:%ld pct:%ld sb_io:%d "
