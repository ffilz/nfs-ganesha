...
static void qos_token_exausted_deffer_task(qos_class_t *qos_class,
					   void *caller_data,
...
static void insert_timer_entry(struct glist_head *head,
...
static inline uint64_t get_time_future_useconds(uint64_t current,
						...
						uint64_t mseconds,
						uint64_t useconds);
static void setNode_pc(qos_class_t *node, struct gsh_client *client_addr,
...
static inline void resume_ops_io(qos_class_t *export, unsigned int op_type);
static inline void pepc_reschedule_iops(qos_class_t *export,
					unsigned int op_type);
static inline void resume_iops_pepc(qos_class_t *export, unsigned int op_type);
...
#define QOS_PRINT_EXPORT(str, gsh_export)				\
	do {								\
		...
		LogDebug(COMPONENT_QOS, "%s SI: %d path: %s pseudo: %s",\
				str, gsh_export->export_id,		\
				TMP_FULLPATH(&tmp),			\
				TMP_PSEUDOPATH(&tmp));			\
...
#define QOS_PRINT_CLASS(str, class)					\
	do {								\
		if (class->type == QOS_EXPORT) {			\
			QOS_PRINT_EXPORT(str, class->export_addr);	\
		} else {						\
			QOS_PRINT_CLIENT(str, class->client_addr);	\
		}							\
	} while (0)

/* q_qos_lock is global lock which is used for below senarios.
 * Allocation of new QOS structure in IO path.
 * 1. PerExport case      : Global qos values are there in config,
 *			    but export level config not present,
 *			    so populate value at runtime for this share.
 ...
 *
 * @param [in] callback Callback function's string representation is needed.
 *
...
bool pepc_per_export_free_mem_cb(struct gsh_export *gsh_export, void *state)
{
	...
	qos_class_t *e_qos_class = gsh_export->qos_class;
	qos_class_t *c_qos_class = NULL;

	/* list is not populated */
	if (!e_qos_class || glist_empty(&e_qos_class->clients))
		return true;

	PTHREAD_MUTEX_lock(&e_qos_class->lock);
	c_qos_class = pepc_get_client_from_list(&(e_qos_class->clients),
						(struct gsh_client *)state);
	if (c_qos_class)
		glist_del(&c_qos_class->clients);
	PTHREAD_MUTEX_unlock(&e_qos_class->lock);

	if (c_qos_class) {
		QOS_PRINT_EXPORT("freeing client for export", gsh_export);
		qos_drain_ios(c_qos_class);
...
void qos_free_mem(void *gsh_ptr, unsigned int qos_class_type)
{
	...
	case QOS_PEREXPORT_PERCLIENT_ENABLED:
		if (qos_class_type == QOS_EXPORT) {
			qos_class_t *e_qos_class = export->qos_class;
			qos_class_t *c_qos_class;
			struct glist_head *glist;

			/* releasing waiting io's */
			PTHREAD_MUTEX_lock(&e_qos_class->lock);
			glist_for_each(glist, &export->clients) {
				c_qos_class = glist_entry(glist, qos_class_t,
							 clients);
				qos_drain_ios(c_qos_class);
			}
			pepc_free_client_list(&e_qos_class->clients);
			PTHREAD_MUTEX_unlock(&e_qos_class->lock);
			qos_drain_ios(e_qos_class);
			gsh_free(export->qos_class);
			if (export->qos_block)
				gsh_free(export->qos_block);
			export->qos_class = NULL;
...
void copy_gsh_qos_conf(struct gsh_export *dest, struct gsh_export *src)
{
	...
	case QOS_PEREXPORT_PERCLIENT_ENABLED:
		...
		qos_class_t *qos_export = NULL;
		qos_class_t *qos_client = NULL;
		struct glist_head *glist;
		...
		glist_for_each(glist, &qos_export->clients) {
			qos_client = glist_entry(glist, qos_class_t, clients);
			PTHREAD_MUTEX_lock(&qos_client->lock);
			setNode_pc(qos_client, qos_client->client_addr,
				   dest->qos_block);
			PTHREAD_MUTEX_unlock(&qos_client->lock);
		}
...
{
	const char *string = "wio: %d bw: %ld bw_ldct: %ld mat: %ld "
				"tc: %ld trt: %ld ltct: %ld max_iops: %ld "
				"iops_ldct: %ld iops_consumed: %ld";
	LogDebug(COMPONENT_QOS, string,
 ...
 */
static inline void print_class_values(qos_class_t *class,
				      const char *str)
{
	const char *string = "wio: %d bw_e: %d t_e: %d iops_e: %d "
				"c_bw: %d c_t: %d c_iops: %d";

	QOS_PRINT_CLASS(str, class);

	LogDebug(COMPONENT_QOS, string, class->num_ios_waiting,
			class->bw_enabled, class->token_enabled,
			class->iops_enabled, class->combined_rw_bw_control,
 ...
 * g_qos_lock needs to be held if called from IO path.
 *
 * @param [in] node Pointer to qos_class_t structure representing the export.
 * @param [in] export_addr->export_id ID of the export.
 ...
 * g_qos_lock needs to be held, if getting called from IO path only.
 ...
 * @return Pointer to the newly allocated qos_class_t structure.
 */
qos_class_t *allocate_client(struct gsh_client *client_addr,
			      struct qos_block_config *qos_block)
{
	qos_class_t *node = allocate_qos_class(QOS_CLIENT);
	setNode_pc(node, client_addr, qos_block);
	return node;
}
...
qos_class_t *pepc_get_client_from_list(struct glist_head *head,
					struct gsh_client *client_addr)
{
	struct glist_head *glist;
	qos_class_t *client;

	glist_for_each(glist, head) {
		client = glist_entry(glist, qos_class_t, clients);
 ...
 */
qos_class_t *pepc_alloc_get_client(qos_class_t *export,
				    struct gsh_client *client_addr)
{
	/* Client Not Found */
	qos_class_t *c_qos_class;
	...
	c_qos_class = pepc_get_client_from_list(&export->clients, client_addr);
	if (!c_qos_class) {
		c_qos_class = allocate_client(client_addr,
					 op_ctx->ctx_export->qos_block);
		glist_add_tail(&(export->clients), &c_qos_class->clients);
	}
 ...
 * Caller needs to hold the g_qos_lock
 *
 * @param [in] qos_block Pointer to the qos_block_config_t structure containing
 ...
 * @param [in] op_type Type of the operation (read/write).
 */
static inline void qos_bw_deffer_task(qos_class_t *qos_class, void *caller_data,
				      ...
				      unsigned int op_type)
{
	...
	qos_bw_bucket_deffer_task(bucket, caller_data, rsize, timeout, op_type);
}
...
static inline bool qos_check_pepc(void *class_ptr, uint64_t rsize,
			   ...
			   compound_data_t *data)
{
	qos_class_t *e_qos_class = class_ptr;
	qos_class_t *c_qos_class =
		pepc_alloc_get_client(e_qos_class, op_ctx->client);

	int export_token_available =
		qos_check_token_availability(e_qos_class, rsize, op_type);
	int client_token_available =
		qos_check_token_availability(c_qos_class, rsize, op_type);
	...
	if (!export_token_available) {
		...
		qos_consume_token(e_qos_class, rsize, op_type);
		qos_consume_token(c_qos_class, rsize, op_type);
		if (c_qos_class->bw_enabled) {
			qos_bw_deffer_task(c_qos_class, caller_data, rsize,
			    ...
			    compound_data_t *data, unsigned int op_type)
{
	...
	return qos_check_pe_pc(op_ctx->ctx_export->qos_class, size, op_type,
			    caller_data, data);
}
 ...
 * @return : QOS_TASK_ASYNC_NOT_SCHEDULED/ QOS_TASK_ASYNC_SCHEDULED
 */
static inline bool QoS_Process_pc(unsigned int size, void *caller_data,
			    compound_data_t *data, unsigned int op_type)
{
	...
	return qos_check_pe_pc(op_ctx->client->qos_class, size, op_type,
			    caller_data, data);
}
 ...
 * @return : QOS_TASK_ASYNC_NOT_SCHEDULED/ QOS_TASK_ASYNC_SCHEDULED
 */
static inline bool QoS_Process_pepc(unsigned int size, void *caller_data,
			      compound_data_t *data, unsigned int op_type)
{
	qos_class_t *export = op_ctx->ctx_export->qos_class;
	int ret = QOS_TASK_ASYNC_NOT_SCHEDULED;
	...
	if (export->bw_enabled || export->token_enabled) {
		...
		ret = qos_check_pepc(export, size, op_type, caller_data, data);
	}
	return ret;
}
					...
					unsigned int op_type)
{
	...
	if ((bucket->tokens_consumed >= bucket->max_available_tokens) &&
		...
		LogDebug(COMPONENT_QOS, "CT: %ld LCT: %ld TRT: %ld cal: %ld",
			ltime, bucket->token_ldct, bucket->tokens_renew_time,
 ...
 */
static inline bool refresh_tokens(qos_class_t *export_entry)
{
	/* since the io waitlist queue are same for read and write ios,
	 * check for both and then return */
	return (refresh_bucket_token(export_entry, QOS_READ) ||
		refresh_bucket_token(export_entry, QOS_WRITE));
}
...
static qos_client_entry_t *alloc_clientdetails_pe(compound_data_t *data)
{
	...
	new_entry->client_addr = op_ctx->client;
	new_entry->data = data;
...
get_and_insert_client_details(struct glist_head *head, compound_data_t *data)
{
	...
	if (glist_empty(head)) {
		...
		glist_add(head, &new_entry->glist);
		return new_entry;
	}

	glist_for_each(glist, head) {
		current = glist_entry(glist, qos_client_entry_t, glist);
		if (current->client_addr == op_ctx->client)
			return current;
	}
 ...
 */
static void resume_timer_entry(timer_entry_t *entry)
{
 ...
 */
inline void release_wait_ios(struct glist_head *head, unsigned int *counter1,
			     unsigned int *counter2)
{
	struct glist_head *glist, *glistn;
	timer_entry_t *entry;
 ...
 */
void execute_qos_expired_timers(struct glist_head *head, unsigned int *counter1,
				unsigned int *counter2)
{
	uint64_t ctime = get_time_in_usec();
	struct glist_head *glist, *glistn;
				      ...
				      qos_class_t *qos_class)
{
	...
	struct gsh_client *client_addr = NULL;
	...
	glist_for_each_safe(glist, glistn, clients) {
		...
		LogFullDebug(COMPONENT_QOS, "CI: %p CWIO's: %d ",
		...
		if (!client->num_ios_waiting) {
			...
			QOS_PRINT_CLIENT("resume socket ", client_addr);
...
static inline void refresh_qos_token_by_class(qos_class_t *qos_class)
{
	...
	QOS_PRINT_EXPORT(__func__, qos_class->export_addr);
	LogDebug(COMPONENT_QOS, " TR: %d WIO's: %d",
			tokens_refreshed, qos_class->num_ios_waiting);
	PTHREAD_MUTEX_lock(&(qos_class->lock));
...
{
	qos_class_t *export = get_export_qos(gsh_export);
	struct glist_head *glist;
	qos_class_t *client;

	if (export && export->token_enabled) {
		QOS_PRINT_EXPORT(__func__, gsh_export);
		refresh_qos_token_by_class(export);

		/* Check all clients in export for token exhaustion
		 * and replenishment */
		glist_for_each(glist, &export->clients) {
			client = glist_entry(glist, qos_class_t, clients);
			refresh_qos_token_by_class(client);
		}
	}
	/* Continue iteration */
	return true;
...
{
	qos_class_t *client = get_client_qos(cl);
 ...
 * @param [in] op_type Type of the operation (read/write)
 */
static inline void resume_bw_io_pepc(qos_class_t *export, unsigned int op_type)
{
	qos_bucket_t *bucket = qos_get_bw_bucket(export, op_type);
	uint64_t ctime = get_time_in_usec();
	int check_delay =
 ...
 * @param [in] op_type Type of the operation (read/write).
 */
static inline void pepc_reschedule_bw_io(qos_class_t *export,
					 unsigned int op_type)
{
	uint64_t ctime = get_time_in_usec();
	qos_class_t *client;
	qos_bucket_t *sbucket = qos_get_bw_bucket(export, op_type);
	...
	glist_for_each(glist, &export->clients) {
		client = glist_entry(glist, qos_class_t, clients);
		cbucket = qos_get_bw_bucket(client, op_type);
...
bool ps_io_control_cb(struct gsh_export *gsh_export, void *state)
{
	...
	if (export && export->bw_enabled)
		resume_bw_io(export, *(unsigned int *)state);

	if (export && export->iops_enabled)
		resume_ops_io(export, *(unsigned int *)state);
...
bool pc_io_control_cb(struct gsh_client *cl, void *state)
{
	...
	if (client && client->bw_enabled)
		resume_bw_io(client, *(unsigned int *)state);

	if (client && client->iops_enabled)
		resume_ops_io(client, *(unsigned int *)state);
...
int QoS_Process_iops_pe(compound_data_t *data, uint32_t op_type)
{
	...
	bucket = qos_get_iops_bucket(export, op_type);
	if (!bucket) {
...
{
	qos_class_t *client = op_ctx->client->qos_class;
	qos_bucket_t *bucket = NULL;
	int ret = 0;
	...
	if (!client->iops_enabled)
		return QOS_TASK_ASYNC_NOT_SCHEDULED;

	bucket = qos_get_iops_bucket(client, op_type);
	if (!bucket) {
		ret = QOS_TASK_ASYNC_NOT_SCHEDULED;
		goto out;
	}
...
{
	qos_class_t *export = op_ctx->ctx_export->qos_class;
	struct gsh_client *client_addr = op_ctx->client;
	...
	if (export->iops_enabled) {
		qos_class_t *client = NULL;
		qos_bucket_t *bucket = NULL;

		client = pepc_alloc_get_client(export, client_addr);
		bucket = qos_get_iops_bucket(client, op_type);
					...
					unsigned int op_type)
{
	...
	struct glist_head *glist;
	struct glist_head *io_list = NULL;
	qos_bucket_t *sbucket = qos_get_iops_bucket(export, op_type);
	...
	glist_for_each(glist, &export->clients) {
		client = glist_entry(glist, qos_class_t, clients);
		cbucket = qos_get_iops_bucket(client, op_type);
...
static inline void resume_iops_pepc(qos_class_t *export, unsigned int op_type)
{
	qos_bucket_t *bucket = qos_get_iops_bucket(export, op_type);
	uint64_t ctime = get_time_in_usec();
	int check_delay =
 ...
 * @return true to continue iteration.
 */
bool pc_stop_cb(struct gsh_client *cl, void *state)
{
	qos_class_t *client = get_client_qos(cl);
