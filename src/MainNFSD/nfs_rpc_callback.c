...
{
	int code;
	char *uaddr2 = gsh_strdup(uaddr, MEM_COMP_CLIENT);
	char *dot, *p1, *p2;
	uint16_t port;
...
out:
	...
	gsh_free(uaddr2, MEM_COMP_CLIENT);
}
 ...
 */
static void nfs_rpc_call_free(struct clnt_req *cc, size_t unused, uint8_t comp)
{
