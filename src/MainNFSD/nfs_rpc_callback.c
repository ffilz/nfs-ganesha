...
enum clnt_stat nfs_rpc_call(rpc_call_t *call, uint32_t flags)
{
	...
	if (re_status == RPC_SUCCESS) {
		if (chan->type == RPC_CHAN_V40) {
			...
			/* v4.1 backchannels share the session fore-channel
			 * xprt and must use CLNT_CALL_BACK so replies
			 * are multiplexed by the TI-RPC event loop.
			 * v4.0 callback sockets are standalone and use
			 * CLNT_CALL_WAIT (see rpc_cb_null()).
			 */
