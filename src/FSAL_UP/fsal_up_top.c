				   ...
				   struct attrlist *attr, uint32_t flags)
{
	...
	struct attrlist *entry_attrs = get_attrs(entry->obj_handle);

	if (attr->expire_time_attr != 0)
		entry_attrs->expire_time_attr =
	...
	if (FSAL_TEST_MASK(attr->mask, ATTR_ACL)) {
		...
		nfs4_acl_release_entry(entry_attrs->acl,
				       &acl_status);
	...
	if (FSAL_TEST_MASK(attr->mask, ATTR_ATIME)
		...
		||
		(gsh_time_cmp
		 (&attr->atime, &entry_attrs->atime) == 1))) {
	...
	if (FSAL_TEST_MASK(attr->mask, ATTR_CTIME)
		...
		||
		(gsh_time_cmp
		 (&attr->ctime, &entry_attrs->ctime) == 1))) {
	...
	if (FSAL_TEST_MASK(attr->mask, ATTR_MTIME)
		...
		||
		(gsh_time_cmp
		 (&attr->mtime, &entry_attrs->mtime) == 1))) {
