...
struct compound_data {
	...
	struct COMPOUND4res_extended *cached_result; /* <NFv41: pointer to
	   the slot cached result, for reply request, we use data->cached_result
	   to save data->slot->cached_result.
	   Otherwise crash happens, for example:
		step1: thread1 receive a replay request, and found slot->cached_result
		       not null, and assign slot to data->slot and unlock the slot->lock.
		step2: thread2 receive a normal request, got slot->lock and
		       release_slot(slot), so slot->cached_result was set null.
		step3: thread1 in complete_op will assign *status with
