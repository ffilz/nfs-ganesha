					...
					const char *fun_name, const int line)
{
	...
	if (rc == 0) {
		DisplayLogComponentLevel(COMPONENT_RW_LOCK, filename, line,
					 fun_name, NIV_FULL_DEBUG,
					 "Acquired mutex %p (%s)", mtx,
					 mtx_name);
	} else if (rc == EBUSY) {
		DisplayLogComponentLevel(COMPONENT_RW_LOCK, filename, line,
					 fun_name, NIV_FULL_DEBUG,
					 "Busy mutex %p (%s)", mtx, mtx_name);
	} else {
		DisplayLogComponentLevel(COMPONENT_RW_LOCK, filename, line,
					 fun_name, NIV_CRIT,
					 "Error %d, acquiring mutex %p (%s)",
					 ...
					 const char *fun_name, const int line)
{
	...
	if (rc == 0) {
		...
		DisplayLogComponentLevel(
			...
			NIV_FULL_DEBUG,
			"Error %d, Wait cond %p, abstime: %ld.%09ld", rc, cond,
			abstime->tv_sec, abstime->tv_nsec);
	} else {
		DisplayLogComponentLevel(
			COMPONENT_RW_LOCK, filename, line, fun_name, NIV_CRIT,
			"Error %d, Wait cond %p, abstime: %ld.%09ld", rc, cond,
			abstime->tv_sec, abstime->tv_nsec);
		abort();
	}
