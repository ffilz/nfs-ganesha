	...
	do {								\
		...
		if (rc == 0) {						\
			...
			LogCrit(COMPONENT_RW_LOCK,			\
				...
				"at %s:%d", rc, _cond_attr, #_cond_attr,		\
				__FILE__, __LINE__);			\
			abort();					\
		}							\
	} while (0)
...
#define PTHREAD_COND_ATTR_set_mono_clock(_cond_attr)					\
	do {								\
		int rc;							\
									\
		rc = pthread_condattr_setclock(_cond_attr, CLOCK_MONOTONIC);			\
		if (rc == 0) {						\
			LogFullDebug(COMPONENT_RW_LOCK,			\
				     "Set cond attr to mono time %p (%s) at %s:%d",	\
				     _cond_attr, #_cond_attr,			\
				     __FILE__, __LINE__);		\
		} else {						\
			LogCrit(COMPONENT_RW_LOCK,			\
				"Error %d, Set cond attr to mono time %p (%s) "		\
				"at %s:%d", rc, _cond_attr, #_cond_attr,		\
				__FILE__, __LINE__);			\
			abort();					\
		}							\
