...
static inline size_t nfs3_sizeof_handle(struct file_handle_v3 *hdl)
{
	...
	hsize = offsetof(struct file_handle_v3, fsopaque) +
				(hdl->fs_v3len & FH3_LEN_MASK);
...
static inline short nfs3_FhandleToExportId(nfs_fh3 *pfh3)
{
	...
	if (mixed_endian) {
		...
		if (pfile_handle->fs_v3len & FH3_LITTLE_ENDIAN)
			pfile_handle->exportid =
					bswap_16(pfile_handle->exportid);
#else
		if (pfile_handle->fs_v3len & FH3_BIG_ENDIAN)
			pfile_handle->exportid =
					bswap_16(pfile_handle->exportid);
#endif
	}
