...
struct copy_offload_state {
	/* nfs4_Check_Stateid returns state_t * */
	 ...
	 */
	struct fsal_obj_handle *cos_src;
	struct fsal_obj_handle *cos_dst;
	state_t *cos_src_state;
	state_t *cos_dst_state;
	uint64_t cos_src_off;
	uint64_t cos_dst_off;
	struct gsh_export *cos_ctx_export;
	struct fsal_export *cos_fsal_export;

	/*
	 * Progress and completion entries.
	 */
	uint64_t cos_bytes_copied;
	uint64_t cos_total_count; /* immutable after create */
	nfsstat4 cos_status;
