...
#define UTF8_SCAN_STRICT  0x08	/* only validate utf8 if strict */
...
nfsstat4 nfs4_utf8string_scan(const utf8string *input, int scan)
{
	...
	/* Check if we are doing optional UTF-8 validation */
	/** @todo this will be modified by checking config in a subsequent
	 *        patch.
	 */
	if (scan & UTF8_SCAN_STRICT)
		scan |= UTF8_SCAN_CKUTF8;
