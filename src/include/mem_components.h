...
typedef enum {
	...
	MEM_COMP_DUP_REQ, 	/* Duplicate request cache (dupreq entries + TCP DRC) */
	...
	MEM_COMP_LIBNTIRPC, 	/* libntirpc (single bucket; libntirpc itself unchanged) */
	MEM_COMP_MDCACHE, 	/* MDCACHE entry/handle cache (directory cache is DIRENT) */
	MEM_COMP_PROTOCOL, 	/* NFS / NLM / 9P / NFS4 / NFS3, incl. nfs_res_t */
	...
	MEM_COMP_TRANSIENT, 	/* Short-lived scratch, freed within same function */
	MEM_COMP_MAX
} mem_components_t;
