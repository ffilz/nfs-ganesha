...
/*
 * gshMC[] and mem_components_t:
 *   Indices 0 through NTIRPC_MEM_COMP_MAX - 1  — libntirpc (add MEM_COMP_X =
 *   NTIRPC_MEM_COMP_X here for every entry in <rpc/types.h> before MAX).
 *   Indices starting at NTIRPC_MEM_COMP_MAX  — Ganesha-only; first slots are
 *   MEM_COMP_CONFIG, MEM_COMP_EXPORT, then the rest.
 */
typedef enum {
	MEM_COMP_LIBNTIRPC = NTIRPC_MEM_COMP_LIBNTIRPC,
	MEM_COMP_CONFIG = NTIRPC_MEM_COMP_MAX, /* ganesha.conf, parser */
