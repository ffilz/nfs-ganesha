 ...
 *  - ca_source_server_len  > 0  → inter-server
 *
 */
				   ...
				   state_t **state_out)
{
	...
	} else if (need_write && state_deleg_conflict(obj, true)) {
 ...
 */
static void fill_copy_response_sync(COPY4res *res, uint64_t sync_bytes_copied)
{
			      ...
			      struct copy_ctx *ctx)
{
	...
	if (args->ca_count == 0) {
		...
		if (ctx->src_off + to_copy > src_size)
			to_copy = src_size - ctx->src_off;
	}
				 ...
				 struct nfs_resop4 *resp)
{
	...
	if (ctx.to_copy == 0) {
		fill_copy_response_sync(res, 0);
		goto out;
	}

	status = copy_run_sync(&ctx, &copied);
	if (status == NFS4_OK)
		fill_copy_response_sync(res, copied);
	else
		res->cr_status = status;
