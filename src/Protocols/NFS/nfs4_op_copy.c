...
int nfs4_copy_fridge_shutdown(void)
{
	...
	if (rc == ETIMEDOUT) {
		...
		LogMajor(COMPONENT_THREAD,
			 "COPY fridge shutdown error: %d", rc);
	} else {
 ...
 */
static nfsstat4 check_copy_stateid(stateid4 *sid,
				   ...
				   state_t **state_out)
{
	...
	status = nfs4_Check_Stateid(sid, obj, &state, data,
...
static void copy_offload_state_free(struct state_t *state)
{
	struct copy_offload_state *cos =
		container_of(state, struct copy_offload_state, state);
	 ...
	 */
	if (cos->src_state) {
		dec_state_t_ref(cos->src_state);
		cos->src_state = NULL;
	}
	if (cos->dst_state) {
		dec_state_t_ref(cos->dst_state);
		cos->dst_state = NULL;
	}
	if (cos->src) {
		cos->src->obj_ops->put_ref(cos->src);
		cos->src = NULL;
	}
	if (cos->dst) {
		cos->dst->obj_ops->put_ref(cos->dst);
		cos->dst = NULL;
	}

	/* Free the destination FH buffer stored for CB_OFFLOAD coa_fh */
	gsh_free(cos->dst_fh.nfs_fh4_val);
	cos->dst_fh.nfs_fh4_val = NULL;
	...
	compound_data_t *data, struct fsal_obj_handle *dst, uint64_t tcount)
{
	...
	cos = gsh_calloc(1, sizeof(*cos));
	if (cos == NULL)
		return NULL;

	cos->state.state_type      = STATE_TYPE_COPY_OFFLOAD;
	/*
	 ...
	 */
	cos->state.state_seqid     = 0;
	cos->state.state_refcount  = 1; /* owner's reference */
	cos->state.state_free      = copy_offload_state_free;
	cos->state.state_export    = op_ctx->ctx_export;
	if (cos->state.state_export)
		get_gsh_export_ref(cos->state.state_export);
	 ...
	 */
	cos->state.state_owner = &clientid->cid_owner;
	 ...
	 */
	PTHREAD_MUTEX_init(&cos->state.state_mutex, NULL);

	/*
	 * glist_head fields must be self-referential (empty-list sentinel).
	 * gsh_calloc gives NULL which breaks glist_empty() and glist_del().
	 */
	glist_init(&cos->state.state_list);
	glist_init(&cos->state.state_owner_list);
	glist_init(&cos->state.state_export_list);

	cos->clientid    = clientid;
	inc_client_id_ref(clientid);
	cos->total_count = tcount;
	cos->status = NFS4_OK;

	{
		...
		cos->start_time.seconds  = now.tv_sec;
		cos->start_time.nseconds = now.tv_nsec;
	}

	nfs4_BuildStateId_Other(clientid, cos->state.stateid_other);

	if (nfs4_State_Set(&cos->state) != STATE_SUCCESS) {
		if (cos->state.state_export)
			put_gsh_export(cos->state.state_export);
		dst->obj_ops->put_ref(dst);
		cos->state.state_obj = NULL;
		dec_state_owner_ref(&clientid->cid_owner);
		cos->state.state_owner = NULL;
		dec_client_id_ref(clientid);
		PTHREAD_MUTEX_destroy(&cos->state.state_mutex);
		gsh_free(cos);
		return NULL;
	}
	 ...
	 * copy it here so it is available after the compound returns and
	 * data->currentFH may be repurposed.
	 *
	 * copy_offload_state_free() frees it on final refcount drop.
	 */
	if (data->currentFH.nfs_fh4_len > 0 &&
	    data->currentFH.nfs_fh4_val != NULL) {
		cos->dst_fh.nfs_fh4_len = data->currentFH.nfs_fh4_len;
		cos->dst_fh.nfs_fh4_val =
			gsh_malloc(data->currentFH.nfs_fh4_len);
		memcpy(cos->dst_fh.nfs_fh4_val, data->currentFH.nfs_fh4_val,
		       data->currentFH.nfs_fh4_len);
	}

	LogDebug(COMPONENT_NFS_V4,
		 "COPY async: created offload state seqid=%u",
		 cos->state.state_seqid);
	return cos;
}
...
static void destroy_copy_offload_state(struct copy_offload_state *cos)
{
	 ...
	 */
	PTHREAD_MUTEX_lock(&cos->state.state_mutex);
	dst = cos->state.state_obj;
	cos->state.state_obj = NULL;
	owner = cos->state.state_owner;
	cos->state.state_owner = NULL;
	export = cos->state.state_export;
	cos->state.state_export = NULL;
	PTHREAD_MUTEX_unlock(&cos->state.state_mutex);
	...
	if (cos->clientid) {
		dec_client_id_ref(cos->clientid);
		cos->clientid = NULL;
	}
...
static void cb_offload_completion(rpc_call_t *call)
{
	...
	if (call->states & NFS_CB_CALL_ABORTED) {
		...
		if (compound_st == NFS4_OK) {
			LogDebug(COMPONENT_NFS_V4,
			   ...
			   cos->cb_retry_count < NFS4_COPY_CB_MAX_RETRIES) {
			/*
	 ...
	 */
	if (cos != NULL && cos->clientid != NULL &&
	    cos->clientid->cid_minorversion != 0)
		nfs41_release_single(call);

	if (do_retry) {
		/* Re-send CB_OFFLOAD */
		nfs4_copy_send_cb_offload(cos,
					  cos->cb_bytes_copied,
					  cos->cb_fsal_status,
					  cos->cb_write_verifier);
		return;
	}
				      ...
				      const verifier4 write_verifier)
{
	...
	args->coa_fh = cos->dst_fh;
	COPY_STATEID(&args->coa_stateid, &cos->state);
	...
	if (FSAL_IS_ERROR(st)) {
		...
		LogDebug(COMPONENT_NFS_V4,
			...
			bytes_copied, cos->state.state_seqid);
	}
	 ...
	 */
	rc = nfs_rpc_cb_single(cos->clientid, &op, NULL,
...
{
	if (atomic_fetch_uint8_t((uint8_t *)&cos->cancelled)) {
		LogInfo(COMPONENT_NFS_V4,
			"COPY async worker stopping OFFLOAD_CANCEL received");
		return false;
	}

	if (atomic_fetch_uint8_t(&copy_fridge_stopping)) {
		LogEvent(COMPONENT_THREAD,
			 "COPY async worker stopping - server shutdown in progress");
		return false;
	}

	if (cos->clientid != NULL &&
			cos->clientid->cid_confirmed == EXPIRED_CLIENT_ID) {
		LogDebug(COMPONENT_NFS_V4,
			"client (id=%"PRIx64") lease expired or was revoked",
			cos->clientid->cid_clientid);
		return false;
	}
...
static void copy_worker(struct fridgethr_context *fridge_ctx)
{
	...
	uint64_t remaining    = cos->total_count;
	uint64_t src_off      = cos->src_off;
	uint64_t dst_off      = cos->dst_off;
	 ...
	 */
	init_op_context_simple(&op_context, cos->ctx_export, cos->fsal_export);
	...
	LogDebug(COMPONENT_NFS_V4,
		...
		cos->total_count, cos->src_off, cos->dst_off,
	...
	while (remaining > 0) {
		...
		LogFullDebug(COMPONENT_NFS_V4,
			     ...
			     chunk_num, src_off, chunk,
			     bytes_copied, cos->total_count);

		st = cos->src->obj_ops->copy_file_range(
			cos->src, cos->src_state,
			cos->dst, cos->dst_state,
			src_off, dst_off, chunk, &chunk_copied);
		if (FSAL_IS_ERROR(st)) {
			LogWarn(COMPONENT_NFS_V4,
				"COPY async worker FSAL ERROR at chunk #%"
				PRIu64 " byte_offset=%" PRIu64
				": major=%u minor=%u"
	...
	rate_kbps   = elapsed_ms > 0
		? (bytes_copied * 1000ULL / elapsed_ms / 1024ULL) : 0;
	...
	if (FSAL_IS_ERROR(st)) {
		...
		LogDebug(COMPONENT_NFS_V4,
			...
			" KB/s) chunks=%" PRIu64,
			bytes_copied, elapsed_ms, rate_kbps,
			chunk_num);
	}

	cos->status       = FSAL_IS_ERROR(st) ? nfs4_Errno_status(st) : NFS4_OK;
	atomic_store_uint64_t(&cos->bytes_copied, bytes_copied);
	atomic_store_uint8_t(&cos->complete, 1);
	 ...
	 */
	cos->cb_bytes_copied = bytes_copied;
	cos->cb_fsal_status  = st;
	memcpy(cos->cb_write_verifier, write_verifier, sizeof(verifier4));
			      ...
			      struct copy_ctx *ctx)
{
	...
	status = check_copy_stateid(&args->ca_dst_stateid, ctx->dst, data,
				    true, &ctx->dst_state);
	if (status != NFS4_OK)
		goto err_release;
	...
	if (args->ca_count == 0) {
		to_copy = (ctx->src_off < src_size) ?
			  (src_size - ctx->src_off) : 0;
	} else {
		/*
			    ...
			    uint64_t to_copy)
{
	...
	LogDebug(COMPONENT_NFS_V4,
		 ...
		 " (NFSv4.2/session)",
		 (minorver == 0 && get_cb_chan_down(clid)) ?
		 "DOWN (will abort)" : "UP");
			      ...
			      bool do_async)
{
	LogDebug(COMPONENT_NFS_V4,
		...
		" allow_offload=%d min_size=%" PRIu64
		" -> %s"
		" [SERVER RESPONSE will have: wr_ids=%d cr_synchronous=%d]",
		to_copy,
		(int)args->ca_consecutive,
		(int)args->ca_synchronous,
 ...
 */
static void fill_copy_response(COPY4res *res, compound_data_t *data,
			       bool async, struct copy_offload_state *cos,
			       uint64_t sync_bytes_copied)
{
	...
	if (async) {
		...
		/*
		 * update_stateid() increments cos->state.state_seqid from
		 ...
		 */
		update_stateid_locked(&cos->state,
			       &resok->cr_response.wr_callback_id,
			       data, "COPY-OFFLOAD");

		resok->cr_response.wr_count = 0;
		resok->cr_response.wr_committed = UNSTABLE4;
		memset(resok->cr_response.wr_writeverf, 0,
		       sizeof(verifier4));
		...
		LogDebug(COMPONENT_NFS_V4,
			...
			cos->total_count, cos->state.state_seqid);
		return;
	} else {
		struct gsh_buffdesc verf_desc;
...
static nfsstat4 copy_run_sync(const struct copy_ctx *ctx, uint64_t *copied_out)
{
	...
	fsal_st = ctx->src->obj_ops->copy_file_range(
		ctx->src, ctx->src_state,
 ...
 */
static enum copy_async_dispatch copy_run_async(compound_data_t *data,
					       struct copy_ctx *ctx,
					       ...
					       **cos_out)
{
	...
	*cos_out = NULL;
	 ...
	 * the worker keeps independent refs via cos->src and cos->dst.
	 * dst is also held by cos->state.state_obj (the SAL ref taken in
	...
	cos->src        = ctx->src;
	cos->dst        = ctx->dst;
	cos->src_state  = ctx->src_state;
	cos->dst_state  = ctx->dst_state;
	cos->src_off    = ctx->src_off;
	cos->dst_off    = ctx->dst_off;
	cos->ctx_export  = op_ctx->ctx_export;
	cos->fsal_export = op_ctx->fsal_export;
	if (cos->ctx_export)
		get_gsh_export_ref(cos->ctx_export);
	...
	if (submit_rc == EWOULDBLOCK) {
		LogDebug(COMPONENT_NFS_V4, "COPY: all %" PRIu32
 ...
 */
enum nfs_req_result nfs4_op_copy(struct nfs_argop4 *op,
				 compound_data_t *data,
