...
/**
 * @brief Claim a stable xcopyN thread name (called once per thread).
 *
 * Uses the TLS flag copy_thr_named so repeated calls are cheap no-ops.
 */
static inline void copy_claim_thread_name(void)
{
	char thr_name[32];
	uint32_t my_id;
	...
	my_id = atomic_postadd_uint32_t(&copy_thr_next_id, 1);
