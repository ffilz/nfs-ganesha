...
static void copy_offload_state_free(struct state_t *state)
{
	...
	gsh_free(cos->cos_dst_fh.nfs_fh4_val, MEM_COMP_PROTOCOL);
	cos->cos_dst_fh.nfs_fh4_val = NULL;

	gsh_free(cos, MEM_COMP_STATE);
}
	...
	compound_data_t *data, struct fsal_obj_handle *dst, uint64_t tcount)
{
	...
	cos = gsh_calloc(1, sizeof(struct copy_offload_state), MEM_COMP_STATE);
					...
					struct nfs_resop4 *resp)
{
	...
	if (rc != NFS_REQ_ASYNC_WAIT) {
		 ...
		 */
		gsh_free(data->op_data, MEM_COMP_FRIDGE);
		data->op_data = NULL;
	}
	...
	COPY4res *sync_res, struct copy_offload_state **cos_out)
{
	...
	job = gsh_malloc(sizeof(*job), MEM_COMP_FRIDGE);
	jctx = &job->job_ctx;
