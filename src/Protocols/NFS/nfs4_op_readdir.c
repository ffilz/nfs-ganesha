...
struct nfs4_readdir_cb_data {
	...
	/* dircount accounting (cookie+name bytes, XDR sized). 0 = unlimited (RFC). */
...
not_junction:
	...
	{
		size_t add_bytes =
			sizeof(nfs_cookie4) + RNDUP(name.utf8string_len);
...
failure:
	...
	if (!tracker->has_entries && tracker->error == NFS4_OK) {
		/* Hitting dircount is not an error;
		 * TOOSMALL is for maxcount/single-entry. */
		if (!tracker->dircount_hit)
				    ...
				    struct nfs_resop4 *resp)
{
	...
	tracker.max_count = 0; /* 0 = unlimited entries */
