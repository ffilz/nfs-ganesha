...
void set_slot_last_req(compound_data_t *data)
{
	...
	data->slot->last_req.opcode_num = get_nfs4_opcodes(data,
		data->slot->last_req.opcodes, NFS41_COMPOUND_OPERATIONS_LEN);
 ...
 *  @param[in]  opcodes_array_len  size of opcodes array
 *
...
int nfs4_Compound(nfs_arg_t *arg, struct svc_req *req, nfs_res_t *res)
{
	...
	if (likely(component_log_level[NIV_FULL_DEBUG] >= NIV_FULL_DEBUG)) {
