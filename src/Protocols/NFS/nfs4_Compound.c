 ...
 bad_op_state:
	...
	if (data.sa_cachethis) {
		...
		*data.cached_result = res->res_compound4_extended;
		...
		c_res->status = res->res_compound4.status;
		...
		c_res->resarray.resarray_val[0] =
			res->res_compound4.resarray.resarray_val[0];

		if (resarray_len == 2) {
			struct nfs_resop4 *res1 =
				c_res->resarray.resarray_val + 1;
