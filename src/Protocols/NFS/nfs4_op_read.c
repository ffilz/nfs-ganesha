			  ...
			  void *read_data, void *caller_data)
{
	...
	flags = atomic_postset_uint32_t_bits(&data->flags, READ_DONE);

	if ((flags & READ_EXIT) == READ_EXIT) {
				     ...
				     struct io_info *info)
{
	 ...
	 */
	flags = atomic_postset_uint32_t_bits(&read_data->flags, READ_EXIT);

 out:
	if (state_open != NULL)
		dec_state_t_ref(state_open);

	if ((flags & READ_DONE) != READ_DONE) {
