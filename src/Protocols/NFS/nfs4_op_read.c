...
void nfs4_qos_read_cb(void *args)
{
	...
	gsh_free(args, MEM_COMP_SESSION);
}
#endif
						  ...
						  struct nfs_resop4 *resp)
{
	...
	gsh_free(read_data, MEM_COMP_IO_BUFFER);
	data->op_data = NULL;
					...
					struct nfs_resop4 *resp)
{
	...
	if (rc != NFS_REQ_ASYNC_WAIT) {
		 ...
		 */
		gsh_free(data->op_data, MEM_COMP_IO_BUFFER);
		data->op_data = NULL;
	}
				     ...
				     struct nfs_resop4 *resp)
{
	...
	struct nfs4_read_data *read_data =
		gsh_calloc(1, sizeof(*read_data), MEM_COMP_IO_BUFFER);
				     ...
				     struct io_info *info)
{
	...
	read_data = gsh_calloc(1, sizeof(*read_data), MEM_COMP_IO_BUFFER);
