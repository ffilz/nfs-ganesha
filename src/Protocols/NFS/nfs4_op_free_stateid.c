					 ...
					 struct nfs_resop4 *resp)
{
	 ...
	 *
	 * FREE_STATEID in this case serves as the client's acknowledgement
	...
	if (is_stateid_revoked(&arg_FREE_STATEID4->fsa_stateid)) {
		...
		/* Remove stateid from revoked delegation list that was at the time of revoke */
		...
		if (state) {
			...
			if (state->state_type == STATE_TYPE_DELEG) {
				/* Get object and export references for locking and context */
				if (get_state_obj_export_owner_refs(
					...
					/* Unable to get export/obj, fallback: just delete state
					 * without lock
					 */
					LogDebug(
						COMPONENT_STATE,
