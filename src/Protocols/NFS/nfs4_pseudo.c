...
bool pseudo_mount_export(struct gsh_export *export)
{
	...
	*last_slash = '\0';
	...
	ctx_pseudopath = CTX_PSEUDOPATH(op_ctx);
	...
	*last_slash = '/';
	 ...
	 */
	if (ctx_pseudopath[1] == '\0')
		rest = tmp_pseudopath + 1;
	else
		rest = tmp_pseudopath +
		       strlen(ctx_pseudopath) + 1;
...
void pseudo_unmount_export(struct gsh_export *export)
{
	...
	/* Release the export lock because we can't hold it and take the
	 * state_lock to prevent ABBA deadlock.
	 */
	PTHREAD_RWLOCK_unlock(&export->lock);
	...
	if (is_export_pseudo(mounted_on_export) && junction_inode != NULL) {
		...
		/* Initialize req_ctx */
		init_root_op_context(&root_op_context, mounted_on_export,
