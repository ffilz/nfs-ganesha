...
static void populate_callback_params_in_session(
	uint32_t sec_parms_len, callback_sec_parms4 *sec_parms_val,
	uint32_t cb_program, nfs41_session_t * const nfs41_session,
	log_components_t component)
{
	int sec_parms_itr;
	callback_sec_parms4 *extracted_sec_params = gsh_malloc(
		sec_parms_len * sizeof(callback_sec_parms4));

	for (sec_parms_itr = 0; sec_parms_itr < sec_parms_len; ++sec_parms_itr) {
		const callback_sec_parms4 input_sec_param = sec_parms_val[sec_parms_itr];
		callback_sec_parms4 curr_sec_param = extracted_sec_params[sec_parms_itr];
		curr_sec_param.cb_secflavor = input_sec_param.cb_secflavor;

		if (curr_sec_param.cb_secflavor == AUTH_NONE) {
			/* Do nothing */
		} else if (curr_sec_param.cb_secflavor == AUTH_SYS) {
			int gids_itr;
			size_t machname_len;
			struct authunix_parms curr_cb_sys_creds;
			struct authunix_parms input_cb_sys_creds =
				input_sec_param.callback_sec_parms4_u.cbsp_sys_cred;

			curr_cb_sys_creds.aup_uid = input_cb_sys_creds.aup_uid;
			curr_cb_sys_creds.aup_gid = input_cb_sys_creds.aup_gid;
			curr_cb_sys_creds.aup_time = input_cb_sys_creds.aup_time;

			/* Populate aup_machname */
			machname_len = strlen(input_cb_sys_creds.aup_machname);
			curr_cb_sys_creds.aup_machname = gsh_malloc(machname_len + 1);
			memcpy(curr_cb_sys_creds.aup_machname, input_cb_sys_creds.aup_machname,
				machname_len);
			curr_cb_sys_creds.aup_machname[machname_len] = '\0';

			curr_cb_sys_creds.aup_len = input_cb_sys_creds.aup_len;
			curr_cb_sys_creds.aup_gids =
				gsh_malloc(curr_cb_sys_creds.aup_len * sizeof(gid_t));

			for (gids_itr = 0; gids_itr < input_cb_sys_creds.aup_len; ++gids_itr) {
				curr_cb_sys_creds.aup_gids[gids_itr] =
					input_cb_sys_creds.aup_gids[gids_itr];
			}
			curr_sec_param.callback_sec_parms4_u.cbsp_sys_cred = curr_cb_sys_creds;

#ifdef _HAVE_GSSAPI
		} else if (curr_sec_param.cb_secflavor == RPCSEC_GSS) {
			LogInfo(component,
				"We donot support GSS callbacks, skip GSS callback setup");
#endif
		}
	}
