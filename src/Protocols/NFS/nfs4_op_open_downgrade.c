					   ...
					   struct nfs_resop4 *resp)
{
	...
	rc = nfs4_check_stateid_acquire_locks(
		...
		&open_owner, &state_handle_lock_held, &seqid_lock_held);

	if (rc != NFS4_OK && rc != NFS4ERR_REPLAY) {
		res_OPEN_DOWNGRADE4->status = rc;
		LogDebug(
			COMPONENT_STATE,
			"OPEN_DOWNGRADE failed nfs4_check_stateid_acquire_locks");
		return NFS_REQ_ERROR;
	}
	...
	if (data->minorversion == 0) {
		...
		if (!Check_nfs4_seqid(open_owner, arg_OPEN_DOWNGRADE4->seqid,
				      op, state_obj, resp, tag)) {
...
out2:
	if (seqid_lock_held) {
		PTHREAD_MUTEX_unlock(&state_found->seqid_mutex);
	}
