		       ...
		       struct attrlist *attrs)
{
	...
	if (attrs == NULL) {
		pattrs = &attr_buf;
		fsal_prepare_attrs(pattrs, ATTRS_NFS3);

		status = obj->obj_ops.getattrs(obj, pattrs);
	...
	if ((attrs->mask & ATTR_RDATTR_ERR) != 0 || FSAL_IS_ERROR(status)) {
	...
	/* Release any attributes fetched or provided. */
