...
static fattr_xdr_result encode_fsid(XDR *xdr, struct xdr_attrs_args *args)
{
	...
	if (args->data != NULL &&
		...
		LogDebug(COMPONENT_NFS_V4,
			 "module_path = %s, module_name = %s",
			 args->data->current_obj->fsal->path,
			 args->data->current_obj->fsal->name);
		st = args->data->current_obj->
			obj_ops.fs_loc_fsid(args->data->current_obj,
					    &fsidnew);
		LogDebug(COMPONENT_NFS_V4,
			 "st.minor = %d, st.major = %d", st.minor, st.major);
		if (!FSAL_IS_ERROR(st) && fsidnew.major > 0) {
			LogDebug(COMPONENT_NFS_V4,
				 "fs_loc_fsid.major %"PRIu64", fs_loc_fsid.minor = %"PRIu64,
				 fsidnew.major, fsidnew.minor);
			fsid = fsidnew;
		} else {
