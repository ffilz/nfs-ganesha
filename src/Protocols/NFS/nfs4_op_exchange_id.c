					...
					struct nfs_resop4 *resp)
{
	...
	if (cid_server_owner[0] == '\0') {
		...
		if (nfs_param.nfsv4_param.server_owner == NULL) {
			...
			strncpy(cid_server_owner, nfs_param.nfsv4_param.server_owner,
				sizeof(cid_server_owner));
			if (cid_server_owner[sizeof(cid_server_owner)-1] != '\0') {
				cid_server_owner[sizeof(cid_server_owner)-1] = '\0';
				LogDebug(COMPONENT_CLIENTID,
					"Server owner was too long and will be cut. Server owner: %s",
					cid_server_owner);
			}
		}
