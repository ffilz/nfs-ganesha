 ...
 */
bool inline get_nfs4_operations(nfs_opnum4 *opcodes, uint32_t opcodes_len,
	char* operations, uint32_t operations_len)
{
	...
	for (i = 0; i < opcodes_len; i++) {
		const char *opname = nfsop4_to_str(opcodes[i]);
	...
	sequenceid4 req_seq_id, uint32_t slotid)
{
	...
	uint32_t req_xid = data->req->rq_msg.rm_xid, i;
	char[NFS41_COMPOUND_OPERATIONS_LEN] last_operations, curr_operations;
...
errout:
	...
	if (likely(component_log_level[component] >= NIV_EVENT)) {
		...
		LogEvent(COMPONENT_SESSIONS,
			...
			"slotid %"PRIu32" cached request operations %s seqid %"PRIu32
			" xid %"PRIu32" finish time %"PRIu64
			", new request operations %s seqid %"PRIu32" xid %"PRIu32,
			slotid,
			(last_succ ? last_operations : "Unknow operations"),
			slot->last_req.seq_id,
			slot->last_req.xid,
			slot->last_req.finish_time_ms,
			(curr_succ ? curr_operations : "Unknow operations"),
			req_seq_id,
			req_xid);
	}
}
				     ...
				     struct nfs_resop4 *resp)
{
	...
	if (slot->sequence + 1 != arg_SEQUENCE4->sa_sequenceid) {
		...
		if (slot->sequence == arg_SEQUENCE4->sa_sequenceid) {
			check_replay_request(data, slot, arg_SEQUENCE4->sa_sequenceid, slotid);
