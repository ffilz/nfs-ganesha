...
errout:
	...
	if (likely(component_log_level[NIV_EVENT] >= NIV_EVENT)) {
		char last_operations[NFS41_COMPOUND_OPERATIONS_LEN] = "\0";
		char curr_operations[NFS41_COMPOUND_OPERATIONS_LEN] = "\0";
		...
		LogEvent(COMPONENT_SESSIONS,
			"Last request %s slotid %"PRIu32" seqid %"PRIu32" xid %"PRIu32
		...
		LogEvent(COMPONENT_SESSIONS,
			"Current request %s slotid %"PRIu32" seqid %"PRIu32" xid %"PRIu32,
			curr_operations,
			slotid,
			req_seq_id,
			req_xid);
