				     ...
				     struct nfs_resop4 *resp)
{
	...
	LogFullDebug(
		COMPONENT_SESSIONS,
		"Checking revoked delegations flag for session %p, has_revoked_delegations=%s",
		session, session->has_revoked_delegations ? "true" : "false");
	...
	if (session->has_revoked_delegations) {
		/* Check if there are actually any revoked delegations left for this client */
		...
		if (has_revoked) {
			/* Set the flag to indicate there are revoked delegations */
			...
			/* No more revoked delegations, clear the session flag */
			session->has_revoked_delegations = false;
			LogDebug(
				COMPONENT_SESSIONS,
				"Clearing flag for session %p (clientid=0x%llx)",
				session, (unsigned long long)session->clientid);
		}
	} else {
