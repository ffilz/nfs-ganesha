	...
	*file_state = NULL;
	...
	case CLAIM_NULL:
		...
		if (status == CACHE_INODE_SUCCESS) {
			/* We found the file by lookup, discard the filename and
			 * remember that we found the entry by lookup.
			 */
	...
	if (*file_state == NULL) {
		*file_state = op_ctx->fsal_export->exp_ops.alloc_state(
		...
		/* Check if we succeeded in allocating a new state */
		if (*file_state == NULL) {
	...
	if (*new_state) {
		...
		LogFullDebug(COMPONENT_STATE, "Creating new state");
	...
	if (!(*new_state)) {
		...
		LogCrit(COMPONENT_STATE,
			     "Open upgrade access=%x deny=%x access_prev=%x deny_prev=%x",
			     (*file_state)->state_data.share.share_access,
			     (*file_state)->state_data.share.share_deny,
			     (*file_state)->state_data.share.share_access_prev,
			     (*file_state)->state_data.share.share_deny_prev);
