	...
	*new_state = false;
	...
	if (file_obj != NULL) {
		...
		*file_state = nfs4_State_Get_Obj(file_obj, owner);
		if (*file_state != NULL) {
			PTHREAD_MUTEX_lock(&(*file_state)->seqid_mutex);
			seqid_lock_held = true;
			...
			if ((*file_state)->state_owner == NULL) {
				PTHREAD_MUTEX_unlock(
					&(*file_state)->seqid_mutex);
				seqid_lock_held = false;
...
retry_open_file:
	...
	/* Update seqid/stateid here so it will be under the seqid mutex. */
	if (res_OPEN4->status == NFS4_OK) {
		update_stateid(*file_state,
			       &res_OPEN4->OPEN4res_u.resok4.stateid, data,
			       open_tag);
	}
out:
	if (seqid_lock_held) {
		PTHREAD_MUTEX_unlock(&(*file_state)->seqid_mutex);
		seqid_lock_held = false;
	}
