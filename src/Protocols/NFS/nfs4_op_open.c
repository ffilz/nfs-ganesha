				 ...
				 struct nfs_resop4 *resp)
{
	...
	bool res;
	file = NULL;
	res = fetch_file_md(bucket, data->current_obj->fsid.major,
			    data->current_obj->fsid.minor,
			    data->current_obj->fileid, file);

	if (!res) {
		file = (struct file_metadata *)malloc(
			sizeof(struct file_metadata));
		file->fileid = data->current_obj->fileid;
		res = persist_file_md(bucket, data->current_obj->fsid.major,
	...
	if (nfs_get_grace_status(true)) {
		struct open_info *opens = NULL;
		LogAlways(COMPONENT_MIGRATION, "fetch file md: %i", res);
		res = fetch_persisting_opens(bucket, file, opens);
		LogAlways(COMPONENT_MIGRATION, "CAN OPEN: %i", res);
	...
	res_OPEN4->status = open4_validate_claim(data, claim, clientid,
						 &grace_ref, can_open);
	LogAlways(COMPONENT_ALL, "Reached Here");
	if (res_OPEN4->status != NFS4_OK) {
...
out2:
	...
	LogAlways(COMPONENT_ALL, "Persist Open");
	if (res_OPEN4->status == NFS4_OK) {
		// PROBLEM
		if (nfs_get_grace_status(true)) {
