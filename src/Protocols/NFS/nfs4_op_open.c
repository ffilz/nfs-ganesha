				 ...
				 struct nfs_resop4 *resp)
{
	...
	base64url_encode(data->currentFH.nfs_fh4_val,
			 data->currentFH.nfs_fh4_len, rhdlstr, sizeof(rhdlstr));

	if (nfs_get_grace_status(true)) {
		struct open_info *open = NULL;
		fetch_persisting_open(&open, bucket, rhdlstr);
		LogAlways(COMPONENT_ALL, "Open cowner %s", open->cowner);
		if (open) {
			if (strcmp(open->filehandle,
				   data->currentFH.nfs_fh4_val) == 0 &&
			    strcmp(open->cowner, cowner) != 0 &&
			    is_conflict_persisting_opens(
				    open->share_access, open->share_deny,
				    arg_OPEN4->share_access,
				    arg_OPEN4->share_deny))
				can_open = false;

			free(open->cowner);
			free(open->filehandle);
			free(open);
			LogAlways(COMPONENT_ALL, "Conflict ? %i", !can_open);
		}
	}
...
out2:
	...
	if (res_OPEN4->status == NFS4_OK) {
		if (nfs_get_grace_status(true)) {
			persist_open_info(grace_bucket, rhdlstr, cowner,
					  data->currentFH.nfs_fh4_val,
					  arg_OPEN4->share_access,
					  arg_OPEN4->share_deny);
		} else
			persist_open_info(bucket, rhdlstr, cowner,
					  data->currentFH.nfs_fh4_val,
					  arg_OPEN4->share_access,
					  arg_OPEN4->share_deny);
	}
