	...
	nfs41_session_t *session)
{
	...
	struct display_buffer db = {sizeof(session_str), session_str, session_str};
	char xprt_addr_str[LOG_BUFF_LEN / 2] = "\0";
	struct display_buffer db1 =
		{sizeof(xprt_addr_str), xprt_addr_str, xprt_addr_str};

	/* Copy the address coming over the wire for logging. */
	copy_xprt_addr(&addr, rq_xprt);
	display_sockaddr(&db1, &addr);
	display_session(&db, session);

	LogInfo(COMPONENT_SESSIONS, "Set up session: %s backchannel and bind it"
		" to current xprt FD: %d socket-address: %s", session_str,
		rq_xprt->xp_fd, xprt_addr_str);
	 ...
	 * Since Ganesha supports only SP4_NONE as of now, we skip processing of
	 * other mechanisms.
	 */
	if (session->clientid_record->cid_state_protect_how == SP4_NONE) {
		...
		if (rc == 0) {
			LogInfo(COMPONENT_SESSIONS,
				"Successfully created backchannel for session: %s", session_str);
			return NFS4_OK;
		}
	...
	LogFatal(COMPONENT_SESSIONS, "Only SP4_NONE state protection is supported."
				      ...
				      struct nfs_resop4 *resp)
{
	...
	case CDFC4_BACK_OR_BOTH:
		nfsstat4 bindToBackchannel =
			bind_conn_to_session_backchannel(data->req->rq_xprt, session);
		if (bindToBackchannel != NFS4_OK) {
			if (arg_BIND_CONN_TO_SESSION4->bctsa_dir == CDFC4_FORE_OR_BOTH) {
				/* Since it is not mandatory to bind connection to backchannel in
				 * this scenario, we return only successful forechannel creation.
				 */
				server_channel_dir = CDFS4_FORE;
				break;
			}
			LogWarn(COMPONENT_SESSIONS,
				"Mandatory backchannel creation failed. Return error");
			res_BIND_CONN_TO_SESSION4->bctsr_status = bindToBackchannel;
			return NFS_REQ_ERROR;
		}
		if (arg_BIND_CONN_TO_SESSION4->bctsa_dir == CDFC4_BACK_OR_BOTH ||
			arg_BIND_CONN_TO_SESSION4->bctsa_dir == CDFC4_FORE_OR_BOTH) {
