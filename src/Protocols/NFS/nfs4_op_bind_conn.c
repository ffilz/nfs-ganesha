	...
	nfs41_session_t *session)
{
	...
	LogInfo(COMPONENT_SESSIONS,
		"Set up session: %s backchannel and bind it to current xprt"
		" FD: %d socket-address: %s", session_str, rq_xprt->xp_fd,
		xprt_addr_str);
	...
	if (session->clientid_record->cid_state_protect_how == SP4_NONE) {
		...
		if (rc == 0) {
			LogInfo(COMPONENT_SESSIONS,
				"Created backchannel for session: %s",
				session_str);
			return NFS4_OK;
		}
	...
	LogFatal(COMPONENT_SESSIONS,
		"Only SP4_NONE state protection is supported. Code flow should"
		" not reach here");
	return NFS4ERR_SERVERFAULT;
}
				      ...
				      struct nfs_resop4 *resp)
{
	...
	case CDFC4_BACK_OR_BOTH:
		nfsstat4 bindToBackchannel = bind_conn_to_session_backchannel(
