					     ...
					     state_lock_entry_t *lock_entry)
{
	...
	/*
	 * Before attempting the callback RPC, verify the client still has a
	 * working back channel.  A disconnected client will have its callback
	 * channel marked as down (v4.0) or will have no session with the
	 * backchannel flag set (v4.1/4.2).  In either case, sending the RPC
	 * would dereference the already-destroyed transport (ev_p == NULL)
	 * and crash.  Remove this blocked lock entry so the next waiter in
	 * the queue can be considered.
	 */
	if (clientrec->cid_minorversion == 0) {
		if (get_cb_chan_down(clientrec)) {
			LogWarn(COMPONENT_NFS_V4_LOCK,
				"Skipping granted callback: v4.0 callback channel is down for client %p",
				clientrec);
			return STATE_ESTALE;
		}
	} else {
		/*
		 * For v4.1+, check whether at least one session still has an
		 * active backchannel.  nfs_rpc_v41_single() performs the same
		 * check, but doing it here avoids building and then discarding
		 * the CB_COMPOUND payload when we already know the call cannot
		 * succeed.
		 */
		bool bc_available = false;
		struct glist_head *glist;

		PTHREAD_MUTEX_lock(&clientrec->cid_mutex);
		glist_for_each(glist, &clientrec->cid_cb.v41.cb_session_list) {
			nfs41_session_t *session = glist_entry(glist,
							       nfs41_session_t,
							       session_link);
			if (atomic_fetch_uint32_t(&session->flags) &
			    session_bc_up) {
				bc_available = true;
				break;
			}
		}
		PTHREAD_MUTEX_unlock(&clientrec->cid_mutex);

		if (!bc_available) {
			LogWarn(COMPONENT_NFS_V4_LOCK,
				"Skipping granted callback: no v4.1 session with active backchannel for client %p",
				clientrec);
			return STATE_ESTALE;
		}
	}
