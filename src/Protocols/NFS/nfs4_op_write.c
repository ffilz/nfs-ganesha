			  ...
			  void *write_data, void *caller_data)
{
	...
	flags = atomic_postset_uint32_t_bits(&data->flags, WRITE_DONE);

	if ((flags & WRITE_EXIT) == WRITE_EXIT) {
				  ...
				  struct nfs_resop4 *resp)
{
	 ...
	 */
	flags = atomic_postset_uint32_t_bits(&write_data->flags, WRITE_EXIT);
 ...
 out:
	...
	if ((flags & WRITE_DONE) != WRITE_DONE) {
		/* The write was not finished before we got here. When the
		 * read completes, nfs4_write_cb() will have to reschedule the
