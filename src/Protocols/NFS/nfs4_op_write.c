					 ...
					 struct nfs_resop4 *resp)
{
	...
	if (write_data->qos_flag & IS_QOS_IO) {
		if (nfs4_Is_Fh_DSHandle(&data->currentFH)) {
			WRITE4args *const arg_WRITE4 =
				&op->nfs_argop4_u.opwrite;
			WRITE4res *const res_WRITE4 =
				&resp->nfs_resop4_u.opwrite;

			write_data->qos_flag = 0;
			res_WRITE4
				->status = op_ctx->ctx_pnfs_ds->s_ops.dsh_write(
				data->current_ds, &arg_WRITE4->stateid,
				arg_WRITE4->offset,
