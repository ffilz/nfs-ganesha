					   ...
					   struct nfs_resop4 *resp)
{
	...
	cos = container_of(state, struct copy_offload_state, state);
	complete = atomic_fetch_uint8_t((uint8_t *)&cos->complete);
	bytes_copied = atomic_fetch_uint64_t(&cos->bytes_copied);
	completion_status = cos->status;

	/* Log detailed progress for tracking */
	uint64_t total = cos->total_count;
	uint32_t seqid = cos->state.state_seqid;
	uint64_t elapsed_s = 0;

	if (cos->start_time.seconds) {
		struct timespec now;

		clock_gettime(CLOCK_REALTIME, &now);
		elapsed_s = (uint64_t)now.tv_sec -
			    (uint64_t)cos->start_time.seconds;
	}
	...
	LogFullDebug(COMPONENT_NFS_V4,
		...
		total ? (bytes_copied * 100 / total) : 0,
		elapsed_s, complete ? "YES" : "NO",
		(int)(complete ? completion_status : NFS4_OK),
		complete ? nfsstat4_to_str(completion_status) : "in-progress",
		atomic_fetch_uint8_t((uint8_t *)&cos->cancelled) ? "YES"
								 : "NO");
	...
	LogFullDebug(COMPONENT_NFS_V4,
		...
		complete ? (completion_status == NFS4_OK ?
			"done-OK" : "done-ERR") : "in-progress");
