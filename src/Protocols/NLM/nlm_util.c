...
void fill_netobj(netobj *dst, char *data, int len)
{
	...
	dst->n_bytes = NULL;
	if (len != 0) {
		dst->n_bytes = gsh_malloc(len, MEM_COMP_FILE_AND_STATE_LOCK);
		dst->n_len = len;
...
void copy_netobj(netobj *dst, netobj *src)
{
	if (src->n_len != 0) {
		dst->n_bytes =
			gsh_malloc(src->n_len, MEM_COMP_FILE_AND_STATE_LOCK);
...
{
	gsh_free(obj->n_bytes, MEM_COMP_FILE_AND_STATE_LOCK);
}
...
void free_grant_arg(state_async_queue_t *arg)
{
	...
	gsh_free(arg, MEM_COMP_FILE_AND_STATE_LOCK);
}
				    ...
				    state_lock_entry_t *lock_entry)
{
	...
	arg = gsh_calloc(1, sizeof(*arg), MEM_COMP_FILE_AND_STATE_LOCK);
