...
bool nsm_monitor(state_nsm_client_t *host)
{
	...
	assert(nfs_param.core_param.use_statd);
#endif
...
bool local_nlm_info_client_create(struct local_nlm_info *info)
{
	...
	LogFullDebug(COMPONENT_NLM,
		     "clnt_tli_ncreate netid=%s client=%s port=%d server=%s",
		     info->nconf->nc_netid, info->client_address_str,
		     info->info_port, info->server_address_str);
...
static void local_nlm_info_cb(struct clnt_req *req)
{
	...
	LogFullDebug(COMPONENT_NLM, "local_nlm_info_cb");
	...
	/* Remove from wait list - now wait_callback_list thread won't wait for
	 * reply anymore.
	 */
	...
	case RPC_SUCCESS:
		if (my_id->my_prog == RPCBPROG) {
			...
			*/
			...
			/* Now go back to send the actual request setup above.
...
void req_init(struct local_nlm_info *info)
{
	if (nsm_auth == NULL) {
		/* Create AUTH_NONE credential. */
		nsm_auth = authnone_ncreate();
	}

	clnt_req_fill(&info->cc, NULL, nsm_auth, 0, NULL, &info->args, NULL,
		      &info->res);
...
bool req_call(struct local_nlm_info *info)
{
	...
	/* Replace the information for the call with the actual information for
	 * the current call.
	 */
	...
	LogFullDebug(COMPONENT_NLM, "clnt_req_setup");
	info->cc.cc_error.re_status = clnt_req_setup(&info->cc, tout);
	...
	LogFullDebug(COMPONENT_NLM, "CLNT_CALL_ONCE");
	info->cc.cc_error.re_status = CLNT_CALL_ONCE(&info->cc);
...
void send_sm_notify(struct local_nlm_info *info)
{
	...
	if (my_id->my_prog != SM_PROG) {
...
#else
			LogDebug(COMPONENT_NLM,
				 "Starting with PORTMAP for client %s",
				 info->client_address_str);
#endif
		...
		case PMAPVERS:
			LogDebug(COMPONENT_NLM,
				 "Retry PORTMAP (Version 2) for client %s",
				 info->client_address_str);
	...
	/* Once PORTMAP succeeds, the SM_NOTIFY request will have been set up
	 * by local_nlm_info_cb() and we will skip the above block.
	 */
...
void send_local_nlm_info(struct local_nlm_info *info)
{
	assert(info->recovery_type == NLM_CLIENT_ENTRY ||
	       info->recovery_type == NLM_SM_MON_ENTRY);
...
void *wait_callback_list(void *unused)
{
	...
	while (!glist_empty(&local_nlm_info_list)) {
		...
		/* Set this up for making RPC calls. */
		req_init(info);
	...
	while (!glist_empty(&callback_wait_list)) {
		...
		if (info == NULL) {
			/* Nothing left to wait for, done */
			break;
		}

		if (info->reply_due > now) {
			/* Delay until the next due comes around. */
			delay = info->reply_due - now;

			/* Drop mutex while we sleep */
			PTHREAD_MUTEX_unlock(&nsm_mutex);
			/* Just sleep, no need for a condition variable, we are
			 * just waiting for a list to drain, no need to wait
			 * for new work.
			 */
			sleep(delay);
			now = time(NULL);
...
pthread_t wait_callback_list_thrid;
...
void process_local_nlm_info(void)
{
	...
	rc = PTHREAD_create(&wait_callback_list_thrid, &attr_thr,
			    wait_callback_list, NULL);

	if (rc != 0) {
		LogFatal(COMPONENT_NLM,
			 "Could not create wait_callback_list, error = %d (%s)",
