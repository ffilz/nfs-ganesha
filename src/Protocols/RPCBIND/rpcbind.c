...
char *mergeaddr(SVCXPRT *xprt, struct rpcb *request, SVCXPRT *srv_xprt, int af)
{
	...
	//TODO: Handle IPV4 encapsulated in IPv6 for caller address
	...
	if (request != NULL && request->r_addr != NULL &&
	    *request->r_addr != '\0') {
		...
		//TODO: Handle IPV4 encapsulated in IPv6 for hint address
	...
	for (ifap = ifp; ifap != NULL; ifap = ifap->ifa_next) {
		...
		if (ifsa == NULL || !(ifap->ifa_flags & IFF_UP))
			continue;

		if (ifsa->ss_family != hint_sa->ss_family) {
			/* Handle possible IPv4 encapsulated in IPv6 */
			if (ifsa->ss_family == AF_INET6) {
				// TODO: check if IPv4 encapsulated in IPv6 */
			}

			if (ifsa->ss_family != hint_sa->ss_family) {
				/* Otherwise, true mismatch, skip this
				 * interface.
				 */
				continue;
			}
