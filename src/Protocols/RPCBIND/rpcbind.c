...
char *mergeaddr(SVCXPRT *xprt, struct rpcb *request, SVCXPRT *srv_xprt, int af)
{
	...
	sockaddr_t *hint_sa = NULL, *serv_sa, ss, caller_v4, hint_v4;
	struct ifaddrs *ifap, *ifp = NULL, *bestif, *exactif;
	struct netbuf *serv_nbp, *hint_nbp = NULL, tbuf;
	int size = 0;

	/* Convert caller to IPv4 if it eas encapsulated. It is OK to re-use
	 * caller here because we don't need the original IPv6 form anymore.
	 */
	caller = convert_ipv6_to_ipv4(COMPONENT_DISPATCH, caller, &caller_v4);
	...
	if (request != NULL && request->r_addr != NULL &&
	    *request->r_addr != '\0') {
		...
		hint_sa = convert_ipv6_to_ipv4(COMPONENT_DISPATCH,
					       hint_nbp->buf, &hint_v4);
	...
	for (ifap = ifp; ifap != NULL; ifap = ifap->ifa_next) {
		sockaddr_t *ifsa, *ifmasksa, ifsa_v4, ifmasksa_v4;

		/* No address or interface not up, skip */
		if (ifap->ifa_addr == NULL || !(ifap->ifa_flags & IFF_UP))
			continue;

		/* Get interface address, converted to v4 if necessary */
		ifsa = convert_ipv6_to_ipv4(COMPONENT_DISPATCH,
					    (sockaddr_t *)ifap->ifa_addr,
					    &ifsa_v4);

		ifmasksa = (sockaddr_t *)ifap->ifa_netmask;

		if (ifsa == &ifsa_v4) {
			/* Need to convert mask to V4 also */
			void *ab;
			struct sockaddr_in *paddr;

			paddr = (struct sockaddr_in *)&ifmasksa_v4;

			memset(&ifmasksa_v4, 0, sizeof(ifmasksa_v4));
			ab = &(SA2SIN6(ifmasksa)->sin6_addr.s6_addr[12]);

			paddr->sin_port = SA2SIN6(ifmasksa)->sin6_port;
			paddr->sin_addr.s_addr = *(in_addr_t *)ab;
			ifmasksa_v4.ss_family = AF_INET;

			if (isMidDebug(COMPONENT_DISPATCH)) {
				char ips4[SOCK_NAME_MAX];
				char ips6[SOCK_NAME_MAX];
				struct display_buffer dspbuf4 = { sizeof(ips4),
								  ips4, ips4 };
				struct display_buffer dspbuf6 = { sizeof(ips6),
								  ips6, ips6 };

				display_sockip(&dspbuf4, &ifmasksa_v4);
				display_sockip(&dspbuf6, ifmasksa);
				LogMidDebug(
					COMPONENT_DISPATCH,
			...
			ifmasksa = (sockaddr_t *)&ifmasksa_v4;
		}
		...
		/* If after all connversions address family doesn't match, skip.
		 */
		if (ifsa->ss_family != hint_sa->ss_family)
			continue;
...
found:
	...
	/* We don't need to handle IPv4 encapsulation in IPv6 here because we
	 * only use the serv_sa for the port.
	 */
...
freeit:
	...
	if (hint_nbp != NULL) {
		gsh_free(hint_nbp->buf, MEM_COMP_LIBNTIRPC);
		gsh_free(hint_nbp, MEM_COMP_LIBNTIRPC);
	}

	if (serv_nbp != NULL) {
		gsh_free(serv_nbp->buf, MEM_COMP_LIBNTIRPC);
		gsh_free(serv_nbp, MEM_COMP_LIBNTIRPC);
	}

	if (ifp != NULL)
		freeifaddrs(ifp);

	gsh_free(serv_uaddr, MEM_COMP_LIBNTIRPC);
...
bool xdr_rpcbind_dump_res(XDR *xdrs, SVCXPRT **caller_xprt)
{
	...
	for (proto = 0; proto < P_COUNT; proto++) {
		...
		gsh_free(rpcb.r_addr, MEM_COMP_LIBNTIRPC);
...
bool xdr_rpcbind_getaddrlist_res(XDR *xdrs, nfs_request_t **req)
{
	...
	gsh_free(ent.r_maddr, MEM_COMP_LIBNTIRPC);
...
void rpcbind_res_uaddr_free(nfs_res_t *res)
{
	if (res->res_uaddr != nullstr)
		gsh_free(res->res_uaddr, MEM_COMP_LIBNTIRPC);
}
...
void rpcbind_res_taddr_free(nfs_res_t *res)
{
	if (res->res_taddr != &empty_netbuf) {
		gsh_free(res->res_taddr->buf, MEM_COMP_LIBNTIRPC);
		gsh_free(res->res_taddr, MEM_COMP_LIBNTIRPC);
	}
}
