...
char *save_token(char *token, bool quot, bool esc, struct parser_state *st)
{
	...
	if (ret_tok != NULL) {
		gsh_free(new_tok, MEM_COMP_INIT);
...
void free_parse_tree(struct config_root *tree)
{
	...
	free(tree->root.filename);
	if (tree->conf_dir != NULL)
		gsh_free(tree->conf_dir, MEM_COMP_MISC);
	file = tree->files;
	while (file != NULL) {
		next_file = file->next;
		gsh_free(file->pathname, MEM_COMP_MISC);
		gsh_free(file, MEM_COMP_INIT);
		file = next_file;
	}
	free_token_tree(token_tree.root);
	gsh_free(tree, MEM_COMP_INIT);
	return;
}
