 ...
 */
static struct avltree avl_token;
				  ...
				  const struct avltree_node *rhs)
{
	...
	return token_compare_hash(lk->token_hash, rk->token_hash);
}
...
char *save_token(char *token, bool esc, struct parser_state *st)
{
	...
	if (!st->root_node->avl_token_initiated) {
		avltree_init(&avl_token, token_cmpf_hash, 0);
		st->root_node->avl_token_initiated = true;
	}
	...
	if (ret_tok != NULL) {
		gsh_free(&new_tok->token);
		gsh_free(new_tok);
		return ret_tok->token;
	}

	avltree_insert(&new_tok->token_node, &avl_token);
...
static void print_token(FILE *output, struct avltree_node *node)
{
	struct token_tab *token;

	if (node->left != NULL)
		print_token(output, node->left);
	if (node->right != NULL)
		print_token(output, node->right);
...
void print_parse_tree(FILE *output, struct config_root *tree)
{
	...
	print_token(output, avl_token.root);
	fprintf(output, "   </TOKEN_TABLE>\n");
	fprintf(output, "</SUMMARY>\n");
...
static void free_avl_token(struct avltree_node *node)
{
	struct token_tab *token;

	if (node->left != NULL)
		free_avl_token(node->left);
	if (node->right != NULL)
		free_avl_token(node->right);
	token = avltree_container_of(node, struct token_tab, token_node);
	gsh_free(token);
}
