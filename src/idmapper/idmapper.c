			 ...
			 bool *got_gid, char *at)
{
	...
	if (group) {
		...
		if (err == 0)
			return true;
		LogWarn(COMPONENT_IDMAPPER, "getgrnam_r %s failed, error: %d",
			name, err);
		if (err != ENOENT)
			return false;
#ifndef USE_NFSIDMAP
		...
		*id = gid;
		...
		LogWarn(COMPONENT_IDMAPPER, "getpwnam_r %s failed, error: %d",
			name, err);
		if (err != ENOENT)
			return false;
#ifndef USE_NFSIDMAP
...
#endif
{
	...
	if (nfs_param.nfsv4_param.use_getpwnam) {
		...
		if (!nfs_param.directory_services_param
			     .pwutils_use_fully_qualified_names) {
			/* Strip off the realm from the principal for pwnam lookup */
