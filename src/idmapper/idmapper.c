...
static bool idmapper_set_owner_domain(void)
{
	...
	owner_domain.domain.addr = gsh_strdup(domain_addr, MEM_COMP_PROTOCOL);
...
static void idmapper_clear_owner_domain(void)
{
	...
	gsh_free(owner_domain.domain.addr, MEM_COMP_PROTOCOL);
...
static int name_to_gid(const char *name, gid_t *gid)
{
	...
	do {
		buf = gsh_malloc(buflen, MEM_COMP_CLIENT);
