...
static inline bool xdr_io_data_encode(XDR *xdrs, io_data *objp)
{
	...
	if (size != size2) {
		...
		if (objp->last_iov_buf_size >= (uio->uio_vio[last].vio_length + n)) {
			uio->uio_vio[last].vio_tail = uio->uio_vio[last].vio_tail + n;
			uio->uio_vio[last].vio_wrap = uio->uio_vio[last].vio_wrap + n;
			uio->uio_vio[last].vio_length = uio->uio_vio[last].vio_length + n;
			uio->uio_count--;
			goto putbufs;
		}
...
static inline bool xdr_io_data_decode(XDR *xdrs, io_data *objp)
{
	...
	/* For rdma_reads data received seperately, so get data buffer */
	...
	if (!XDR_SETPOS(xdrs, XDR_GETENDDATAPOS(xdrs, start, RNDUP(objp->data_len)))) {
		gsh_free(objp->iov);
		objp->iov = NULL;
		return false;
	}
