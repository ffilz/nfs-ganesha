...
static inline const nfs_function_desc_t *nfs_dupreq_func(dupreq_entry_t *dv)
{
	...
	if (dv->hin.rq_prog == NFS_pcpp[P_NFS]) {
	...
	} else if (dv->hin.rq_prog == NFS_pcpp[P_MNT]) {
		switch (dv->hin.rq_vers) {
		case MOUNT_V1:
...
#ifdef _USE_NLM
	} else if (dv->hin.rq_prog == NFS_pcpp[P_NLM]) {
		switch (dv->hin.rq_vers) {
		case NLM4_VERS:
...
#endif /* _USE_NLM */
	} else if (dv->hin.rq_prog == NFS_pcpp[P_RQUOTA]) {
		switch (dv->hin.rq_vers) {
		case RQUOTAVERS:
