...
static inline void init_shared_drc(void)
{
	...
	drc->lane_hiwat = nfs_param.core_param.drc.udp.hiwat;
	if (!drc->lane_hiwat) {
		drc->lane_hiwat = 1;
	}
...
static inline uint32_t nfs_dupreq_ref_drc(drc_t *drc, drc_lane_t *lane)
{
	...
	refcnt = atomic_inc_uint32_t(&lane->refcnt);

	LogFullDebug(COMPONENT_DUPREQ,
		"drc %p lane %p ix %d refcnt %" PRIu32, drc, lane,
		lane->ix, refcnt);
					...
					const char *tag)
{
	...
	refcnt = atomic_dec_uint32_t(&lane->refcnt);

	LogFullDebug(COMPONENT_DUPREQ,
		"tag %s drc %p lane %p ix %d refcnt %" PRIu32, tag, drc, lane,
		lane->ix, refcnt);
	if (refcnt > 4294960000) {
		LogFullDebug(COMPONENT_DUPREQ,
			"tag %s underrun drc %p lane %p ix %d refcnt %" PRIu32,
			tag, drc, lane, lane->ix, refcnt);
 ...
 *
 * @param[in] req  The svc_req being processed.
 * @param[in] lane On success, lane corresponding to req.
 * @param[in] flags Control flags, esp, requirements to lock lane.
 *
 * @return The ref'd DRC if sucessfully located, else NULL.
 */
static /* inline */ drc_t *
nfs_dupreq_get_drc(struct svc_req *req, drc_lane_t **olane, uint32_t flags)
{
...
retry:
	...
	case DRC_TCP_V3:
		...
		if (drc) {
			...
			/* count xprt's ref against lane[0].refcnt */
			xprt_lane = drc_lane_of_ix(drc, 0);
			(void)nfs_dupreq_ref_drc(drc, xprt_lane);
 ...
 */
void nfs_dupreq_put_drc(drc_t *drc, drc_lane_t *lane, const char *tag)
{
	LogFullDebug(COMPONENT_DUPREQ, "tag %s drc %p lane %d", tag, drc,
		lane->ix);

	(void) nfs_dupreq_unref_drc(drc, lane, __func__);
...
/**
 * @brief Return transport ref on DRC, freeing it
 *
...
{
	uint32_t ix, refcnt, waitcnt;
	drc_lane_t *xprt_lane;
	struct timespec ts;

	xprt_lane = drc_lane_of_ix(drc, 0);
	(void)nfs_dupreq_unref_drc(drc, xprt_lane, __func__);
		...
		if (refcnt > 0) {
			...
			if (waitcnt == 100) {
				for (ix = 0; ix < drc->nlane; ++ix) {
					...
					lane = drc_lane_of_ix(drc, ix);
					refcnt = atomic_fetch_uint32_t(
						&lane->refcnt);
					LogEvent(COMPONENT_DUPREQ,
						"waitcnt %d lane %d refs %d\n",
						waitcnt, ix, refcnt);
				}
			}
		}
...
dupreq_status_t nfs_dupreq_start(nfs_request_t *reqnfs, struct svc_req *req)
{
	...
	drc = nfs_dupreq_get_drc(req, &lane, DRC_FLAG_NONE);
...
dq_again:
 ...
 unlock:
	pthread_spin_unlock(&lane->sp);
	nfs_dupreq_put_drc(drc, lane, "nfs_dupreq_finish");
...
dupreq_status_t nfs_dupreq_delete(struct svc_req *req)
{
	...
	nfs_dupreq_put_drc(drc, lane, "nfs_dupreq_delete");
