...
void del_dbus_broadcast(struct dbus_bcast_item *to_remove)
{
	...
	gsh_free(to_remove, MEM_COMP_MISC);
}
					   ...
					   uint32_t bcast_interval, int count)
{
	...
	new_bcast = (struct dbus_bcast_item *)gsh_malloc(
		sizeof(struct dbus_bcast_item), MEM_COMP_MISC);
			       ...
			       struct gsh_dbus_interface **interfaces)
{
	...
	handler =
		gsh_malloc(sizeof(struct ganesha_dbus_handler), MEM_COMP_INIT);
	...
	if (!thread_state.dbus_conn) {
		...
		gsh_free(handler->name, MEM_COMP_MISC);
		gsh_free(handler, MEM_COMP_INIT);
		goto out;
	}
