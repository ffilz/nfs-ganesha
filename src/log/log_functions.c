...
int ReturnMatchPolicyAscii(const char *str)
{
	...
	for (i = 0; i < COND_LOG_MATCH_MAX; i++)
		if (ConditionalLogPolicy[i].policy_name &&
		    ...
		    (strcasecmp(str,
				ConditionalLogPolicy[i].policy_name) == 0 ||
				...
				void *cnode, struct config_error_type *err_type)
{
	...
	if (rc == -1) {
		LogWarn(COMPONENT_CONFIG, "Duplicate client entry found: %s",
			token);
		/*
		 * Overwrite rc to 0 so that in case of
		 * duplicate entry, it should not crash
		 */
		rc = 0;
		goto out;
	}
	...
	LogFullDebug(COMPONENT_CONFIG, "Added new client: %s", token);
					       ...
					       DBusError *error)
{
	...
	if (export_entry) {
		errormsg = "Conditional Logging already enabled for Export Id";
		goto out;
	}
						...
						DBusError *error)
{
	...
	export_entry = conditional_logging_export_match(export);
	if (!export_entry) {
		errormsg = "Conditional Logging not enabled for Export Id";
		goto out;
	}
	...
	LogEvent(COMPONENT_LOG,
		 ...
		 export->export_id);
	errormsg = "Conditional logging disable: Success";
