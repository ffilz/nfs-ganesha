					       ...
					       DBusError *error)
{
	...
	if (rc > 0) {
		errormsg =
			"Conditional logging client enable: client add failed";
		LogAlways(COMPONENT_LOG,
			  "Conditional logging client enable: "
			  "client(%s) add failed with error(%d)", arg_str, rc);
		goto out;
	}
						...
						DBusError *error)
{
	...
	removed = remove_base_client(COMPONENT_LOG, &global_client_ip_list,
				     arg_str, TERM_V4CIDR);
	if (!removed) {
		errormsg =
		...
		LogAlways(COMPONENT_LOG,
			  ...
			  "Entry not exist (%s)", arg_str);
		goto out;
	}

	errormsg = "Conditional logging client disable: Entry deleted";
	LogAlways(COMPONENT_LOG,
		  "Conditional logging client disable: Entry deleted(%s)",
		  arg_str);
