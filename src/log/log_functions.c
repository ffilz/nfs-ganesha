...
/* Default log level, can be set by -N and by LOG { default_log_level } */
log_levels_t default_log_level = NIV_EVENT;
...
static void apply_logger_config_levels(struct logger_config *logger)
{
	...
	if (logger->default_log_level != NB_LOG_LEVEL &&
	...
	for (comp = COMPONENT_LOG; comp < COMPONENT_COUNT; comp++) {
		...
		} else if (log_level_all != NB_LOG_LEVEL) {
			/* COMPONENT { ALL } was set, so use that */
			level = log_level_all;
		} else {
			/* No COMPONENT was set, use Default_Log_Level */
