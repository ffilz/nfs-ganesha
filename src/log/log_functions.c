...
int ReturnMatchPolicyAscii(const char *str)
{
	...
	for (i = 0; i < COND_LOG_MATCH_MAX; i++)
		if (ConditionalLogPolicy[i].policy_name &&
		    ...
		    (strcmp(str, ConditionalLogPolicy[i].policy_name) == 0 ||
		     strcmp(str, ConditionalLogPolicy[i].policy_str) == 0))
			return i;
	...
	glist_for_each(export_glist, &global_export_id_list) {
		...
		if (export_id_list->export_id ==
			...
			LogEvent(COMPONENT_CONFIG,
				 "Conditional Logging already "
				 "enabled for Export Id: %d",
				...
				void *cnode, struct config_error_type *err_type)
{
	...
	if (is_base_client_match(COMPONENT_CONFIG, &global_client_ip_list,
				 token, type_hint)) {
		LogAlways(COMPONENT_CONFIG, "Duplicate client entry found: %s",
			  token);
		goto out;
	}
	...
	if (rc > 0) {
		LogAlways(COMPONENT_CONFIG,
			  "Adding new client failed: %s with errcnt %d", token,
			  rc);
		goto out;
	}
			   ...
			   struct config_error_type *err_type)
{
	...
	if (errcnt == 0) {
		...
		if (logger->match_policy > COND_LOG_MATCH_ANY &&
			...
			LogChanges("Conditional logging match policy "
				   "changed to (%s)",
					       ...
					       DBusError *error)
{
	...
	if (cidr == NULL) {
		...
		LogAlways(COMPONENT_LOG,
			  "Only IP/CIDR clients are allowed via DBUS (%s)",
			  arg_str);
		goto arg_error;
	}
	cidr_free(cidr);
	...
	if (is_base_client_match(COMPONENT_LOG, &global_client_ip_list, arg_str,
				 TERM_V4CIDR)) {
		errormsg = "Conditional logging enable: client already exist";
		LogAlways(COMPONENT_LOG,
			  "Conditional logging enable: "
			  "client(%s) already exist",
			  arg_str);
		goto out;
	}

	rc = add_client(COMPONENT_LOG, &global_client_ip_list, arg_str,
			TERM_V4CIDR, NULL, NULL, NULL, NULL, NULL);

	if (rc > 0) {
		errormsg = "Conditional logging enable: client add failed";
		LogAlways(COMPONENT_LOG,
			  "Conditional logging enable: "
			  "client(%s) add failed with error(%d)",
			  arg_str, rc);
		goto out;
	}

	LogAlways(COMPONENT_LOG, "Conditional logging enable: Success(%s)",
		  arg_str);
						...
						DBusError *error)
{
	...
	if (!deleted) {
		...
		LogAlways(COMPONENT_LOG,
			  "Conditional logging disable: "
			  "Exact client not found (%s)",
			  arg_str);
		goto out;
	}
						  ...
						  DBusError *error)
{
	...
	if (glist_empty(&global_client_ip_list) == true) {
		...
		glist_for_each(client_glist, &global_client_ip_list) {
			...
			LogAlways(COMPONENT_LOG,
				  "Conditional Logging Enabled for Client(%s)",
				  cli_str);
						     ...
						     DBusError *error)
{
	...
	if (match_policy >= COND_LOG_MATCH_ANY &&
		...
		LogAlways(COMPONENT_LOG,
			  "Conditional logging Match Policy Invalid(%s)",
			  match_policy_str);
						   ...
						   DBusError *error)
{
	...
	LogAlways(COMPONENT_LOG,
		  "Current Conditional logging Match Policy : (%s)",
		  ConditionalLogPolicy[cond_log_match_policy].policy_name);
