...
{
	static bool abort_on_next_run = false;
		       ...
		       unw_word_t *out_binary_base)
{
	...
	static char scan_format_buffer[200];
	static const char *generate_scan_format =
		"%%lx-%%lx %%*s %%*s %%*s %%*s %%%d[^\n]";
	int n;
	FILE *maps_file;
	...
	while (fgets(line, BUFFER_WITH_PATH_LENGTH, maps_file) != NULL) {
		/* format: start_addr-end_addr permissions offset dev inode binary_path */
		n = snprintf(scan_format_buffer, sizeof(scan_format_buffer),
			     generate_scan_format, BUFFER_WITH_PATH_LENGTH);
		if (n <= 0)
			continue;
		sscanf(line, scan_format_buffer, &start_addr, &end_addr,
		       (char *)binary_path);
...
void gsh_libunwind_enriched_bt(void)
{
	...
	unsigned int i = 0;
	char *binary_path_to_log, *code_path_to_log, *procname_to_log;
