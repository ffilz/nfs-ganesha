...
{
	int rc = 0;
	CIDR *cidr = cidr_from_str(token);
	struct base_client_entry *client_entry;
	struct glist_head *glist;

	if (!cidr) {
		LogAlways(COMPONENT_LOG, "Invalid client IP/CIDR (%s)", token);
		goto out;
	}
	...
	glist_for_each(glist, &global_client_ip_list) {
		...
		if (cidr_equals(client_entry->client.network.cidr, cidr) == 1) {
			LogAlways(COMPONENT_LOG,
				  "Duplicate client entry ignored: %s", token);
			goto out;
		}
	}
...
add:
	...
	LogAlways(COMPONENT_CONFIG, "Added new client: %s", token);
...
conditional_logging_export_match(struct gsh_export *ctx_export)
{
	...
	/*
	 * This check is required atleast during create_pseudofs
	 * during initialization when export is set to NULL
	 */
	if (!ctx_export)
		return export_entry;
...
struct base_client_entry *conditional_logging_client_match(sockaddr_t *sockaddr)
{
	...
	/* Check if the client sockaddr already exist */
	struct base_client_entry *client_entry =
		client_match(COMPONENT_LOG, " for ConditionalLogging", sockaddr,
...
static bool dbus_conditional_log_export_disable(DBusMessageIter *args,
						...
						DBusError *error)
{
	char *errormsg = "Conditional Logging Export Disable: Ok";
	bool success = true;
	uint16_t export_id;
	struct gsh_export *export;
	struct export_id_list *export_entry = NULL;
...
{
	char *arg_str;
	char *errormsg = "Conditional logging client enable: Ok";
	bool success = true;
	struct base_client_entry *client_entry;
	struct glist_head *glist;
	DBusMessageIter iter;
	...
	if (!cidr) {
		errormsg = "Conditional logging client enable: Invalid IP/CIDR";
		LogAlways(COMPONENT_LOG,
			  "Conditional logging client enable: "
			  "Invalid client IP/CIDR (%s)",
			  arg_str);
		goto arg_error;
	}
	...
	glist_for_each(glist, &global_client_ip_list) {
		...
		if (cidr_equals(client_entry->client.network.cidr, cidr) == 1) {
			errormsg = "Conditional logging client enable: "
				   "Entry already exist";
			LogAlways(COMPONENT_LOG,
				  "Conditional logging client enable: "
				  "Entry already exist (%s)",
				  arg_str);
			goto out;
		}
	}
...
add:
	add_client(COMPONENT_LOG, &global_client_ip_list, arg_str, TERM_V4CIDR,
		   NULL, NULL, NULL, NULL, NULL);
	LogAlways(COMPONENT_LOG,
		  "Conditional logging client enable: Success(%s)", arg_str);
	errormsg = "Conditional logging client enable: Success";
...
{
	char *arg_str;
	char *errormsg = "Conditional logging client disable: Ok";
	bool success = true;
	bool found = false;
	...
	if (!cidr) {
		errormsg =
			"Conditional logging client disable: Invalid IP/CIDR";
		LogAlways(COMPONENT_LOG,
			  "Conditional logging client disable: "
			  "Invalid client IP/CIDR (%s)",
			  arg_str);
		goto arg_error;
	}
	...
	if (glist_empty(&global_client_ip_list)) {
		errormsg = "Conditional logging client disable: List Empty!!";
		LogAlways(COMPONENT_LOG,
			  "Conditional logging client disable: List Empty!!");
		goto out;
	}

	/* Check if the client sockaddr already exist */
	glist_for_each(glist, &global_client_ip_list) {
		entry = glist_entry(glist, struct base_client_entry, cle_list);

		if (cidr_equals(entry->client.network.cidr, cidr) == 1) {
			/* delete the exact match entry only */
			switch (entry->type) {
			case NETWORK_CLIENT:
				if (entry->client.network.cidr != NULL)
					cidr_free(entry->client.network.cidr);
				break;
			case NETGROUP_CLIENT:
				gsh_free(entry->client.netgroup.netgroupname);
				break;
			case WILDCARDHOST_CLIENT:
				gsh_free(entry->client.wildcard.wildcard);
				break;
			case GSSPRINCIPAL_CLIENT:
				gsh_free(entry->client.gssprinc.princname);
				break;
			case PROTO_CLIENT:
	...
	if (found) {
		errormsg = "Conditional logging client disable: Entry deleted";
		LogAlways(COMPONENT_LOG,
			  "Conditional logging client disable: "
			  "Entry deleted(%s)",
			  arg_str);
		...
		errormsg =
			"Conditional logging client disable: Entry not exist";
		LogAlways(COMPONENT_LOG,
			  "Conditional logging client disable: "
			  "Entry not exist (%s)",
			  arg_str);
