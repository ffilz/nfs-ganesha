...
TEST_F(ReaddirEmptyLatencyTest, SIMPLE)
{
	...
	status = test_dir->obj_ops->readdir(test_dir, &whence, NULL,
					    populate_dirent, 0, &eod);
...
TEST_F(ReaddirEmptyLatencyTest, SIMPLE_BYPASS)
{
	...
	status = sub_hdl->obj_ops->readdir(sub_hdl, &whence, NULL,
					   populate_dirent, 0, &eod);
	EXPECT_EQ(status.major, 0);
}
...
TEST_F(ReaddirEmptyLatencyTest, FSALREADDIR)
{
	...
	for (int i = 0; i < EMPTY_LOOP_COUNT; ++i) {
		status = fsal_readdir(test_dir, cookie, &num_entries, &eod_met,
				      0, readdir_callback, &tracker);
		EXPECT_EQ(status.major, 0);
	}
