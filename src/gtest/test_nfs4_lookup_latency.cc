...
namespace {
  ...
  protected:
    ...
    virtual void SetUp() {
      ...
      /* LOOKUP doesn't care about minor version, so just use 0 */
      data.minorversion = 0;
    ...
    virtual void TearDown() {
      ...
      /** @todo NEED to tear down arg array. */
    ...
    void setup_lookup(const char *name) {
      gsh_free(op->nfs_argop4_u.oplookup.objname.utf8string_val);
      op->argop = NFS4_OP_LOOKUP;
      op->nfs_argop4_u.oplookup.objname.utf8string_len = strlen(name);
      op->nfs_argop4_u.oplookup.objname.utf8string_val = gsh_strdup(name);
    }
...
TEST_F(LookupEmptyLatencyTest, SIMPLE)
{
  ...
  set_current_entry(&data, root_entry);
  setup_lookup(TEST_ROOT);

  enableEvents(event_list);

  rc = nfs4_op_lookup(op, &data, &resp);
...
TEST_F(LookupFullLatencyTest, BIG_SINGLE)
{
  ...
  set_current_entry(&data, test_root);
  rc = nfs4_op_lookup(op, &data, &resp);
  EXPECT_EQ(rc, NFS4_OK);
...
TEST_F(LookupFullLatencyTest, BIG)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    rc = nfs4_op_lookup(op, &data, &resp);
    EXPECT_EQ(rc, NFS4_OK);
    //EXPECT_EQ(objs[i], data.current_obj);
  }
