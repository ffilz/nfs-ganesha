...
#define TEST_ROOT "old_name"
#define TEST_ROOT_NEW "new_name"
...
namespace {
  ...
  protected:
    ...
    virtual void TearDown() {
      ...
      status = test_root->obj_ops.unlink(root_entry, test_root, TEST_ROOT_NEW);
...
TEST_F(RenameEmptyLatencyTest, SIMPLE)
{
  ...
  status = root_entry->obj_ops.rename(test_root, root_entry, TEST_ROOT, root_entry, TEST_ROOT_NEW);
  EXPECT_EQ(status.major, 0);
  root_entry->obj_ops.lookup(root_entry, TEST_ROOT_NEW, &lookup, NULL);
  EXPECT_EQ(lookup, test_root);
...
TEST_F(RenameEmptyLatencyTest, SIMPLE_BYPASS)
{
  ...
  status = sub_hdl->obj_ops.rename(test_root, sub_hdl, TEST_ROOT, sub_hdl, TEST_ROOT_NEW);
  EXPECT_EQ(status.major, 0);
  root_entry->obj_ops.lookup(root_entry, TEST_ROOT_NEW, &lookup, NULL);
  EXPECT_EQ(lookup, test_root);
...
TEST_F(RenameEmptyLatencyTest, LOOP)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = root_entry->obj_ops.rename(test_root, root_entry, fname, root_entry, fname_new);
    EXPECT_EQ(status.major, 0);
  ...
  // Rename test_root from fname to TEST_ROOT_NEW as it's expected while tear down.
  status = root_entry->obj_ops.rename(test_root, root_entry, fname, root_entry, TEST_ROOT_NEW);
  ASSERT_EQ(status.major, 0);
}
...
TEST_F(RenameEmptyLatencyTest, FSALRENAME)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = fsal_rename(root_entry, fname, root_entry, fname_new);
    EXPECT_EQ(status.major, 0);
    strncpy(fname, fname_new, NAMELEN);
  }
...
TEST_F(RenameFullLatencyTest, BIG_BYPASS)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = sub_hdl->obj_ops.rename(test_root, sub_hdl, fname, sub_hdl, TEST_ROOT_NEW);
    ASSERT_EQ(status.major, 0) << " failed to rename " << fname;
    strncpy(fname, fname_new, NAMELEN);
  }
