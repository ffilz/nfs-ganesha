...
TEST_F(RenameEmptyLatencyTest, SIMPLE_BYPASS)
{
  ...
  /* Create file for the the test */
  status = fsal_create(sub_hdl, TEST_FILE, REGULAR_FILE, &attrs, NULL, &obj,
                  NULL);
  ASSERT_EQ(status.major, 0);
  ASSERT_NE(obj, nullptr);

  status = sub_hdl->obj_ops.rename(obj, sub_hdl, TEST_FILE, sub_hdl,
                  TEST_FILE_NEW);
  EXPECT_EQ(status.major, 0);
  ...
  /* Delete file created for the test */
  status = fsal_remove(sub_hdl, TEST_FILE_NEW);
  ASSERT_EQ(status.major, 0);
}
...
TEST_F(RenameFullLatencyTest, BIG_BYPASS)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = sub_hdl->obj_ops.rename(obj, sub_hdl, fname, sub_hdl, fname_new);
    ASSERT_EQ(status.major, 0) << " failed to rename " << fname;
    strncpy(fname, fname_new, NAMELEN);
  }
