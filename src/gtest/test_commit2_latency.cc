...
namespace {
  ...
  class Commit2FullLatencyTest : public Commit2EmptyLatencyTest {
  protected:
    ...
    virtual void TearDown() {
      ...
      Commit2EmptyLatencyTest::TearDown();
    }
...
TEST_F(Commit2FullLatencyTest, BIG_CACHED)
{
  fsal_status_t status;
  struct timespec s_time, e_time;
...
TEST_F(Commit2FullLatencyTest, BIG_UNCACHED)
{
  fsal_status_t status;
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    status = obj[i]->obj_ops.commit2(obj[i], OFFSET, LENGTH);
    ASSERT_EQ(status.major, 0);
  }
...
TEST_F(Commit2FullLatencyTest, BIG_BYPASS_CACHED)
{
  fsal_status_t status;
  struct fsal_obj_handle *sub_hdl;
  struct timespec s_time, e_time;
...
TEST_F(Commit2FullLatencyTest, BIG_BYPASS_UNCACHED)
{
  fsal_status_t status;
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    status = sub_hdl[i]->obj_ops.commit2(sub_hdl[i], OFFSET, LENGTH);
    ASSERT_EQ(status.major, 0);
  }
