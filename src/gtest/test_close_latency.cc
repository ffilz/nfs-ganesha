...
TEST_F(CloseEmptyLatencyTest, SIMPLE)
{
  ...
  status = test_root->obj_ops.close(obj);
...
TEST_F(CloseEmptyLatencyTest, SIMPLE_BYPASS)
{
  fsal_status_t status;
  ...
  status = sub_hdl->obj_ops.close(sub_hdl);
  EXPECT_EQ(status.major, 0);
...
TEST_F(CloseEmptyLatencyTest, FSAL_CLOSE)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = test_root->obj_ops.open2(test_root, NULL, FSAL_O_RDWR,
		    FSAL_UNCHECKED, fname, NULL, NULL, &obj[i],
...
TEST_F(CloseFullLatencyTest, BIG)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    status = obj[i]->obj_ops.close(obj[i]);
    ASSERT_EQ(status.major, 0);
  }
...
TEST_F(CloseFullLatencyTest, BIG_BYPASS)
{
  fsal_status_t status;
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = test_root->obj_ops.open2(test_root, NULL, FSAL_O_RDWR, FSAL_UNCHECKED, fname, NULL, NULL, &obj, NULL, &caller_perm_check);
    ASSERT_EQ(status.major, 0);
    ASSERT_NE(obj, nullptr);
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    status = sub_hdl[i]->obj_ops.close(sub_hdl[i]);
    ASSERT_EQ(status.major, 0);
  }
