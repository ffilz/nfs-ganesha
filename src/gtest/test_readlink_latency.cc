...
namespace {
  ...
  protected:
    ...
    virtual void SetUp() {
      ...
      //status = symlink_test_root->obj_ops.readlink(symlink_test_root, &bfr_content, false);
      status = fsal_readlink(symlink_test_root, &bfr_content);
      ASSERT_EQ(status.major, 0);
      //ASSERT_NE(bfr_content, nullptr);
    ...
    struct gsh_buffdesc bfr_content;// =  nullptr;
  };
...
TEST_F(ReadlinkFullLatencyTest, BIG)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = symlink_test_root->obj_ops.readlink(symlink_test_root, &link_content, false);
    ASSERT_EQ(status.major, 0) << " failed to readlink " << fname;
  }
