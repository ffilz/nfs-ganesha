  ...
  protected:
    ...
    virtual void SetUp() {
      ...
      for (int i = 0; i < DIR_COUNT; ++i) {
        ...
        status = fsal_create(root_entry, fname, REGULAR_FILE, &attrs, NULL,
                   &obj, &attrs_out);
        ASSERT_EQ(status.major, 0);
...
TEST_F(MknodeEmptyLatencyTest, SIMPLE)
{
  ...
  status = root_entry->obj_ops.mknode(root_entry, TEST_ROOT, SOCKET_FILE,
             &attrs, &mknode, NULL);
  EXPECT_EQ(status.major, 0);
...
TEST_F(MknodeEmptyLatencyTest, SIMPLE_BYPASS)
{
  ...
  status = sub_hdl->obj_ops.mknode(sub_hdl, TEST_ROOT, SOCKET_FILE, &attrs,
             &mknode, NULL);
...
TEST_F(MknodeEmptyLatencyTest, LOOP)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = root_entry->obj_ops.mknode(root_entry, fname, SOCKET_FILE, &attrs,
               &obj, NULL);
    EXPECT_EQ(status.major, 0);
    obj->obj_ops.put_ref(obj);
  }
...
TEST_F(MknodeEmptyLatencyTest, FSALCREATE)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = fsal_create(root_entry, fname, SOCKET_FILE, &attrs, NULL, &obj,
               NULL);
    EXPECT_EQ(status.major, 0);
    obj->obj_ops.put_ref(obj);
  }
