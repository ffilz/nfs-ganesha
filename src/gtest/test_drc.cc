...
namespace {
  ...
  uint32_t drc_check_dupcnt = (drc_hiwat / drc_lanes) - 1;
  ...
  public:
    ...
    LatencyWorker() = delete;

  LatencyWorker(uint32_t thr_ix)
    : thr_ix(thr_ix) {
	...
	guard.unlock();
      ...
      uint32_t req_ix = 0;
      uint32_t start_ix = (rand() % (item_wsize-1));
      for (uint32_t call_ix = start_ix; call_ix < num_calls; ++call_ix) {
  ...
  DupWorker(uint32_t thr_ix)
    ...
    void operator()() {
      ...
      uint32_t num_calls = drc_hiwat;
      assert(num_calls > 2);
      assert(num_calls < item_wsize);
	...
	r = nfs_dupreq_start(reqnfs, req);
	ASSERT_NE(req->rq_u1, nullptr);
	ASSERT_EQ(r, DUPREQ_SUCCESS); // !dup
	...
	++req_ix;
      ...
      uint32_t dup_ix = req_ix - drc_check_dupcnt;
      assert(drc_check_dupcnt > 0);

      for (; dup_ix < drc_check_dupcnt; ++dup_ix) {
	NFSRequest* cc_req = req_arr[dup_ix]; // no update
	nfs_request_t* reqnfs = &cc_req->req;
	struct svc_req* req = cc_req->get_svc_req();

	r = nfs_dupreq_start(reqnfs, req);
	ASSERT_EQ(r, DUPREQ_EXISTS);
  ...
  public:
    std::vector<LatencyWorker*> workers;
	...
	auto worker = new LatencyWorker(ix);
  ...
  class DRCDup1 : public ::testing::Test {
  public:
