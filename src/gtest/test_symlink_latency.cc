...
TEST_F(SymlinkEmptyLatencyTest, SIMPLE)
{
  ...
  status = root_entry->obj_ops.symlink(root_entry, TEST_ROOT_LINK, TEST_ROOT, &attrs, &symlink, NULL);
  EXPECT_EQ(status.major, 0);
...
TEST_F(SymlinkEmptyLatencyTest, SIMPLE_BYPASS)
{
  ...
  status = sub_hdl->obj_ops.symlink(sub_hdl, TEST_ROOT_LINK, TEST_ROOT, &attrs, &symlink, NULL);
  EXPECT_EQ(status.major, 0);
  root_entry->obj_ops.lookup(root_entry, TEST_ROOT_LINK, &lookup, NULL);
  EXPECT_EQ(lookup, symlink);
...
TEST_F(SymlinkEmptyLatencyTest, LOOP)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = root_entry->obj_ops.symlink(root_entry, fname, TEST_ROOT, &attrs, &obj, NULL);
    EXPECT_EQ(status.major, 0);
    obj->obj_ops.put_ref(obj);
  }
...
TEST_F(SymlinkEmptyLatencyTest, FSALCREATE)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = fsal_create(root_entry, fname, SYMBOLIC_LINK, &attrs, TEST_ROOT, &obj, NULL);
    EXPECT_EQ(status.major, 0);
    obj->obj_ops.put_ref(obj);
  }
...
TEST_F(SymlinkFullLatencyTest, BIG_BYPASS)
{
  ...
  for (int i = 0; i < LOOP_COUNT; ++i) {
    ...
    status = sub_hdl->obj_ops.symlink(sub_hdl, fname, TEST_ROOT, &attrs, &obj, NULL);
    ASSERT_EQ(status.major, 0) << " failed to symlink " << fname;
    obj->obj_ops.put_ref(obj);
  }
