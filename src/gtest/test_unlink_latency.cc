...
TEST_F(UnlinkEmptyLatencyTest, SIMPLE_BYPASS)
{
  ...
  status = fsal_create(sub_hdl, TEST_FILE, REGULAR_FILE, &attrs, NULL,
                  &obj, NULL);
  ASSERT_EQ(status.major, 0);
  ASSERT_NE(obj, nullptr);

  status = sub_hdl->obj_ops.unlink(sub_hdl, obj, TEST_FILE);
  EXPECT_EQ(status.major, 0);
...
TEST_F(UnlinkEmptyLatencyTest, LOOP)
{
  fsal_status_t status;
  char fname[NAMELEN];
  struct fsal_obj_handle *obj;
