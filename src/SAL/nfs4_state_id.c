 ...
 *       nfs4_check_stateid_acquire_locks function.
 *
 * @param[in]  stateid     Stateid to look up
 * @param[in]  fsal_obj    Associated file (if any)
			    ...
			    const char *tag)
{
	...
	status = nfs4_check_stateid_acquire_locks(
...
void update_stateid(state_t *state, stateid4 *resp, compound_data_t *data,
		    const char *tag)
{
	/* TODO: All calls to update_stateid should eventually migrate to use
	 * update_stateid_locked once their respective call paths have been
	 * refactored to guarantee STATELOCK is acquired.
	 */
 ...
 *
 * Identical to update_stateid, but enforces the contract that the caller
 * holds the STATELOCK protecting the state resource.
 */
void update_stateid_locked(state_t *state, stateid4 *resp,
			   compound_data_t *data, const char *tag)
{
	update_stateid(state, resp, data, tag);
}
