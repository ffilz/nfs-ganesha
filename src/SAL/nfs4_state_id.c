	...
	bool *out_seqid_lock_held_p)
{
	...
	*out_open_owner_pp = NULL;
	...
	if (should_lock) {
		...
		PTHREAD_MUTEX_lock(&state2->seqid_mutex);
		*out_seqid_lock_held_p = true;
...
failure:
	if (*out_seqid_lock_held_p) {
		PTHREAD_MUTEX_unlock(&state2->seqid_mutex);
		*out_seqid_lock_held_p = false;
	}
