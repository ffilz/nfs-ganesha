...
check_it:
	...
	/* Check if the given stateid corresponds to a delegation that has already
	 ...
	 *
	 * This list-based check ensures that if the stateid was recorded in the
	 * revoked_delegations_list at revoke time (via add_to_revoked_delegations()),
	 * we detect it here and immediately return NFS4ERR_DELEG_REVOKED to the
	 * client instead of proceeding with normal state validation.
	 */
