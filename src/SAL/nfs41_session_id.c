...
int32_t _dec_session_ref(nfs41_session_t *session, const char *func, int line)
{
	...
	if (refcnt == 0) {
		...
		for (i = 0; i < session->cb_sec_parms.sec_parms_len; ++i) {
			callback_sec_parms4 cb_sec_params =
				session->cb_sec_parms.sec_parms_val[i];
			if (cb_sec_params.cb_secflavor == AUTH_NONE) {
				/* Do nothing */
			} else if (cb_sec_params.cb_secflavor == AUTH_SYS) {
				struct authunix_parms cb_auth_sys_params =
					cb_sec_params.callback_sec_parms4_u.cbsp_sys_cred;
				gsh_free(cb_auth_sys_params.aup_machname);
				gsh_free(cb_auth_sys_params.aup_gids);
#ifdef _HAVE_GSSAPI
			} else if (cb_sec_params.cb_secflavor == RPCSEC_GSS) {
				LogFatal(COMPONENT_SESSIONS,
					"GSS callbacks unsupported, code flow should not reach here");
#endif
			}
		}
...
void remove_session_connection(nfs41_session_t *session, SVCXPRT *xprt)
{
	...
	struct display_buffer xprt_db = {sizeof(xprt_addr_string), xprt_addr_string,
		xprt_addr_string};
	...
	for (i = 0; i < session->num_conn; i++) {
		...
		if (isFullDebug(COMPONENT_SESSIONS)) {
			...
			struct display_buffer curr_xprt_db = {sizeof(curr_xprt_addr_string),
			...
			LogFullDebug(COMPONENT_SESSIONS,
				...
				xprt_addr_string, curr_xprt_addr_string);
		}

		/* During removal, the xprt address must match, and not just the
		 * socket-address. We do not want to remove a different xprt with same
		 * socket-address.
		 */
	...
	if (i >= session->num_conn) {
		PTHREAD_RWLOCK_unlock(&session->conn_lock);
		LogWarn(COMPONENT_SESSIONS,
			"Found no matching connection for input xprt addr %s", xprt_addr_string);
		return;
	}
	...
	/* Left shift by 1 all the entries present on the right of current index.
	 *
	 * Note that there can be a maximum of only NFS41_MAX_CONNECTIONS entries in
	 * the array. The array being contiguous, the NFS41_MAX_CONNECTIONS number
	 * of accesses should be faster than a smaller number of random accesses
	 * if we used a linked-list.
	 */
	...
	PTHREAD_RWLOCK_unlock(&session->conn_lock);
	LogDebug(COMPONENT_SESSIONS,
		"Successfuly removed the connection for xprt addr %s", xprt_addr_string);
}
...
void release_all_session_connections(nfs41_session_t *session)
{
	...
	for (i = 0; i < session->num_conn; i++) {
		...
		if (!xprt->xp_u1) {
			LogDebug(COMPONENT_SESSIONS,
				"Skip processing xprt FD: %d without client data", xprt->xp_fd);
		...
		for (curr_node = xprt_client_data->nfs41_session_list.next;
			...
			nfs41_session_list_entry_t *curr_session =
				glist_entry(curr_node, nfs41_session_list_entry_t, node);
	 ...
	 release_xprt_ref:
		...
		/* Release the connection-xprt's ref held by the session being destroyed */
