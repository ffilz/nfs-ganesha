...
bool nfs_get_grace_status(bool want_grace)
{
	...
	do {
		...
		/* Bump the counter */
		new = cur + GRACE_STATUS_REF_INCREMENT;
		old = __sync_val_compare_and_swap(&grace_status, cur, new);
	} while (old != cur);
	return true;
}
...
void nfs_start_grace(nfs_grace_start_t *gsp)
{
	...
	do {
		...
		if (old & GRACE_STATUS_COUNT_MASK) {
			new = old | GRACE_STATUS_CHANGE_REQ;
		} else {
			new = old | GRACE_STATUS_ACTIVE;
			new &= ~GRACE_STATUS_CHANGE_REQ;
		}

		/* If there are no changes, then we don't need to update */
		if (new == old)
			break;
		cur = __sync_val_compare_and_swap(&grace_status, old, new);
	} while (cur != old);
