...
char *parse_lstr(char *pos, char **lstr)
{
	...
	if (*pos != '(') {
		LogFullDebug(COMPONENT_NLM, "Missing open parenthesis");
		return NULL;
	}
	...
	if (*(endstr + len + 1) != ')') {
		LogFullDebug(COMPONENT_NLM, "LSTR missing closing parenthesis");
		return NULL;
	}
...
char *parse_cli_srv_netid_client(char *start, struct local_nlm_info *info)
{
	...
	*endstr = '\0';
	...
	if (*(endstr + 1) != '(') {
		LogFullDebug(COMPONENT_NLM, "Missing open parenthethis");
		return NULL;
	}
	...
	if (endstr == NULL) {
		LogFullDebug(COMPONENT_NLM, "Could not find netid addr");
		return NULL;
	}
...
int nlm_cleate_local_nlm_cb_entry(struct display_buffer *dspbuf,
				  struct local_nlm_info *info)
{
...
int create_nlm_entry(struct display_buffer *dspbuf, struct local_nlm_info *info)
{
	...
	case NLM_CALLBACK_ENTRY:
		return nlm_cleate_local_nlm_cb_entry(dspbuf, info);
