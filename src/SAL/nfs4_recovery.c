 ...
 */
	...
	{
		return atomic_fetch_uint32_t(&grace_status) &
		       GRACE_STATUS_ACTIVE;
	}
 ...
 */
	...
	void nfs_try_lift_grace(void)
	{
		...
		if (!(atomic_fetch_uint32_t(&grace_status) &
		      GRACE_STATUS_ACTIVE))
			return;
		...
		if (!in_grace) {
			...
			if (nfs_param.nfsv4_param.sticky_grace) {
				 ...
				 * has now been requested */
					...
					if (pro == old)
						break;
					cur = __sync_val_compare_and_swap(
						&grace_status, old, pro);
				} while (cur != old);
	...
	void nfs_wait_for_grace_enforcement(void)
	{
		...
		while (nfs_in_grace() && !nfs_grace_enforcing()) {
			...
			pthread_cond_timedwait(&enforcing_cond,
					       &enforcing_mutex, &timeo);
 ...
 */
	...
	static bool check_clid(nfs_client_id_t * clientid,
			       clid_entry_t * clid_ent)
	{
		...
		if (clientid->cid_recov_tag &&
		    !strncmp(clientid->cid_recov_tag, clid_ent->cl_name,
			     PATH_MAX))
			ret = true;
 ...
 * @param[in] clientid Client record
 */
	void nfs4_chk_clid_impl(nfs_client_id_t * clientid,
				clid_entry_t * *clid_ent_arg)
	{
 ...
 */
	void nfs4_record_revoke(nfs_client_id_t * delr_clid,
				nfs_fh4 * delr_handle)
	{
 ...
 */
	bool nfs4_check_deleg_reclaim(nfs_client_id_t * clid, nfs_fh4 * fhandle)
	{
		...
		b64ret = base64url_encode(fhandle->nfs_fh4_val,
					  fhandle->nfs_fh4_len, rhdlstr,
					  sizeof(rhdlstr));
		assert(b64ret != -1);
		...
		if (clid_ent) {
			...
			{
				rfh_entry =
					glist_entry(node, rdel_fh_t, rdfh_list);
 ...
 */
	...
	static int ip_match(char *ip, nfs_client_id_t *cid)
	{
		...
		LogDebug(COMPONENT_STATE,
			 "NFS Server V4 match ip %s with (%.*s)", ip, len,
			 value);
...
restart:
					  ...
					  struct config_error_type * err_type)
	{
		 ...
		 */
			...
			return rados.load_config_from_parse(parse_tree,
							    err_type);
#endif
		default:
			LogCrit(COMPONENT_CLIENTID, "Unsupported Backend %s",
