			       ...
			       const char *func, int line)
{
	...
	/* If Max_Open_State_Per_Client is enabled and too many files are open
	 * from same client then stop going ahead and return EIO
	 */
	if (nfs_param.nfsv4_param.max_open_state_per_client
	    && (atomic_fetch_uint32_t(&clientid->cid_open_state_counter)
		>= nfs_param.nfsv4_param.max_open_state_per_client)) {
		display_clientid(&dspbuf, clientid->cid_clientid);
		LogCrit(COMPONENT_STATE,
		    "Too many files(cur:%u>=limit:%u) opened by {CLIENTID %s}",
		    atomic_fetch_uint32_t(&clientid->cid_open_state_counter),
		    nfs_param.nfsv4_param.max_open_state_per_client, str);
		display_reset_buffer(&dspbuf);
