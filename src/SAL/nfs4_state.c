...
static void release_nfs4_state(enum state_type type)
{
	state_t *state;
	state_t *first;
	int errcnt = 0;
 ...
 again:
	...
	glist_for_each_safe(glist, glistn,
		...
		/* Release stats of specific type. Skip all other states
...
void state_export_release_nfs4_state(void)
{
	...
	release_nfs4_state(STATE_TYPE_LAYOUT);

	/* Release LOCK states before the SHARE states. */
	release_nfs4_state(STATE_TYPE_LOCK);

	/* Okay to delete all states */
	release_nfs4_state(STATE_TYPE_NONE);
}
