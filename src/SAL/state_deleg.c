...
/**
 * @brief Check if the file is write delegated
 *
 * Check if the file is write delegated. If yes, return the
 ...
 */
bool is_write_delegated(struct fsal_obj_handle *obj, struct gsh_client **client)
{
	...
	if (write_delegated && client)
		*client = obj->state_hdl->file.write_deleg_client->gsh_client;
