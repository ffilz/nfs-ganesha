			   ...
			   bool *prerecall)
{
	...
	*prerecall = false;
	...
	/* If some client is holding a write file descriptor donot delegate.
	 * OPEN4_SHARE_ACCESS_READ and OPEN4_SHARE_ACCESS_WRITE are handled 
	 * differently because if we are currently handling OPEN4_SHARE_ACCESS_WRITE
	 * then we would have incremented the counter before calling this function.
	 */
