...
int total_num_files_delegated_g;
			     ...
			     state_owner_t *owner)
{
	...
	if (statistics->fds_curr_delegations == 0) {
		...
		int new_total_num_files_delegated =
			atomic_inc_int32_t(&total_num_files_delegated_g);
		LogFullDebug(COMPONENT_STATE,
			"total_num_files_delegated incremented to %d",
			     ...
			     struct state_t *deleg)
{
	...
	if (statistics->fds_curr_delegations == 0) {
		...
		int new_total_num_files_delegated =
			atomic_dec_int32_t(&total_num_files_delegated_g);
		LogFullDebug(COMPONENT_STATE,
			   ...
			   bool *prerecall)
{
	...
	*prerecall = false;
	...
	if ((nfs_param.nfsv4_param.max_files_delegatable != 0) &&
		(atomic_fetch_int32_t(&total_num_files_delegated_g) >=
		nfs_param.nfsv4_param.max_files_delegatable)){
		LogFullDebug(COMPONENT_STATE,
			"Can't delegate file since Max_Files_Delegatable limit is reached");
		resok->delegation.open_delegation4_u.od_whynone.ond_why =
								WND4_RESOURCE;
		return false;
	}
