					   ...
					   nfs_client_id_t *clientid)
{
	...
	glist_for_each_safe(glist, glist_next, &revoked_delegations_list) {
		revoked = glist_entry(glist, struct revoked_delegation, list);
	...
	if (!has_revoked && clientid != NULL) {
		/* Need to clear session flags - acquire client mutex */
		...
		glist_for_each(session_glist,
			...
			session = glist_entry(session_glist, nfs41_session_t,
					      session_link);
