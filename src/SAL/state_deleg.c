			   ...
			   bool *prerecall)
{
	...
	*prerecall = false;
	...
	if (client->num_revokes > 2) { /* more than 2 revokes */
		LogFullDebug(
			COMPONENT_STATE,
			"Not granting delegation: client has too many revokes (num_revokes=%u)",
			client->num_revokes);
		resok->delegation.open_delegation4_u.od_whynone.ond_why =
			WND4_RESOURCE;
		return false;
	...
	if ((args->share_access & OPEN4_SHARE_ACCESS_READ &&
		...
		LogFullDebug(
			COMPONENT_STATE,
			"Not granting delegation: conflicting write opens (share_access=0x%x, write_opens=%d)",
			args->share_access, file_stats->fds_num_write_opens);
		resok->delegation.open_delegation4_u.od_whynone.ond_why =
			WND4_CONTENTION;
		return false;
	}
		     ...
		     struct export_perms *export_perms, uint32_t share_access)
{
	...
	if (obj->type != REGULAR_FILE) {
		LogFullDebug(
			COMPONENT_STATE,
			"Delegation not supported: object type %d is not regular file",
			obj->type);
		return false;
	}
