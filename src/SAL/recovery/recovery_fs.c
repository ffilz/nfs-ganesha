...
static void fs_create_clid_name(nfs_client_id_t *clientid)
{
	...
	char cidstr_lenx[20];
	int total_len, cidstr_lenx_len, cidstr_len, str_client_addr_len;
	...
	if (fs_convert_opaque_value_max_for_dir(&dspbuf,
		...
		if (cidstr_lenx_len >= sizeof(cidstr_lenx)) {
			...
			cidstr_lenx[sizeof(cidstr_lenx) - 1] = '\0';
		}

		total_len = cidstr_len + str_client_addr_len + 5 +
			    cidstr_lenx_len;
...
static void fs_rm_revoked_handles(char *path)
{
	...
	for (dentp = readdir(dp); dentp != NULL; dentp = readdir(dp)) {
		...
		if (snprintf(del_path, sizeof(del_path), "%s/%s",
			     path, dentp->d_name) >= sizeof(del_path)) {
			    ...
			    char *parent_path, int parent_len)
{
	...
	memcpy(path, parent_path, parent_len + 1);
	path[parent_len] = '/';
				     ...
				     add_rfh_entry_hook add_rfh_entry)
{
	...
	for (dentp = readdir(dp); dentp != NULL; dentp = readdir(dp)) {
		...
		if (tgtdir) {
			...
			if (snprintf(lopath, sizeof(lopath), "%s/%s",
				     tgtdir, dentp->d_name) >= sizeof(lopath)) {
				LogCrit(COMPONENT_CLIENTID,
					"Path %s/%s too long",
					 tgtdir, dentp->d_name);
			} else {
				  ...
				  add_rfh_entry_hook add_rfh_entry)
{
	...
	case EVENT_UPDATE_CLIENTS:
		if (snprintf(path, sizeof(path), "%s", v4_recov_dir)
		    > sizeof(path)) {
			LogCrit(COMPONENT_CLIENTID,
				"Path %s too long",
				v4_recov_dir);
	...
	case EVENT_TAKE_NODEID:
		if (snprintf(path, sizeof(path), "%s/%s/node%d",
			     NFS_V4_RECOV_ROOT, NFS_V4_RECOV_DIR,
			     gsp->nodeid)
		    > sizeof(path)) {
			LogCrit(COMPONENT_CLIENTID,
