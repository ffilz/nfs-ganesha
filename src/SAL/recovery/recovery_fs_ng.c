...
static void legacy_fs_db_migrate(void)
{
	...
	if (!ret && S_ISDIR(st.st_mode)) {
		...
		if (snprintf(pathbuf, sizeof(pathbuf), "%s.XXXXXX",
			     v4_recov_link) >= sizeof(pathbuf)) {
...
static int fs_ng_create_recov_dir(void)
{
	...
	if (snprintf(v4_recov_dir, sizeof(v4_recov_dir), "%s/%s",
	...
	if (snprintf(v4_recov_link, sizeof(v4_recov_link), "%s/%s/%s",
		     NFS_V4_RECOV_ROOT, NFS_V4_RECOV_DIR, host)
	    >= sizeof(v4_recov_link)) {
		LogCrit(COMPONENT_CLIENTID,
			"Path too long %s/%s/%s",
			NFS_V4_RECOV_ROOT, NFS_V4_RECOV_DIR, host);
		return -EINVAL;
	...
	if (snprintf(v4_recov_dir, sizeof(v4_recov_dir), "%s.XXXXXX",
		     v4_recov_link)
	    >= sizeof(v4_recov_dir)) {
		LogCrit(COMPONENT_CLIENTID,
...
static void fs_ng_swap_recov_dir(void)
{
	...
	if (snprintf(tmp_link, sizeof(tmp_link), "%s.tmp", v4_recov_link)
	    >= sizeof(tmp_link)) {
		LogCrit(COMPONENT_CLIENTID,
