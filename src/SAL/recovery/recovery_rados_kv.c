...
int rados_kv_traverse(traverse_callback_t callback, void *args,
			const char *object)
{
	int ret;
	char *key_out = NULL;
...
static void rados_kv_pop_clid_entry(char *key, char *val, void *args)
{
	int ret;
	char *dupval;
	char *cl_name, *rfh_names, *rfh_name;
	clid_entry_t *clid_ent;
	struct pop_args *pop_args = args;
