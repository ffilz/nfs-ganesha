...
{
	int rc = snprintf(key, size, "%lu", (uint64_t)clientid->cid_clientid);
...
char *rados_kv_create_val(nfs_client_id_t *clientid, size_t *size)
{
	...
	cidstr_lenx_len = snprintf(cidstr_lenx, sizeof(cidstr_lenx), "%zd",
				   cidstr_len);
	if (cidstr_lenx_len >= sizeof(cidstr_lenx)) {
		cidstr_lenx_len = sizeof(cidstr_lenx) - 1;
		cidstr_lenx[cidstr_lenx_len] = '\0';
	}

	*size = str_client_addr_len + 2 + cidstr_lenx + 1 + cidstr_len + 2;

	/* hold both long form clientid and IP */
	val = gsh_malloc(*size);
	memcpy(val, str_client_addr, str_client_addr_len);
	memcpy(val + str_client_addr_len, "-(", 2);
	memcpy(val + str_client_addr_len + 2, cidstr_lenx, cidstr_lenx_len);
	val[str_client_addr_len + 2 + cidstr_lenx] = ':';
	memcpy(str_client_addr_len + 2 + cidstr_lenx + 1], cidstr, cidstr_len);
	memcpy(val + str_client_addr_len + 2 + cidstr_lenx + 1 + cidstr_len],
	       ")", 2);
...
int rados_kv_init(void)
{
	...
	if (nfs_param.core_param.clustered) {
		if (snprintf(host, sizeof(host), "node%d", g_nodeid)
		    >= sizeof(host)) {
					...
					add_rfh_entry_hook add_rfh_entry)
{
	...
	if (snprintf(object_takeover, sizeof(object_takeover), "%s_recov",
		     gsp->ipaddr)
	    >= sizeof(object_takeover)) {
		LogCrit(COMPONENT_CLIENTID,
			"object_takeover too long %s_recov", gsp->ipaddr);
		object_takeover[sizeof(object_takeover) - 1] = '\0';
	}
	ret = rados_kv_traverse(rados_kv_pop_clid_entry, &args,
				object_takeover);
	if (ret < 0) {
