...
void fs_create_recov_dir(void)
{
	...
	snprintf(recov_root, PATH_MAX, "%s",
...
static void __fs_rm_clid(char *recov_dir, char *parent_path, int position)
{
	int err;
	char *path;
	char *segment;
	 ...
	 *clientid
	 */
	__fs_rm_clid(recov_dir, path, position + segment_len);
...
{
	__fs_rm_clid(clientid->cid_recov_tag, v4_recov_dir, 0);
}
 ...
 */
static int __fs_read_recov_clids(const char *parent_path,
				 char *clid_str,
				 char *tgtdir,
				 int takeover,
				 ...
				 add_rfh_entry_hook add_rfh_entry)
{
	...
	for (dentp = readdir(dp); dentp != NULL; dentp = readdir(dp)) {
		...
		if (tgtdir)
			rc = __fs_read_recov_clids(sub_path,
						   build_clid,
						   new_path,
						   takeover,
						   add_clid_entry,
				 ...
				 add_rfh_entry_hook add_rfh_entry)
{
	...
	rc = __fs_read_recov_clids(v4_old_dir, NULL, NULL, 0,
				   add_clid_entry,
				   add_rfh_entry);
	if (rc == -1) {
		LogEvent(COMPONENT_CLIENTID,
	...
	rc = __fs_read_recov_clids(v4_recov_dir, NULL, v4_old_dir, 0,
				   add_clid_entry,
				   add_rfh_entry);
	if (rc == -1) {
		LogEvent(COMPONENT_CLIENTID,
				  ...
				  add_rfh_entry_hook add_rfh_entry)
{
	...
	rc = __fs_read_recov_clids(path, NULL, v4_old_dir, 1,
				   add_clid_entry,
				   add_rfh_entry);
	if (rc == -1) {
		LogEvent(COMPONENT_CLIENTID,
...
static void __fs_clean_old_recov_dir(char *parent_path)
{
	DIR *dp;
	struct dirent *dentp;
	char *path = NULL;
...
{
	__fs_clean_old_recov_dir(v4_old_dir);
}
...
struct nfs4_recovery_backend_t fs_backend = {
...
void fs_backend_init(struct nfs4_recovery_backend_t **backend)
{
	*backend = &fs_backend;
}
