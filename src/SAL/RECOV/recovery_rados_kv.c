...
#define RADOS_CMD_PUT_FMT	"{ \"prefix\" : \"%s\",	\
				\"key\" : \"%s\",	\
				\"val\" : \"%s\" }"

#define RADOS_CMD_GET_FMT	"{ \"prefix\" : \"%s\",	\
				\"key\" : \"%s\" }"

#define RADOS_CMD_DEL_FMT	"{ \"prefix\" : \"%s\",	\
				\"key\" : \"%s\" }"
...
void rados_kv_init(void)
{
	...
	snprintf(user_id, NAME_MAX, "%s",
		 nfs_param.nfsv4_param.user_id);
	snprintf(rados_conf, PATH_MAX, "%s",
		 nfs_param.nfsv4_param.rados_conf);
