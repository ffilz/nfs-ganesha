...
out:
	LogAlways(COMPONENT_MIGRATION, "Got document %i", reply->type);
	freeReplyObject(reply);
	return status;
}
			     ...
			     char ****documents, int *num_document)
{
	...
	*num_document = 0;
	...
	LogAlways(COMPONENT_MIGRATION, "GET DOCUEMENT PREFIX");
	...
	do {
		reply = redisCommand(redis_context,
				     ...
				     pattern);
		if (reply == NULL) {
			LogAlways(COMPONENT_ALL, "Redis SCAN failed: %s",
				  redis_context->errstr);
			goto out;
		}
		if (reply->type == REDIS_REPLY_ERROR) {
			LogAlways(COMPONENT_ALL, "Redis SCAN error: %s",
				  reply->str);
			goto out;
		}
		...
		*documents = temp;
		...
		LogAlways(COMPONENT_ALL, "In Loop");
	...
	LogAlways(COMPONENT_ALL, "Out of Loop %i", status);
