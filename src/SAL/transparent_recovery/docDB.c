...
{
	LogEvent(COMPONENT_MIGRATION, "DATABASE: Connecting to %s:%d", ip_addr,
		 port);
	...
	if (redis_context == NULL || redis_context->err) {
		LogDebug(COMPONENT_MIGRATION, "Redis Connection failed");
		return -1;
	}
	return 0;
}
...
int set_key(const char *key, const char *value)
{
	...
	if (reply == NULL || reply->type != REDIS_REPLY_INTEGER) {
		...
		LogDebug(COMPONENT_MIGRATION, "Failed to set k-v %s-%s in DB",
			 key, value);
		return -1;
	}
...
int get_key(const char *key, char **value)
{
	...
	if (reply->len == 0) {
		freeReplyObject(reply);
		LogDebug(COMPONENT_MIGRATION, "Failed to get key %s", key);
		return -1;
	}
...
int del_key(const char *key)
{
	...
	if (reply == NULL || reply->type != REDIS_REPLY_INTEGER) {
		if (reply)
			freeReplyObject(reply);
		LogDebug(COMPONENT_MIGRATION, "Filed to delete key %s", key);
		return 0;
	}
		 ...
		 int num_fields)
{
	LogFullDebug(COMPONENT_MIGRATION,
		     "Setting document for key %s in bucket %s", primary_key,
		     bucket);
		 ...
		 int *num_fields)
{
	...
	if (reply == NULL || reply->type != REDIS_REPLY_ARRAY ||
		...
		if (reply)
			freeReplyObject(reply);
		LogDebug(COMPONENT_MIGRATION,
			 "Failed to get key %s in bucket %s", key, bucket);
		return -1;
	}
		    ...
		    const char *new_value)
{
	...
	if (reply == NULL || reply->type != REDIS_REPLY_INTEGER) {
		if (reply)
			freeReplyObject(reply);
		LogDebug(COMPONENT_MIGRATION,
			 "Failed to update key %s in bucket %s", key, bucket);
		return -1;
	}
...
int del_document(const char *bucket, const char *key)
{
	...
	if (reply == NULL || reply->type != REDIS_REPLY_INTEGER) {
		if (reply)
			freeReplyObject(reply);
		LogDebug(COMPONENT_MIGRATION,
			 "Failed to delete key %s in bucket %s", key, bucket);
		return -1;
	}
			     ...
			     char ****documents, int *num_document)
{
	...
	sprintf(pattern, sizeof(pattern), "%s:%s:*", bucket, prefix);
	...
	do {
		...
		*documents = (char ***)realloc(
		...
		for (size_t i = 0; i < keys->elements; i++) {
			...
			if (get_document_without_bucket(key, &(*documents)[i],
							&num_fields) < 0) {
				freeReplyObject(reply);
				return -1;
			}
