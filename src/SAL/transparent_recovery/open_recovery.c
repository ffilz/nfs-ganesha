			    ...
			    struct open_info **opens)
{
	...
	get_document_with_prefix(bucket, file->open_key_prefix, &documents,
				 &num_document);
	opens = malloc(sizeof(struct open_info *) * num_document);

	for (int i = 0; i < num_document; i++) {
		opens[i] = (struct open_info *)malloc(sizeof(struct open_info));

		opens[i]->key = documents[i][1];
		opens[i]->fileid = strtoul(documents[i][3], NULL, 20);
		opens[i]->open_owner = documents[i][5];
		opens[i]->share_access = strtou(documents[i][7], NULL, 10);
		opens[i]->share_deny = strtou(documents[i][9], NULL, 10);
	}
}
