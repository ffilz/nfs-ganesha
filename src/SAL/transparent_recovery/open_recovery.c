...
bool persist_open_info(const char *bucket, struct file_metadata *file,
		       struct open_info *open)
{
	...
	LogAlways(COMPONENT_ALL, "%s %lu", file->open_key_prefix,
		  file->open_counter);
	...
	open->key = primary_key;

	LogAlways(COMPONENT_ALL, "%s %s", primary_key, open->key);
	...
	LogAlways(COMPONENT_ALL, "%s %s %s %s %s", bucket, fileid, share_access,
		  share_deny, open->key);

	if (set_document(bucket, open->key, document, 5) < 0)
		return false;
...
bool fetch_persisting_opens(const char *bucket,
			    const struct file_metadata *file,
			    struct open_info *opens)
{
	char ***documents = NULL;
	int num_document = 0;
	if (get_document_with_prefix(bucket, file->open_key_prefix, &documents,
				     &num_document) < 0) {
		LogAlways(COMPONENT_MIGRATION, "Fetching Persisting Opens");
		return false;
	}
	...
	free(documents);
	LogAlways(COMPONENT_ALL, "Fetching Opens Complete");
	return true;
}

bool delete_persisting_open(const char *bucket, const struct open_info *open)
{
	return del_document(bucket, open->key) == 0;
}
