...
bool grace_end_exchange_buckets()
{
	...
	rc = (set_key("BUCKET_NAME", bucket) ==
	      0); /* Set the bucket name in redis storage for future recovery */

	if (old_bucket !=
	    NULL) /* Delete old bucket, i.e., bucket used for checking conflicts during grace period */
		rc = (delete_bucket(old_bucket) == 0);
...
{
	LogEvent(COMPONENT_MIGRATION, "Initializing transparent recovery");

	if (connect_doc_db("127.0.0.1", 6379) <
	    0) { /* Connecting to redis instance */
		LogFatal(COMPONENT_MIGRATION,
