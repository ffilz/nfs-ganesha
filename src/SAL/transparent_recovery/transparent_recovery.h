 ...
 * }
 * 
 * - Primary Key: Open Key Prefix + Serial Number Open Counter
 * Open
 * {
 * 	Key
 * 	FileID
 ...
 * }
 * 
 * - Primary Key: Lock Key Prefix + Serial Number Lock Counter
 * Lock
 * {
 * 	Key
 * 	File ID
...
static void persist_file_md(char *bucket, uint32_t fsid_maj, uint32_t fsid_min,
			    const struct file_metadata *file)
{
	char *fileid, *open_counter, *lock_counter, primary_key;
...
static void fetch_file_md(char *bucket, uint32_t fsid_maj, uint32_t fsid_min,
			  uint64_t fileid, struct file_metadata *file)
{
	...
	file = malloc(sizeof(struct file_metadata));

	file->key = document[1];
	file->fileid = atoul(document[3]);
	file->open_key_prefix = document[5];
	file->lock_key_prefix = document[7];
	file->open_counter = atoul(document[9]);
	file->lock_counter = atoul(document[11]);
...
void persist_open_info(const char *bucket, const struct open_info *open);
...
bool is_conflicting_lock(const struct lock_info **locks,
			 const struct lock_info *lock);
