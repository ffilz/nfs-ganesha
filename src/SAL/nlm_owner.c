...
void dec_nlm_client_ref(state_nlm_client_t *client)
{
	...
	/* Since the refcnt is zero, some other thread that needs this
	 * entry may delete this nlm client from the hash table to
	 * insert its own nlm client. So expect not finding this nlm
	 * client in the hash table.
	 */
	switch (rc) {
				   ...
				   char *caller_name)
{
	...
	if (rc == HASHTABLE_SUCCESS) {
		...
		if (refcount == 0) {
			 ...
			 * and pretend as though it wasn't found in hash
			 * table. The thread that was deleting this
			 * entry will not find it in the hash table but
			 * will free this entry.
			 */
			hashtable_deletelatched(ht_nsm_client, &buffkey,
						&latch, &old_key, &buffval);
			goto not_found;
		}
