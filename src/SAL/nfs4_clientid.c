...
/* Keeping expired clients for 24 hours only, else reaper cleans it off */
#define MAX_EXPIRY_WAIT_TIME 86400
			  ...
			  bool make_stale, bool force_expire)
{
	...
	if (clientid->cid_confirmed == EXPIRED_CLIENT_ID) {
		...
		release_op_context();
		return false;
	} else if (!make_stale && !force_expire &&
...
int reap_expired_client_list(nfs_client_id_t *conflicted_client)
{
	...
	/* If feature disabled or valid client passed
	 * or max threshold not reached, then skip the reaper task
	 */
	if (!((nfs_param.nfsv4_param.expired_client_threshold) &&
	    ...
	    nfs_param.nfsv4_param.expired_client_threshold)))) {
		LogFullDebug(COMPONENT_CLIENTID,
			"Ignoring expired client list reaper Curr_Expired(%u) <= Threshold(%u)",
			atomic_fetch_uint32_t(&num_of_curr_expired_clients),
			nfs_param.nfsv4_param.expired_client_threshold);
		return count;
	}
	...
	glist_for_each_safe(expired_client_i,
		...
		if (conflicted_client
		    && (conflicted_client->cid_clientid
		    != expired_client->cid_clientid)) {
			continue;
		/* Breaking if max threshold not reached
		 * OR client had got expired within MAX_EXPIRY_WAIT_TIME
		 */
		} else if (!(((time(NULL) - expired_client->cid_last_renew)
		    >= MAX_EXPIRY_WAIT_TIME) ||
		    (atomic_fetch_uint32_t(&num_of_curr_expired_clients) >
		    nfs_param.nfsv4_param.expired_client_threshold))) {
			PTHREAD_MUTEX_unlock(&expired_client_ids_list_lock);
			return count;
