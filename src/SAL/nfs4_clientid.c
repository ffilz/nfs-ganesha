...
void free_client_id(nfs_client_id_t *clientid)
{
	...
	gsh_free(clientid->cid_recov_tag, MEM_COMP_MISC);
	...
	gsh_free(clientid, MEM_COMP_NFS4_CLIENT_ID_POOL);
}
...
{
	nfs_client_id_t *client_rec = gsh_calloc(1, client_id_pool->object_size,
...
int nfs_Init_client_id(void)
{
	...
	client_id_pool = pool_basic_init("NFS4 Client ID Pool",
