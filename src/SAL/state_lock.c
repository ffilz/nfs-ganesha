...
#define LOCK_CLIENTID_HINT_MIXED UINT64_MAX
 ...
 * @param[in] file Per-file state
 */
static void state_file_lock_clientid_note_empty(struct state_file *file)
{
	if (file != NULL)
 ...
 */
static void add_lock_to_file_list(struct state_file *file,
				  state_lock_entry_t *lock_entry)
{
...
static void remove_from_locklist(state_lock_entry_t *lock_entry)
{
	...
	if (glist_empty(&lock_entry->sle_obj->state_hdl->file.lock_list))
		state_file_lock_clientid_note_empty(
			&lock_entry->sle_obj->state_hdl->file);
