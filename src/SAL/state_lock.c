			  ...
			  bool_t overlap)
{
	...
	struct fsal_fd *open_fd = NULL;
	struct fsal_fd *lock_fd = NULL;

	lock->lock_sle_type = FSAL_POSIX_LOCK;

	if (state != NULL) {
		lock_fd = state->state_fd;
		if (state->state_type == STATE_TYPE_LOCK &&
		    state->state_data.lock.openstate != NULL)
			open_fd = state->state_data.lock.openstate->state_fd;
	}
	...
	if (fsal_export->exp_ops.fs_supports(fsal_export,
		...
		if (fsal_export->exp_ops.get_fd_size(op_ctx->fsal_export)
									== 0) {
			/* Call legacy lock_op */
			 ...
			 */
			fsal_status = entry->obj_handle->obj_ops.lock_op_fd(
					entry->obj_handle,
					open_fd,
					lock_fd,
					owner,
			  ...
			  fsal_lock_param_t *conflict)
{
	...
	if (state == NULL &&
	    op_ctx->fsal_export->exp_ops.get_fd_size(op_ctx->fsal_export)
									== 0) {
