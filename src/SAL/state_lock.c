	...
	state_owner_t *owner, state_t *state, fsal_lock_param_t *lock)
{
	...
	new_entry =
		gsh_calloc(1, sizeof(*new_entry), MEM_COMP_FILE_AND_STATE_LOCK);
...
void lock_entry_dec_ref(state_lock_entry_t *lock_entry)
{
	...
	if (refcount == 0) {
		...
		gsh_free(lock_entry, MEM_COMP_FILE_AND_STATE_LOCK);
	}
}
...
void free_cookie(state_cookie_entry_t *cookie_entry, bool unblock)
{
	...
	gsh_free(cookie, MEM_COMP_FILE_AND_STATE_LOCK);
	gsh_free(cookie_entry, MEM_COMP_FILE_AND_STATE_LOCK);
}
				      ...
				      state_cookie_entry_t **cookie_entry)
{
	...
	*cookie_entry = NULL;
	...
	buffkey.addr = gsh_malloc(cookie_size, MEM_COMP_FILE_AND_STATE_LOCK);
...
error:
	...
	gsh_free(hash_entry, MEM_COMP_FILE_AND_STATE_LOCK);
	gsh_free(buffkey.addr, MEM_COMP_FILE_AND_STATE_LOCK);
...
state_status_t state_lock_init(void)
{
	...
	state_owner_pool = (pool_t *)gsh_malloc(sizeof(pool_t),
						MEM_COMP_NFS4_STATE_OWNER_POOL);
	state_owner_pool->object_size = sizeof(state_owner_t);
	state_owner_pool->name = gsh_strdup("NFSv4 state owners",
					    MEM_COMP_NFS4_STATE_OWNER_POOL);
