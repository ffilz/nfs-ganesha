			  ...
			  fsal_lock_param_t *conflict)
{
	...
	new_lock = (struct lock_info *)malloc(sizeof(struct lock_info));
	...
	base64url_encode(owner->so_owner_val, owner->so_owner_len, rhdlstr,
			 sizeof(rhdlstr));
	new_lock->lock_owner = rhdlstr;
	new_lock->fileid = obj->fileid;
	new_lock->start = lock->lock_start;
	new_lock->length = lock->lock_length;
	new_lock->type = lock->lock_type;

	file = (struct file_metadata *)malloc(sizeof(struct file_metadata));

	res = fetch_file_md(bucket, obj->fsid.major, obj->fsid.minor,
			    obj->fileid, file);
	if (!res) {
		file->fileid = obj->fileid;
		res = persist_file_md(bucket, obj->fsid.major, obj->fsid.minor,
				      file);
	}

	if (nfs_get_grace_status(true)) {
		struct lock_info *locks = NULL;

		res = fetch_persisting_locks(bucket, file, locks);
		res = is_conflicting_lock(locks, new_lock);
...
recheck_for_conflicting_entries:
	...
	if (status == STATE_SUCCESS) {
		if (nfs_get_grace_status(true)) {
			persist_lock_info(grace_bucket, file, new_lock);
		} else
			persist_lock_info(bucket, file, new_lock);
			    ...
			    int32_t nsm_state, fsal_lock_param_t *lock)
{
	...
	if (status == STATE_SUCCESS) {
		/* delete the lock entry from the database */
		// delete_persisting_lock(bucket, lock, owner, obj);
	}
