			  ...
			  fsal_lock_param_t *conflict)
{
	...
	fetch_file_md(bucket, obj->fsid.major, obj->fsid.minor, obj->fileid,
		      file);

	if (!file) {
		file->fileid = obj->fileid;
		persist_file_md(bucket, obj->fsid.major, obj->fsid.minor, file);
	}
	...
	if (nfs_get_grace_status(true)) {
		...
		grace_conflict = is_conflicting_lock(locks, new_lock);
