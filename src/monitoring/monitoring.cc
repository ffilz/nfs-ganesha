...
class Metrics {
 public:
  ...
  prometheus::Family<prometheus::Counter> &clientRequestBytesTotal;
  prometheus::Family<prometheus::Counter> &clientResponseBytesTotal;
  ...
  prometheus::Family<prometheus::Gauge> &activeClients;
  prometheus::Family<prometheus::Gauge> &activeClientsThresholdSeconds;
...
Metrics::Metrics(prometheus::Registry &registry) :
  ...
  clientRequestsTotal(
      ...
      .Register(registry)),
  clientRequestBytesTotal(
      prometheus::BuildCounter()
      .Name("client_request_bytes_total")
      .Help("Total request bytes by client.")
      .Register(registry)),
  clientResponseBytesTotal(
      prometheus::BuildCounter()
      .Name("client_response_bytes_total")
      .Help("Total response bytes by client.")
      .Register(registry)),
  ...
  activeClients(
      ...
      .Register(registry)),
  activeClientsThresholdSeconds(
      prometheus::BuildGauge()
      .Name("active_clients_threshold_seconds")
      .Help("Timeout in seconds for a client to be considered active.")
      .Register(registry)),
  ...
  bytesReceivedTotalByOperation(
      prometheus::BuildCounter()
      .Name("nfs_request_bytes_total")
      .Help("Total request bytes.")
      .Register(registry)),
  bytesSentTotalByOperation(
      prometheus::BuildCounter()
      .Name("nfs_response_bytes_total")
  ...
  bytesReceivedTotalByOperationExport(
      prometheus::BuildCounter()
      .Name("nfs_request_bytes_by_export_total")
      .Help("Total request bytes by export.")
      .Register(registry)),
  bytesSentTotalByOperationExport(
      prometheus::BuildCounter()
      .Name("nfs_response_bytes_by_export_total")
...
static std::map<std::string, int64_t> clientActivity;
static std::unique_ptr<std::thread> monitoringThread;
static constexpr uint32_t activeThresholdSeconds = 60;

static void monitoringLoop() {
  metrics->activeClientsThresholdSeconds.Add({}).Set(activeThresholdSeconds);
  while (true) {
    ...
    // TODO(leffler): Make use of clientActivity thread safe.
    // TODO(leffler): Prune entries eventually?
    for (auto const& [key, val] : clientActivity) {
      const uint32_t durationSeconds = abs(epoch - val);
      if (durationSeconds < activeThresholdSeconds) {
        active++;
      }
    }
    metrics->activeClients.Add({}).Set(active);
    sleep (10);
  }
}
...
static void observeNfsRequest(const char *operation,
  ...
  if (client_ip != NULL) {
    ...
    metrics->clientRequestsTotal
        ...
        .Increment();
    // TODO(leffler): Make use of clientActivity thread safe.
    clientActivity.insert_or_assign(client, epoch);
  ...
  metrics->latencyByOperation
    .Add({{kOperation, operation}}, latencyBuckets)
    .Observe(latency_ms);
  ...
  metrics->latencyByOperationExport
    .Add({{kOperation, operation}, {kExport, exportLabel}}, latencyBuckets)
    .Observe(latency_ms);
}
...
void monitoring_register_export_label(const export_id_t export_id,
                                      const char* label) {
  // Only set once, as the label is returned as a char* to C code.
  if (exportLabels.find(export_id) == exportLabels.end()) {
...
void monitoring_init(const uint16_t port) {
  ...
  monitoringThread =
      std::unique_ptr<std::thread>(new std::thread(monitoringLoop));
  initialised = true;
}
...
void monitoring_nfs_io(const size_t bytes_requested,
  ...
  const size_t bytes_received = (is_write ? bytes_transferred : 0);
  const size_t bytes_sent = (is_write ? 0 : bytes_transferred);
  if (client_ip != NULL) {
    std::string client(client_ip);
    client = trimIPv6Prefix(client);
    metrics->clientRequestBytesTotal
        .Add({{kClient, client},
              {kOperation, operation}})
        .Increment(bytes_requested);
    metrics->clientResponseBytesTotal
        .Add({{kClient, client},
              {kOperation, operation}})
...
void monitoring_worker_thread_min(const uint64_t value) {
  metrics->workerThreads.Add({{"label", "min"}}).Set(value);
}

void monitoring_worker_thread_max(const uint64_t value) {
  metrics->workerThreads.Add({{"label", "max"}}).Set(value);
}
