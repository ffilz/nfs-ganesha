...
private:
	static constexpr int INVALID_SOCKET_FD = 0;
	int socket_fd_;
	bool aborted_ = false;
	...
	void send_failed() {
		if (!aborted_) {
			PERROR("Could not send metrics, aborting connection");
			close(socket_fd_);
...
void Exposer::stop() {
  keep_running_ = false;
  close(server_fd_); // Wakes up the thread
  thread_id_.join();
}
