...
void do_open(struct response *resp)
{
	...
	if (filehandles[resp->r_fpos] != NULL) {
		...
		resp->r_errno = EINVAL;
		safe_strcpy(errdetail, "fpos in use");
		safe_printf(badtoken, "%ld", resp->r_fpos);
		return;
	}
	...
	if ((resp->r_flags & O_CREAT) != 0) {
		...
		safe_strcpy(fullpath, resp->r_data);
		...
		if (rc >= 0) {
			...
			if (rc < 0)
				safe_strcpy(errdetail, "ceph_ll_create");
		} else {
			safe_printf(errdetail, "ceph_ll_walk %s", path);
		}
	} else {
		...
		if (rc >= 0) {
			...
			if (rc < 0)
				safe_strcpy(errdetail, "ceph_ll_open");
		} else {
			safe_strcpy(errdetail, "ceph_ll_walk");
		}
	}
	...
	if (rc < 0) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = -rc;
		safe_strcpy(badtoken, resp->r_data);
		return;
	}
...
void do_write(struct response *resp)
{
	...
	if (resp->r_fpos != 0 && filehandles[resp->r_fpos] == NULL) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = EBADF;
		safe_strcpy(errdetail, "Invalid file number");
		safe_printf(badtoken, "%ld", resp->r_fpos);
		return;
	}
	...
	if (rc < 0) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = -rc;
		safe_strcpy(errdetail, "Write failed");
		safe_printf(badtoken, "%lld", resp->r_length);
		return;
	}
	...
	if (rc != resp->r_length) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = EIO;
		safe_strcpy(errdetail, "Short write");
		safe_printf(badtoken, "%lld", rc);
		return;
	}
...
void do_read(struct response *resp)
{
	...
	if (rc < 0) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = -rc;
		safe_strcpy(errdetail, "Read failed");
		safe_printf(badtoken, "%lld", resp->r_length);
		return;
	}
...
void do_seek(struct response *resp)
{
	...
	if (rc < 0) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = -rc;
		safe_strcpy(errdetail, "Seek failed");
		safe_printf(badtoken, "%lld", resp->r_start);
		return;
	}
...
bool do_lock(struct response *resp, enum thread_type thread_type)
{
	...
	if (rc < 0) {
		if (rc == -EAGAIN) {
			...
			resp->r_status = STATUS_ERRNO;
			resp->r_errno = -rc;
			safe_strcpy(errdetail, "Lock failed");
			safe_printf(badtoken, "%s %lld %lld",
...
void do_hop(struct response *resp)
{
	...
	for (pos = resp->r_start; pos < resp->r_start + resp->r_length; pos++) {
		...
		if (rc < 0) {
			if (rc == -EAGAIN) {
				...
				resp->r_status = STATUS_ERRNO;
				resp->r_errno = -rc;
				safe_strcpy(errdetail, "Hop failed");
				safe_printf(badtoken, "%s %ld",
	...
	if (resp->r_status != STATUS_GRANTED) {
		...
		if (rc < 0) {
			resp->r_status = STATUS_ERRNO;
			resp->r_errno = -rc;
			safe_strcpy(errdetail, "Hop Unlock failed");
			safe_printf(badtoken, "%lld %lld", resp->r_start,
				    resp->r_length);
		}
	}
}
...
void do_unhop(struct response *resp)
{
	...
	for (pos = resp->r_start; pos < resp->r_start + resp->r_length; pos++) {
		...
		if (rc < 0) {
			resp->r_status = STATUS_ERRNO;
			resp->r_errno = -rc;
			safe_strcpy(errdetail, "Unhop failed");
			safe_printf(badtoken, "%s %ld",
				    str_lock_type(resp->r_lock_type),
				    lock.l_start);
			break;
	...
	if (resp->r_status != STATUS_GRANTED) {
		...
		if (rc < 0) {
			resp->r_status = STATUS_ERRNO;
			resp->r_errno = -rc;
			safe_strcpy(errdetail, "Unhop Unlock failed");
			safe_printf(badtoken, "%lld %lld", resp->r_start,
				    resp->r_length);
		}
	}
...
void do_unlock(struct response *resp)
{
	...
	if (rc < 0) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = -rc;
		safe_strcpy(errdetail, "Unlock failed");
...
void do_test(struct response *resp)
{
	...
	if (rc < 0) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = -rc;
		safe_strcpy(errdetail, "Test failed");
		safe_printf(badtoken, "%s %lld %lld",
			    str_lock_type(lock.l_type),
			    resp->r_start, resp->r_length);
		return;
...
void do_close(struct response *resp)
{
	...
	if (rc < 0) {
		resp->r_status = STATUS_ERRNO;
		resp->r_errno = -rc;
		safe_strcpy(errdetail, "Close failed");
		safe_printf(badtoken, "%ld", resp->r_fpos);
		return;
	}
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, options)) != EOF) {
		...
		case 'p':
			...
			oflags |= 4;
			safe_strcpy(portstr, optarg);
			port = atoi(optarg);
			break;

		case 'C':
			safe_strcpy(ceph_config, optarg);
			break;
