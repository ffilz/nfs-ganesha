...
char *SkipWhite(char *line, enum requires_more requires_more, const char *who)
{
	...
	case REQUIRES_MORE:
		if (*c == '\0' || *c == '#') {
			safe_printf(errdetail,
				    "Expected more characters on command %s",
				    who);

			if (*c == '\0')
				safe_strcpy(badtoken, "<NULL>");
			else
				safe_strcpy(badtoken, c);
		      ...
		      const char *invalid)
{
	...
	safe_strcpy(errdetail, invalid);
	safe_strncpy(badtoken, t, len);
	errno = EINVAL;
	return NULL;
}
	...
	*pclient = client;
	...
	if (client == NULL) {
		if (create) {
			...
			safe_strncpy(client->c_name, t, len);
			...
			*pclient = client;
			...
			safe_strcpy(errdetail, "Could not find client");
			errno = ENOENT;
			safe_strncpy(badtoken, t, len);
			return NULL;
		}
...
char *get_command(char *line, enum commands *cmd)
{
	...
	*cmd = NUM_COMMANDS;
	...
	safe_strcpy(errdetail, "Invalid command 3");
	safe_strcpy(badtoken, line);
	return NULL;
}
...
char *get_fpos(char *line, long int *fpos, enum requires_more requires_more)
{
	...
	if (*fpos < 0 || *fpos > MAXFPOS) {
		safe_strcpy(errdetail, "Invalid fpos");
		safe_printf(badtoken, "%ld", *fpos);
		errno = EINVAL;
		return NULL;
	}
		...
		enum requires_more requires_more)
{
	...
	if (*c != '"') {
		if (requires_more != REQUIRES_NO_MORE) {
			errno = EINVAL;
			safe_strcpy(errdetail, "Expected string");
			safe_strcpy(badtoken, c);
			return NULL;
		}
		quoted = false;
	...
	if (quoted && *c == '\0') {
		errno = EINVAL;
		safe_strcpy(errdetail, "Unterminated string");
		safe_strcpy(badtoken, t - 1);
		return NULL;
	}
	...
	if (len > max) {
		errno = EINVAL;
		safe_printf(errdetail, "String length %d longer than %d",
			    len, max);
		safe_strcpy(badtoken, t - 1);
		return NULL;
	}
	...
	resp->r_length = len;
	safe_strncpy(resp->r_data, t, len);
	c++;
...
char *get_status(char *line, struct response *resp)
{
	...
	safe_strcpy(errdetail, "Invalid status");
	safe_strncpy(badtoken, t, len);
	errno = EINVAL;
	return NULL;
}
...
char *parse_response(char *line, struct response *resp)
{
	...
	if (resp->r_original[0] == '\0')
		safe_strcpy(resp->r_original, line);
	...
	case STATUS_OK:
		...
		case CMD_UNHOP:
		case NUM_COMMANDS:
			safe_strcpy(errdetail, "Unexpected Status");
			errno = EINVAL;
			safe_printf(badtoken, "%s", str_status(resp->r_status));
			goto fail;
...
#define return_if_ne_long(expected, received, fmt)			\
	do {								\
		if (expected != -1 &&					\
		    expected != received) {				\
			safe_printf(errdetail, fmt " %ld", received);	\
			return false;					\
		}							\
	} while (0)
...
#define return_if_ne_longlong(expected, received, fmt)			\
	do {								\
		if (expected != -1 &&					\
		    expected != received) {				\
			safe_printf(errdetail, fmt " %lld", received);	\
			return false;					\
		}							\
	} while (0)
	...
	do {								\
		if (strcmp(expected, "*") != 0 &&			\
		   strcmp(expected, received) != 0) {			\
			safe_printf(errdetail, fmt " %s", received);	\
			return false;					\
		}							\
	} while (0)
...
int compare_responses(struct response *expected, struct response *received)
{
	...
	if (received == NULL) {
		safe_strcpy(errdetail, "Unexpected NULL response");
		return false;
	}
	...
	if (expected->r_client != received->r_client
		...
		safe_printf(errdetail, "Unexpected response from %s",
			    received->r_client->c_name);
		return false;
	}

	if (expected->r_cmd != received->r_cmd) {
		safe_printf(errdetail, "Unexpected command %s",
	...
	if (expected->r_status != received->r_status) {
		safe_printf(errdetail, "Unexpected status %s",
	...
	case STATUS_OK:
		...
		case CMD_UNHOP:
		case NUM_COMMANDS:
			safe_printf(errdetail, "Unexpected Status %s for %s",
				    str_status(received->r_status),
				    commands[received->r_cmd].cmd_name);
			return false;
...
char *parse_lock(char *line, struct response *req)
{
	...
	if (req->r_lock_type != F_RDLCK && req->r_lock_type != F_WRLCK) {
		errno = EINVAL;
		safe_strcpy(errdetail, "Invalid lock type");
		safe_printf(badtoken, "%s", str_lock_type(req->r_lock_type));
	}
