...
void do_accept(void)
{
	...
	safe_printf(client->c_name, "<UNKNOWN_%d>", client->c_socket);
...
struct response *process_client_response(struct client *client)
{
	...
	if (len >= 0) {
		safe_printf(client_resp->r_original, "%s %s",
			    client->c_name, line);
		...
		if (client_resp->r_cmd == CMD_HELLO) {
			...
			safe_strcpy(client->c_name, client_resp->r_data);
		}
	} else {
...
struct response *receive_response(bool watchin, long int timeout_secs)
{
	...
	if (fd == -2 && timeout_secs >= 0) {
		...
		if (fd == -3) {
			...
			safe_strcpy(resp->r_original, "-1 QUIT CANCELED");
			errno = 0;
			...
			resp->r_errno = errno;
			resp->r_tag = -1;
			safe_strcpy(resp->r_data, "Receive failed");

			safe_printf(resp->r_original,
				    "-1 QUIT ERRNO %d \"%s\" \"Receive failed\"",
				    errno, strerror(errno));

			safe_strcpy(errdetail, "Receive failed");
			safe_strcpy(badtoken, "");
		}
		return resp;
				 ...
				 const char *last, bool could_quit)
{
	...
	while (expected_responses != NULL
		...
		if (expect_resp != NULL) {
			...
			free_response(client_resp, NULL);
			safe_strcpy(errdetail, "Unexpected response");
			error();
...
void handle_quit(void)
{
	...
	if (client_list != NULL) {
		for (client = client_list; client != NULL;
			...
			safe_printf(expect_resp->r_original,
				    "EXPECT %s * QUIT OK",
				    client->c_name);
...
void mcmd_client_cmd(struct master_state *ms)
{
	...
	if (script)
		safe_printf(ms->last, "Line %4ld: %s", lno, ms->line);
	else
		safe_strcpy(ms->last, ms->line);
...
void mcmd_open_brace(struct master_state *ms)
{
	if (ms->inbrace) {
		errno = 0;
		safe_strcpy(errdetail, "Illegal nested brace");
		ms->rest = NULL;
	}
...
void mcmd_close_brace(struct master_state *ms)
{
	if (!ms->inbrace) {
		errno = 0;
		safe_strcpy(errdetail, "Unmatched close brace");
		ms->rest = NULL;
	} else if (!syntax) {
		ms->inbrace = false;
...
void mcmd_clients(struct master_state *ms)
{
	...
	while (ms->rest != NULL && ms->rest[0] != '\0'
		...
		safe_strcpy(ms->expect_resp->r_data, ms->client->c_name);

		safe_printf(ms->expect_resp->r_original,
			    "EXPECT %s * HELLO OK \"%s\"",
	...
	if (ms->count == 0) {
		errno = 0;
		safe_strcpy(errdetail, "Expected at least one client");
		ms->rest = NULL;
		return;
	}
...
void mcmd_fork(struct master_state *ms)
{
	if (ms->inbrace) {
		errno = 0;
		safe_strcpy(errdetail, "FORK command not allowed inside brace");
		ms->rest = NULL;
		return;
	}
	...
	safe_strcpy(ms->client_cmd->r_data, ms->expect_resp->r_client->c_name);
	safe_strcpy(ms->expect_resp->r_data, ms->expect_resp->r_client->c_name);

	safe_printf(ms->client_cmd->r_original,
		    "EXPECT %s * FORK OK \"%s\"",
		    ms->client->c_name, ms->client->c_name);
	...
	if (syntax) {
		...
		safe_strcpy(ms->client_cmd->r_data,
...
void mcmd_simple(struct master_state *ms)
{
	...
	case NUM_COMMANDS:
		safe_strcpy(errdetail, "Invalid command");
		errno = 0;
		ms->rest = NULL;
		break;
	}
