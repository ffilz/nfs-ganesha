...
void mcmd_client_cmd(struct console_state *cs)
{
...
void mcmd_sleep(struct console_state *cs)
{
	long int secs;
	int t_end, t_now;
...
void mcmd_open_brace(struct console_state *cs)
{
	if (cs->inbrace) {
		errno = 0;
...
void mcmd_close_brace(struct console_state *cs)
{
	if (!cs->inbrace) {
		errno = 0;
...
void mcmd_clients(struct console_state *cs)
{
	if (cs->inbrace) {
		errno = 0;
		array_strcpy(errdetail,
...
void mcmd_fork(struct console_state *cs)
{
	if (cs->inbrace) {
		errno = 0;
		array_strcpy(errdetail,
...
void mcmd_expect(struct console_state *cs)
{
...
void mcmd_simple(struct console_state *cs)
{
	array_strcpy(cs->last, cs->line);
	...
	if (cs->cmd == MCMD_SIMPLE_OK)
		cs->client_cmd->r_status = STATUS_OK;
	else if (cs->cmd == MCMD_SIMPLE_AVAILABLE)
		cs->client_cmd->r_status = STATUS_AVAILABLE;
	else if (cs->cmd == MCMD_SIMPLE_GRANTED)
		cs->client_cmd->r_status = STATUS_GRANTED;
	else if (cs->cmd == MCMD_SIMPLE_DEADLOCK)
	...
	case CMD_LOCKW:
		if (cs->cmd != MCMD_SIMPLE_DEADLOCK) {
			array_sprintf(errdetail,
	...
	case CMD_LOCK:
	case CMD_HOP:
		if (cs->cmd != MCMD_SIMPLE_DENIED
		    && cs->cmd != MCMD_SIMPLE_GRANTED) {
			array_sprintf(errdetail,
	...
	case CMD_TEST:
	case CMD_LIST:
		if (cs->cmd != MCMD_SIMPLE_AVAILABLE) {
			array_sprintf(errdetail,
