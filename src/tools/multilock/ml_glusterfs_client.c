...
char options[] = "c:qdx:s:n:p:h?v:";
char usage[] =
	"Usage: ml_glusterfs_client -s server -v volname -p port -n name [-q] [-d] [-c path]\n"
	"       ml_glusterfs_client -v volname -x script [-q] [-d] [-c path]\n"
	"       ml_glusterfs_client -v volname ceph-config [-q] [-d] [-c path]\n"
...
int main(int argc, char **argv)
{
	...
	rc = glfs_set_volfile_server(fs, "tcp", server, 24007);
	if (rc != 0) {
