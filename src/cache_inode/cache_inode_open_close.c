				       ...
				       cache_entry_t **entry)
{
 ...
 open_by_name:
	 ...
	 */
	access_mask = FSAL_EXECUTE_ACCESS;
	status = cache_inode_access(*entry, access_mask);

	if (status == CACHE_INODE_SUCCESS)
		goto out;

	reason = "after checking for executer permission";

 close_file:

	/* The permission check failed, we mist close the file and return
	 * the failed permission check status.
	 */
	LogDebug(COMPONENT_CACHE_INODE,
		 "cache_inode_access returned %s%s",
		 cache_inode_err_str(status), reason);

	fsal_status = obj_handle->obj_ops.close2(state);

	if (FSAL_IS_ERROR(fsal_status)) {
		LogDebug(COMPONENT_CACHE_INODE,
			 "FSAL close_fd failed with %s",
			 cache_inode_err_str(
 ...
 got_entry:
	...
	if (status != CACHE_INODE_SUCCESS) {
		...
		/* If just a permission error and file was opened read only,
