...
{
	std::call_once(start_once_, [this, port, server_crt, server_key, ca_crt,
				     ip_addr, ip_family]() {
		server_thread_ =
			std::thread([this, port, server_crt, server_key, ca_crt,
				     ip_addr, ip_family]() {
				gRPCServerStart(port, server_crt, server_key,
						ca_crt, ip_addr, ip_family);
			});
	});
}
...
void GrpcServer::gRPCServerStart(uint16_t port, std::string_view server_crt,
				 ...
				 std::string_view ip_addr, uint16_t ip_family)
{
	...
	{ /* Taking a lock */
        ...
        */
		if (ip_family == AF_INET6) {
			server_address_ = ("[" + std::string(ip_addr) +
					   "]:" + std::to_string(port));
		} else {
			server_address_ = (std::string(ip_addr) + ":" +
					   std::to_string(port));
		...
		grpc::SslServerCredentialsOptions::PemKeyCertPair
			key_cert_pair = { std::string(server_key),
					  std::string(server_crt) };

		ssl_opts.pem_key_cert_pairs.push_back(key_cert_pair);

		/* To validate client certificates */
		ssl_opts.pem_root_certs = ca_crt;
		...
		sockaddr_t *addr)
{
	...
	LogDebug(COMPONENT_GRPC,
		 ...
		 std::string(server_crt).c_str(),
		 std::string(server_key).c_str(), std::string(ca_crt).c_str());
    ...
    */
	if (key.empty() || cert.empty() || ca.empty()) {
		LogWarn(COMPONENT_GRPC,
			"Failed to get server key or certificate or CA certificate."
