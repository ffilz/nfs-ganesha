...
/**
 * gRPC API to show cache and lru data utilization
 */
	...
	exportService::ShowMDCacheResponse *response)
{
	...
	lru_pb->set_open_fds(lru.open_fds);
	lru_pb->set_fd_limit(lru.fd_limit);
	lru_pb->set_fd_state(static_cast<exportService::FdState>(lru.fd_state));
        ...
        exportService::GetExportDetailsResponse *response)
{
    ...
    grpc_get_export_details(request->export_id(),
                            &stats,
                            &success,
                            errmsg,
                            sizeof(errmsg));
...
#ifdef _USE_NFS3
    fill_version_ce_stats_proto(&stats.v3,
                                response->mutable_nfsv3());
#endif

    fill_version_ce_stats_proto(&stats.v40,
                                response->mutable_nfsv40());

    fill_version_ce_stats_proto(&stats.v41,
                                response->mutable_nfsv41());

    fill_version_ce_stats_proto(&stats.v42,
                                response->mutable_nfsv42());
