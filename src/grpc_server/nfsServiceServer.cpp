				 ...
				 nfsService::GetClientIdsResponse *response)
{
	try {
		...
		for (uint32_t i = 0; i < ht->parameter.index_size; ++i) {
			...
			RBT_LOOP(head_rbt, pn)
			{
				...
				client_ids.push_back(
					clientid); // Add the client ID to the list
				RBT_INCREMENT(pn);
			} // RBT_LOOP
		...
		for (auto &id : client_ids) {
			if (it != metadata.end() &&
			    it->second != "custom-client") {
				//grpc_cli call
				std::cout << "client id: " << id << std::endl;
			} else {
				response->add_client_ids(
					id); // Adds client ID to the repeated field
	...
	nfsService::GetNfsGraceResponse *response)
{
	try {
		...
		bool ingrace =
			nfs_in_grace(); // Function to check if in grace period
		// Set the response
	...
	try {
		...
		for (i = 0; i < ht->parameter.index_size; i++) {
			...
			RBT_LOOP(head_rbt, pn)
			{
				pdata = (hash_data *)RBT_OPAQ(pn);
				session_data =
					(nfs41_session_t *)pdata->val.addr;
				b64_ntop((unsigned char *)
